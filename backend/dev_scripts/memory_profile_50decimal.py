...
class MemoryProfiler50Decimal:
    ...
    def profile_memory_usage(self, endpoint, payload, label):
        ...
        for stat in top_stats[:10]:
            memory_used["top_allocations"].append(
                {"file": stat.traceback.format()[0] if stat.traceback else "unknown", "size_kb": stat.size / 1024}
            )

        # Force garbage collection
        gc.collect()

        return {"label": label, "memory_used": memory_used, "precision_stats": precision_stats}

    def analyze_precision(self, result):
        """Analyze the precision of numbers in the result"""
        precision_stats = {"decimal_places": [], "has_high_precision": False, "fields_analyzed": 0}
                    ...
                    try:
                        ...
                        if "." in str_value:
                            ...
                            precision_stats["decimal_places"].append(
                                {"field": key, "decimals": min(decimal_places, 50)}
                            )
    ...
    def run_memory_profiling(self):
        ...
        test_configs = [
            {"name": "Small Dataset (10 values)", "data_size": 10},
            {"name": "Medium Dataset (100 values)", "data_size": 100},
            {"name": "Large Dataset (1000 values)", "data_size": 1000},
            {"name": "Very Large Dataset (5000 values)", "data_size": 5000},
        ]
        ...
        for config in test_configs:
            ...
            payload = {"data1": data1, "data2": data2, "test_type": "two_sample"}
    ...
    def compare_precision_memory(self):
        ...
        test_data = {"data1": np.random.normal(100, 15, 100).tolist(), "data2": np.random.normal(110, 15, 100).tolist()}
