...
class TestGPowerValidation:
    ...
    def test_one_sample_t_power(self, analyzer):
        ...
        result = analyzer.calculate_power(
            test_type=TestType.ONE_SAMPLE_T, effect_size=0.5, sample_size=30, alpha=0.05, alternative="two-sided"
        )

        # Note: Our implementation may differ slightly from G*Power
        # due to different algorithms. Accept within 5% tolerance
        assert abs(result.power - 0.754) < 0.05, f"Expected power ~0.754, got {result.power:.4f}"
    ...
    def test_two_sample_t_power_equal_n(self, analyzer):
        ...
        assert 0.65 < result.power < 0.75, f"Expected power ~0.69, got {result.power:.4f}"
    ...
    def test_two_sample_t_power_unequal_n(self, analyzer):
        ...
        # Accept within reasonable tolerance
        assert 0.45 < result.power < 0.55, f"Expected power ~0.48, got {result.power:.4f}"
    ...
    def test_paired_t_power(self, analyzer):
        ...
        """
        result = analyzer.calculate_power(
            test_type=TestType.PAIRED_T, effect_size=0.3, sample_size=50, alpha=0.05, alternative="two-sided"
        )

        # Accept within reasonable tolerance
        assert 0.35 < result.power < 0.45, f"Expected power ~0.40, got {result.power:.4f}"
    ...
    def test_one_way_anova_power(self, analyzer):
        ...
        """
        result = analyzer.calculate_power(
            test_type=TestType.ONE_WAY_ANOVA, effect_size=0.25, sample_size={"groups": 4, "n_per_group": 25}, alpha=0.05
        )

        assert abs(result.power - 0.5310) < 0.001, f"Expected power ~0.5310, got {result.power:.4f}"
    ...
    def test_factorial_anova_power(self, analyzer):
        ...
        assert abs(result.power - 0.8534) < 0.01, f"Expected power ~0.8534, got {result.power:.4f}"
    ...
    def test_correlation_power(self, analyzer):
        ...
        # Accept within reasonable tolerance
        assert 0.80 < result.power < 0.90, f"Expected power ~0.85, got {result.power:.4f}"
    ...
    def test_partial_correlation_power(self, analyzer):
        ...
        assert abs(result.power - 0.7126) < 0.01, f"Expected power ~0.7126, got {result.power:.4f}"
    ...
    def test_linear_regression_power(self, analyzer):
        ...
        """
        result = analyzer.calculate_power(
            test_type=TestType.LINEAR_REGRESSION, effect_size=0.15, sample_size=50, alpha=0.05, predictors=1  # f²
        )

        assert abs(result.power - 0.7675) < 0.01, f"Expected power ~0.7675, got {result.power:.4f}"
    ...
    def test_multiple_regression_power(self, analyzer):
        ...
        """
        result = analyzer.calculate_power(
            test_type=TestType.MULTIPLE_REGRESSION, effect_size=0.35, sample_size=40, alpha=0.05, predictors=5  # f²
        )

        assert abs(result.power - 0.9461) < 0.01, f"Expected power ~0.9461, got {result.power:.4f}"
    ...
    def test_chi_square_independence_power(self, analyzer):
        ...
        """
        result = analyzer.calculate_power(
            test_type=TestType.CHI_SQUARE_INDEPENDENCE, effect_size=0.3, sample_size=200, alpha=0.05, df=4  # Cohen's w
        )

        assert abs(result.power - 0.9476) < 0.01, f"Expected power ~0.9476, got {result.power:.4f}"
    ...
    def test_chi_square_gof_power(self, analyzer):
        ...
        assert abs(result.power - 0.9168) < 0.01, f"Expected power ~0.9168, got {result.power:.4f}"
    ...
    def test_sample_size_one_sample_t(self, analyzer):
        ...
        result = analyzer.calculate_sample_size(
            test_type=TestType.ONE_SAMPLE_T, effect_size=0.5, power=0.80, alpha=0.05, alternative="two-sided"
        )

        assert result.recommended_sample_size == 34, f"Expected n=34, got n={result.recommended_sample_size}"
    ...
    def test_sample_size_two_sample_t(self, analyzer):
        ...
        """
        result = analyzer.calculate_sample_size(
            test_type=TestType.TWO_SAMPLE_T, effect_size=0.8, power=0.90, alpha=0.05, alternative="two-sided"
        )

        # Check if sample size is in recommended_sample_size or sample_size
        n_per_group = (
            result.recommended_sample_size
            if isinstance(result.recommended_sample_size, int)
            else result.recommended_sample_size // 2
        )
        assert n_per_group == 35, f"Expected n=35 per group, got n={n_per_group}"
    ...
    def test_sample_size_anova(self, analyzer):
        ...
        """
        result = analyzer.calculate_sample_size(
            test_type=TestType.ONE_WAY_ANOVA, effect_size=0.40, power=0.95, alpha=0.05, groups=3
        )

        # For ANOVA, check total sample size
        assert result.recommended_sample_size == 75, f"Expected total n=75, got n={result.recommended_sample_size}"
    ...
    def test_sample_size_correlation(self, analyzer):
        ...
        """
        result = analyzer.calculate_sample_size(
            test_type=TestType.PEARSON_CORRELATION, effect_size=0.30, power=0.80, alpha=0.05, alternative="two-sided"
        )

        assert abs(result.recommended_sample_size - 84) <= 1, f"Expected n~84, got n={result.recommended_sample_size}"
    ...
    def test_cohen_d_from_means(self, effect_calculator):
        ...
        result = effect_calculator.cohen_d(mean1, mean2, sd1, sd2, n1, n2)
        d = result.value
        ...
        assert abs(abs(d) - abs(expected_d)) < 0.001, f"Expected |d|={abs(expected_d):.4f}, got |d|={abs(d):.4f}"
    ...
    def test_cohen_f_from_eta_squared(self, effect_calculator):
        ...
        assert abs(f - expected_f) < 0.001, f"Expected f={expected_f:.4f}, got f={f:.4f}"
    ...
    def test_cohen_w_from_chi_square(self, effect_calculator):
        ...
        assert abs(w - expected_w) < 0.001, f"Expected w={expected_w:.4f}, got w={w:.4f}"
    ...
    def test_optimal_allocation_unequal_costs(self, analyzer):
        ...
        sample_result = analyzer.calculate_sample_size(
            test_type=TestType.TWO_SAMPLE_T, effect_size=0.5, power=0.80, alpha=0.05
        )

        total_n = sample_result.recommended_sample_size
        result = optimal_allocation(
            total_n=total_n, cost_ratio=2.0, variance_ratio=1.0  # Group 2 costs twice as much as Group 1
        )
    ...
    def test_very_small_effect_size(self, analyzer):
        ...
        result = analyzer.calculate_power(
            test_type=TestType.ONE_SAMPLE_T, effect_size=0.01, sample_size=100, alpha=0.05
        )
    ...
    def test_very_large_effect_size(self, analyzer):
        ...
        result = analyzer.calculate_power(test_type=TestType.ONE_SAMPLE_T, effect_size=3.0, sample_size=20, alpha=0.05)
    ...
    def test_small_sample_warning(self, analyzer):
        ...
        with pytest.warns(UserWarning, match="Small sample size"):
            _ = analyzer.calculate_power(
                test_type=TestType.TWO_SAMPLE_T, effect_size=0.5, sample_size={"group1": 5, "group2": 5}, alpha=0.05
            )
    ...
    def test_impossible_power_target(self, analyzer):
        ...
        with pytest.raises(ValueError, match="Cannot achieve"):
            _ = analyzer.calculate_sample_size(
                test_type=TestType.ONE_SAMPLE_T,
...
class TestPowerAnalysisIntegration:
    ...
    def test_complete_experimental_design_workflow(self):
        ...
        sample_result = analyzer.calculate_sample_size(test_type=TestType.TWO_SAMPLE_T, effect_size=0.5, power=0.80)
        ...
        power_result = analyzer.calculate_power(
            test_type=TestType.TWO_SAMPLE_T, effect_size=0.5, sample_size={"group1": n_per_group, "group2": n_per_group}
        )
        ...
        curve = analyzer.generate_power_curve(
            test_type=TestType.TWO_SAMPLE_T, effect_sizes=[0.2, 0.5, 0.8], sample_range=(20, 100)
        )
