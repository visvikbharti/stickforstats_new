...
class TestEffectSizeValidation:
    ...
    def sample_data(self):
        ...
        np.random.seed(42)
        return {
            "group1": np.array([85, 90, 88, 92, 95, 87, 89, 91, 93, 86, 88, 90, 92, 94, 87, 89, 91, 88, 90, 92]),
            "group2": np.array([78, 82, 80, 85, 83, 79, 81, 84, 86, 80, 82, 83, 85, 87, 81, 83, 84, 82, 84, 86]),
        }
    ...
    def test_cohens_d_two_sample(self, calculator, sample_data):
        ...
        result = calculator.cohens_d(
            sample_data["group1"], sample_data["group2"], pooled_sd=True, hedges_correction=False
        )

        # R gives d = 1.93
        assert abs(result.value - 1.93) < 0.05, f"Expected d~1.93, got {result.value:.2f}"
    ...
    def test_hedges_g_correction(self, calculator, sample_data):
        ...
        """
        result = calculator.hedges_g(sample_data["group1"], sample_data["group2"])

        # Hedges' g should be slightly smaller than Cohen's d
        d_result = calculator.cohens_d(sample_data["group1"], sample_data["group2"])

        assert result.value < d_result.value, "Hedges' g should be smaller than Cohen's d"
    ...
    def test_glass_delta(self, calculator, sample_data):
        ...
        """
        result = calculator.glass_delta(sample_data["group1"], sample_data["group2"])  # treatment  # control
        ...
        assert (
            abs(result.value - expected_delta) < 0.01
    ...
    def test_eta_squared(self, calculator):
        ...
        result = calculator.eta_squared(ss_effect, ss_total, df_effect, df_error, partial=False)

        expected_eta2 = ss_effect / ss_total
        assert abs(result.value - expected_eta2) < 0.001, f"Expected η²={expected_eta2:.3f}, got {result.value:.3f}"
    ...
    def test_partial_eta_squared(self, calculator):
        ...
        result = calculator.eta_squared(ss_effect, ss_total, df_effect, df_error, partial=True)

        expected_partial = ss_effect / (ss_effect + ss_error)
        assert (
            abs(result.value - expected_partial) < 0.001
    ...
    def test_omega_squared(self, calculator):
        ...
        result = calculator.omega_squared(ss_effect, ss_total, ms_error, df_effect, partial=False)

        expected_omega2 = (ss_effect - df_effect * ms_error) / (ss_total + ms_error)
        expected_omega2 = max(0, expected_omega2)  # Cannot be negative

        assert abs(result.value - expected_omega2) < 0.001, f"Expected ω²={expected_omega2:.3f}, got {result.value:.3f}"
    ...
    def test_pearson_correlation(self, calculator):
        ...
        assert 0.4 < result.value < 0.7, f"Expected moderate positive correlation, got r={result.value:.2f}"
    ...
    def test_spearman_correlation(self, calculator):
        ...
        assert result.value > 0.8, f"Expected strong monotonic correlation, got rho={result.value:.2f}"
    ...
    def test_cramers_v(self, calculator):
        ...
        table = np.array([[20, 15, 10], [10, 25, 15], [5, 10, 30]])
    ...
    def test_phi_coefficient(self, calculator):
        ...
        assert abs(result.value - expected_phi) < 0.001, f"Expected φ={expected_phi:.3f}, got {result.value:.3f}"
    ...
    def test_cohen_w(self, calculator):
        ...
        assert abs(result.value - expected_w) < 0.001, f"Expected w={expected_w:.3f}, got {result.value:.3f}"
    ...
    def test_cohen_f2(self, calculator):
        ...
        assert abs(result.value - expected_f2) < 0.001, f"Expected f²={expected_f2:.3f}, got {result.value:.3f}"
    ...
    def test_d_to_r_conversion(self):
        ...
        assert abs(r - expected_r) < 0.001, f"Expected r={expected_r:.3f}, got {r:.3f}"
    ...
    def test_r_to_d_conversion(self):
        ...
        assert abs(d - expected_d) < 0.001, f"Expected d={expected_d:.3f}, got {d:.3f}"
    ...
    def test_eta2_to_f_conversion(self):
        ...
        assert abs(f - expected_f) < 0.001, f"Expected f={expected_f:.3f}, got {f:.3f}"
...
class TestRobustEstimators:
    ...
    def test_trimmed_mean(self, estimator, contaminated_data):
        ...
        assert 95 < result.value < 105, f"Trimmed mean should be robust, got {result.value:.2f}"
        ...
        assert abs(result.value - 100) < abs(
            regular_mean - 100
    ...
    def test_winsorized_mean(self, estimator, contaminated_data):
        ...
        assert 95 < result.value < 105, f"Winsorized mean should be robust, got {result.value:.2f}"
    ...
    def test_huber_m_estimator(self, estimator, contaminated_data):
        ...
        assert 95 < result.value < 105, f"Huber estimator should be robust, got {result.value:.2f}"
    ...
    def test_tukey_biweight(self, estimator, contaminated_data):
        ...
        assert result.converged is True
        assert 95 < result.value < 105, f"Biweight estimator should be robust, got {result.value:.2f}"

        # Check zero weights for extreme outliers
        assert np.sum(result.weights == 0) > 0, "Should completely reject extreme outliers"
    ...
    def test_median_with_ci(self, estimator, contaminated_data):
        ...
        assert 95 < result.value < 105, f"Median should be robust, got {result.value:.2f}"
    ...
    def test_hodges_lehmann(self, estimator):
        ...
        assert abs(result.value - median) < 2, "HL should be close to median for symmetric data"
    ...
    def test_outlier_detection_mad(self, estimator, contaminated_data):
        ...
        assert np.sum(outliers) >= 5, f"Should detect at least 5 outliers, got {np.sum(outliers)}"

        # Check that extreme values are flagged
        assert outliers[np.argmax(contaminated_data)], "Should flag maximum value as outlier"
        assert outliers[np.argmin(contaminated_data)], "Should flag minimum value as outlier"
    ...
    def test_yuen_t_test(self):
        ...
        group1 = np.concatenate([np.random.normal(100, 10, 45), [150, 160, 170, 180, 190]])  # Outliers
        group2 = np.concatenate([np.random.normal(110, 10, 45), [40, 30, 20, 200, 210]])  # Outliers
        ...
        assert result["tm1"] < result["tm2"], "Trimmed mean1 should be less than trimmed mean2"
