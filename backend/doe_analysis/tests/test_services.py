...
class DesignGeneratorServiceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.test_factors = [
            {"name": "Temperature", "low_level": 60, "high_level": 80, "is_categorical": False},
            {"name": "Pressure", "low_level": 100, "high_level": 200, "is_categorical": False},
            {"name": "Catalyst", "is_categorical": True, "categories": ["A", "B", "C"]},
        ]
    ...
    def test_factorial_design_generation(self):
        ...
        design = self.service.generate_design(design_type="FACTORIAL", factors=self.test_factors[:2], center_points=0)
        ...
        self.assertTrue(
            any((row["Temperature_actual"] == 60 and row["Pressure_actual"] == 100) for _, row in design.iterrows())
        )
        self.assertTrue(
            any((row["Temperature_actual"] == 60 and row["Pressure_actual"] == 200) for _, row in design.iterrows())
        )
        self.assertTrue(
            any((row["Temperature_actual"] == 80 and row["Pressure_actual"] == 100) for _, row in design.iterrows())
        )
        self.assertTrue(
            any((row["Temperature_actual"] == 80 and row["Pressure_actual"] == 200) for _, row in design.iterrows())
        )
    ...
    def test_fractional_factorial_design(self):
        ...
        design = self.service.generate_design(
            ...
            + [{"name": "Time", "low_level": 30, "high_level": 60, "is_categorical": False}],
            fraction=1 / 2,
            center_points=0,
        )
    ...
    def test_central_composite_design(self):
        ...
        design = self.service.generate_design(
            design_type="CENTRAL_COMPOSITE", factors=self.test_factors[:2], alpha="rotatable", center_points=3
        )
    ...
    def test_box_behnken_design(self):
        ...
        factors = self.test_factors[:2] + [{"name": "Time", "low_level": 30, "high_level": 60, "is_categorical": False}]

        design = self.service.generate_design(design_type="BOX_BEHNKEN", factors=factors, center_points=3)
    ...
    def test_plackett_burman_design(self):
        ...
        design = self.service.generate_design(design_type="PLACKETT_BURMAN", factors=pb_factors)
    ...
    def test_categorical_factor_handling(self):
        ...
        design = self.service.generate_design(
            design_type="FACTORIAL", factors=[self.test_factors[0], self.test_factors[2]], center_points=0
        )
        ...
        for temp in [60, 80]:
            for cat in ["A", "B", "C"]:
                self.assertTrue(
                    any((row["Temperature_actual"] == temp and row["Catalyst"] == cat) for _, row in design.iterrows())
                )
...
class ModelAnalyzerServiceTests(TestCase):
    ...
    def test_response_optimization(self):
        ...
        response_goals = {"Yield": {"goal": "MAXIMIZE", "lower_bound": 70, "upper_bound": 100, "weight": 1.0}}
