...
class ReportGeneratorService:
    ...
    def __init__(self):
        ...
        self.table_style = ParagraphStyle("TableCell", parent=self.normal_style, fontSize=8, leading=10)
    ...
    def generate_report(self, analysis_results, design_info=None, optimization_results=None, include_raw_data=True):
        ...
        doc = SimpleDocTemplate(
            temp_filename, pagesize=letter, rightMargin=72, leftMargin=72, topMargin=72, bottomMargin=72
        )

        elements = []
        elements.append(Paragraph("DOE Analysis Report", self.title_style))
        elements.append(Spacer(1, 0.25 * inch))
        elements.append(Paragraph(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", self.normal_style))
    ...
    def _add_title_section(self, elements, title, experiment_name, timestamp):
        ...
        elements.append(Paragraph(f"Date: {timestamp.strftime('%Y-%m-%d %H:%M:%S')}", self.normal_style))
        elements.append(Spacer(1, 0.5 * inch))
    ...
    def _add_experiment_design_section(self, elements, experiment_design):
        ...
        elements.append(Paragraph(f"Design Type: {experiment_design.design_type}", self.normal_style))
        elements.append(Paragraph(f"Description: {experiment_design.description}", self.normal_style))
        elements.append(Spacer(1, 0.1 * inch))
        ...
        for factor in factors:
            ...
            factor_data.append([factor.name, factor.units, data_type, str(low_level), str(high_level)])
    ...
    def _add_model_analysis_results(self, elements, model_analysis):
        ...
        elements.append(Paragraph(f"Analysis Type: {model_analysis.model_type}", self.normal_style))
        elements.append(Paragraph(f"Description: {model_analysis.description}", self.normal_style))
        elements.append(Spacer(1, 0.1 * inch))
        ...
        if "anova_tables" in results:
            for response, anova in results["anova_tables"].items():
                ...
                for _, row in anova_df.iterrows():
                    anova_data.append(
                        [str(round(val, 6)) if isinstance(val, (int, float)) else str(val) for val in row]
                    )

                anova_table = Table(anova_data)
                anova_table.setStyle(
                    TableStyle(
                        [
                            ("BACKGROUND", (0, 0), (-1, 0), colors.lightgrey),
                            ("GRID", (0, 0), (-1, -1), 0.25, colors.black),
        ...
        if "model_coefficients" in results:
            for response, coeffs in results["model_coefficients"].items():
                elements.append(Paragraph(f"Model Coefficients for {response}:", self.normal_style))
        ...
        if "model_statistics" in results:
            ...
            for response, stats in results["model_statistics"].items():
                ...
                for stat, value in stats.items():
                    stats_data.append([stat, str(round(value, 6)) if isinstance(value, (int, float)) else str(value)])
    ...
    def _add_optimization_results(self, elements, optimization_analysis):
        ...
        elements.append(Paragraph(f"Optimization Type: {optimization_analysis.optimization_type}", self.normal_style))
        elements.append(Paragraph(f"Description: {optimization_analysis.description}", self.normal_style))
        elements.append(Spacer(1, 0.1 * inch))
        ...
        if "optimal_solutions" in results:
            ...
            for i, solution in enumerate(results["optimal_solutions"]):
                elements.append(
                    Paragraph(
                        f"Solution {i+1} (Desirability: {solution.get('desirability', 0):.4f}):", self.normal_style
                    )
                )
