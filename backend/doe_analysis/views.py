...
from .models import (
    ExperimentDesign, FactorDefinition, ResponseDefinition,
    ModelAnalysis, ExperimentRun, OptimizationAnalysis
)
from .api.serializers import (
    GenerateDesignSerializer, ExperimentDesignDetailSerializer,
    RunModelAnalysisSerializer, ModelAnalysisDetailSerializer,
    RunOptimizationSerializer, OptimizationAnalysisDetailSerializer
)
    ...
    def post(self, request):
        ...
        try:
            ...
            if not runs.exists():
                ...
                analysis.save()
                return Response({"error": "No experiment runs found"},
            ...
            return Response({"error": "Experiment design not found"},
    ...
    def post(self, request):
        ...
        try:
            ...
            if model_analysis.status != 'COMPLETED':
                ...
                optimization.save()
                return Response({"error": "Model analysis is not completed"},
                                status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response({"error": "Model analysis not found"},
                            status=status.HTTP_404_NOT_FOUND)
        except Exception as e:
    ...
    def post(self, request):
        ...
        try:
            ...
            if experiment.design_type not in ['PLACKETT_BURMAN', 'FRACTIONAL_FACTORIAL', 'DEFINITIVE_SCREENING']:
                return Response(
                    {"error": "Screening analysis requires a screening design type"},
                    status=status.HTTP_400_BAD_REQUEST
                )
    ...
    def get(self, request):
        ...
        examples = {
            ...
            'ccd': {
                ...
                'responses': [
                    {'name': 'Conversion', 'symbol': 'Y1', 'unit': '%', 'objective': 'maximize'}
                ]
            },
            'box_behnken': {
                ...
                'responses': [
                    {'name': 'Surface Roughness', 'symbol': 'Y1', 'unit': 'um', 'objective': 'minimize'}
                ]
            },
            'plackett_burman': {
                ...
                'responses': [
                    {'name': 'Response', 'symbol': 'Y', 'objective': 'maximize'}
                ]
            }
        }
