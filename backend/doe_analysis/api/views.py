...
class ExperimentDesignViewSet(viewsets.ModelViewSet):
    ...
    def upload_design_data(self, request, pk=None):
        ...
        try:
            ...
            return Response({"message": "Experiment data uploaded successfully"}, status=status.HTTP_200_OK)
    ...
    def export_design(self, request, pk=None):
        ...
        if not runs.exists():
            return Response({"error": "No runs found for this experiment"}, status=status.HTTP_404_NOT_FOUND)

        _ = list(experiment.factors.values_list("name", flat=True))
        _ = list(experiment.responses.values_list("name", flat=True))
        ...
        response = FileResponse(
            io.BytesIO(csv_buffer.getvalue().encode()), as_attachment=True, filename=f"{experiment.name}_design.csv"
        )
...
class ModelAnalysisViewSet(viewsets.ModelViewSet):
                ...
                if channel_layer:
                    async_to_sync(channel_layer.group_send)(
                        f"analysis_{experiment.id}",
                        {
                            ...
                            "message": {"status": "COMPLETED", "analysis_id": str(analysis.id)},
                        },
                    )
            except Exception:
                pass
    ...
    def generate_report(self, request, pk=None):
        ...
        return FileResponse(open(report_file, "rb"), as_attachment=True, filename=f"{analysis.name}_report.pdf")
...
class OptimizationAnalysisViewSet(viewsets.ModelViewSet):
                ...
                if channel_layer:
                    async_to_sync(channel_layer.group_send)(
                        f"optimization_{experiment.id}",
                        {
                            ...
                            "message": {"status": "COMPLETED", "optimization_id": str(optimization.id)},
                        },
                    )
            except Exception:
                pass
    ...
    def generate_report(self, request, pk=None):
        ...
        return FileResponse(open(report_file, "rb"), as_attachment=True, filename=f"{optimization.name}_report.pdf")
