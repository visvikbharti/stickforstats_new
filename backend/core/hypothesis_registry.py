...
from .multiplicity import (
    MultiplicityCorrector,
    CorrectionMethod,
    CorrectionResult,
    recommend_correction_method,
    generate_correction_statement,
)
...
class HypothesisRegistry:
    ...
    def __init__(
        self, session_id: Optional[str] = None, alpha: float = 0.05, auto_correct: bool = True, strict_mode: bool = True
    ):
        """
    ...
    def apply_correction(
        ...
        logger.info(f"Correction applied: {method.value} " f"({result.n_rejected}/{result.n_tests} rejected)")
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        if n_tests == 0:
            return {"session_id": self.session.session_id, "n_tests": 0, "status": "No tests registered"}
        ...
        if any(t.corrected for t in self.session.tests):
            n_significant_corrected = sum(
                1 for t in self.session.tests if t.adjusted_p_value and t.adjusted_p_value < self.session.alpha_level
            )
        ...
        if not any(t.corrected for t in self.session.tests):
            method = recommend_correction_method(n_tests, self.session.study_type, "unknown")
    ...
    def generate_report(self) -> str:
        ...
        if not any(t.corrected for t in self.session.tests) and summary["n_tests"] > 1:
            lines.append("⚠️ Multiple testing correction recommended!")
    ...
    def export_results(
        self, filepath: Optional[Path] = None, format: str = "json", force: bool = False
    ) -> Optional[str]:
        """
...
class SequentialTestingRegistry(HypothesisRegistry):
    ...
    def register_interim_analysis(
        self, analysis_number: int, information_fraction: float, tests: List[HypothesisRecord]
    ) -> bool:
        """
    ...
    def _check_stopping_rules(self, tests: List[HypothesisRecord], information: float) -> bool:
...
def create_registry(study_type: str = "exploratory", strict: bool = True) -> HypothesisRegistry:
    """
    ...
    if study_type == "sequential":
        registry = SequentialTestingRegistry(auto_correct=True, strict_mode=strict)
    else:
        registry = HypothesisRegistry(auto_correct=True, strict_mode=strict)
