...
class TestManuscriptParser(unittest.TestCase):
    ...
    def test_parse_with_tables(self):
        ...
        results_sections = [
            s for s in result.sections if s.section_type == 'results'
        ]
    ...
    def test_figure_reference_detection(self):
        ...
        if results_sections:
            ...
            self.assertTrue(
                any('figure 1' in r or 'fig' in r for r in ref_strings)
            )
...
class TestStatisticalClaimExtractor(unittest.TestCase):
    ...
    def test_extract_no_claims(self):
        ...
        text = (
            "The weather was sunny and the participants enjoyed the outdoor "
    ...
    def test_extract_assigns_sequential_ids(self):
        ...
        text = (
            "Result A: t(30) = 2.04, p = .05. "
            "Result B: F(2, 45) = 3.67, p = .034."
        )
        claims = self.extractor.extract(text, section='results')
...
class TestConsistencyValidator(unittest.TestCase):
    ...
    def test_validate_batch(self):
        ...
        claims = [
            _MockClaim(claim_id='C001', claim_type='t', statistic=2.50,
                        p_value=p1, df=(30,)),
            _MockClaim(claim_id='C002', claim_type='t', statistic=2.50,
                        p_value=0.900, df=(30,)),
        ]
...
class TestManuscriptGuardian(unittest.TestCase):
    ...
    def test_flag_reporting_issues(self):
        ...
        has_reporting_issue = any(
            'effect size' in t or 'section' in t or 'confidence' in t
...
class TestAdvancedValidators(unittest.TestCase):
    ...
    def test_multiple_testing_with_correction(self):
        ...
        claims = self._make_claims(n_claims=5)
        text = (
            "We ran five tests and applied Bonferroni correction "
    ...
    def test_power_analysis_present(self):
        ...
        text = (
            "A power analysis using G*Power indicated that a sample "
            "of 64 participants was needed for 80% power."
        )
    ...
    def test_reproducibility_software_with_version(self):
        ...
        positive_sw = [
            f for f in findings
            if f.severity == 'positive' and 'software' in f.title.lower()
        ]
    ...
    def test_reproducibility_no_software(self):
        ...
        software_findings = [
            f for f in findings if 'software' in f.title.lower()
        ]
    ...
    def test_reproducibility_data_availability(self):
        ...
        validator = ReproducibilityValidator()
        text = (
            "Data are available at https://osf.io/abc123. "
        ...
        da_findings = [
            f for f in findings
            if f.severity == 'positive' and 'data' in f.title.lower()
        ]
    ...
    def test_reproducibility_preregistration(self):
        ...
        validator = ReproducibilityValidator()
        text = (
            "This study was pre-registered at osf.io/abc123. "
            "Analyses used R version 4.3.1."
        )
        findings = validator.validate(text, [])

        prereg = [
            f for f in findings
            if f.severity == 'positive' and 'pre-registration' in f.title.lower()
        ]
    ...
    def test_repeated_measures_with_independent_t(self):
        ...
        self.assertTrue(
            any('repeated' in f.title.lower() for f in major)
        )
    ...
    def test_ordinal_data_with_pearson(self):
        ...
        findings_relevant = [
            f for f in findings if 'ordinal' in f.title.lower()
        ]
    ...
    def test_run_all_validators(self):
        ...
        severity_order = {
            'blocking': 4, 'major': 3, 'moderate': 2, 'minor': 1, 'positive': 0
        }
        if len(findings) >= 2:
        ...
        for v in validators:
            ...
            for f in findings:
                self.assertIsInstance(
                    f, ValidatorFinding, f"{v.__class__.__name__} bad finding"
                )
...
class TestDisciplineProfiles(unittest.TestCase):
    ...
    def test_severity_overrides(self):
        ...
        self.assertEqual(
            profile.severity_overrides['missing_effect_size'], 'blocking'
        )
