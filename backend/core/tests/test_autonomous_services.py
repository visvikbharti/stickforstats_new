...
class TestSmartProfiler(TestCase):
    ...
    def test_profile_numeric_dataset(self):
        ...
        self.assertIn(result.variable_summary['group']['type'],
                       ['nominal', 'binary'])
    ...
    def test_infer_group_comparison(self):
        ...
        gc_q = next(q for q in result.inferred_questions
                    if q.question_type == InferredQuestionType.GROUP_COMPARISON)
        self.assertGreater(gc_q.confidence, 0)
        self.assertTrue(any('t' in t or 'mann' in t
                            for t in gc_q.suggested_tests))
    ...
    def test_data_health_perfect(self):
        ...
        self.assertTrue(any('missing' in s.lower() or 'no' in s.lower()
    ...
    def test_data_health_missing(self):
        ...
        self.assertTrue(any('missing' in i['message'].lower()
    ...
    def test_data_health_small_sample(self):
        ...
        self.assertTrue(any('small' in i['message'].lower() or 'sample' in i['message'].lower()
                            for i in health.issues))
    ...
    def test_suggest_workflow(self):
        ...
        for qt, expected_wf in mapping.items():
            ...
            self.assertEqual(workflow, expected_wf,
                             f"Workflow for {qt} should be {expected_wf}, got {workflow}")
...
class TestCascadeEngine(TestCase):
    ...
    def test_execute_chi_square(self):
        ...
        b = np.where(a == 1,
                     rng.choice([0, 1], size=100, p=[0.7, 0.3]),
                     rng.choice([0, 1], size=100, p=[0.3, 0.7]))
        result = self.engine.execute_with_cascade(
            [a.astype(float), b.astype(float)], 'chi_square_independence')
