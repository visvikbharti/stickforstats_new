...
class OrgMixin(UserMixin):
    ...
    def create_org(self, name="Test Org", slug="test-org", tier=None, **kwargs):
        ...
        return Organization.objects.create(name=name, slug=slug, tier=tier, is_active=True, **kwargs)

    def create_membership(self, org, user, role="owner"):
        return OrganizationMembership.objects.create(organization=org, user=user, role=role, invitation_accepted=True)
...
class TestManuscriptAnalyzeEndpoint(SecureAPITestCase, UserMixin):
    ...
    def test_analyze_pdf_success(self, MockGuardian):
        ...
        pdf_file = SimpleUploadedFile("manuscript.pdf", b"%PDF-1.4 fake content", content_type="application/pdf")
    ...
    def test_analyze_tex_file(self, MockGuardian):
        ...
        tex_file = SimpleUploadedFile("paper.tex", b"\\documentclass{article}", content_type="text/x-tex")
...
class TestProjectEndpoints(SecureAPITestCase, OrgMixin):
    ...
    def test_get_project_detail(self):
        ...
        response = self.client.get(f"/api/v1/platform/projects/{project.slug}/?org={org.slug}")
    ...
    def test_get_nonexistent_project_returns_404(self):
        ...
        response = self.client.get(f"/api/v1/platform/projects/nonexistent/?org={org.slug}")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_delete_project_archives(self):
        ...
        response = self.client.delete(f"/api/v1/platform/projects/{project.slug}/?org={org.slug}")
