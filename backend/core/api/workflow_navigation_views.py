...
from core.services.workflow import NavigationController, WorkflowContext, DecisionEngine
...
class WorkflowNavigationViewSet(viewsets.ViewSet):
    ...
    def navigate(self, request):
        ...
        try:
            ...
            if success:
                ...
                return Response(
                    {
                        ...
                        "completed_steps": list(set([h["to"] for h in controller.navigation_history if h["to"]])),
                        "progress": progress["percentage"],
                        "recommendations": recommendations,
                ...
                return Response({"error": "Invalid navigation"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def validate(self, request):
        ...
        try:
            ...
            return Response({"is_valid": is_valid, "reason": reason, "errors": [] if is_valid else [reason]})
    ...
    def save_state(self, request):
        ...
        try:
            ...
            if not controller:
                return Response({"error": "No active navigation session"}, status=status.HTTP_400_BAD_REQUEST)

            # Save state
            state_id = controller.save_state()

            return Response({"success": True, "state_id": state_id})
    ...
    def load_state(self, request, workflow_id, state_id):
        ...
        try:
            ...
            if success:
                ...
                return Response({"error": "Failed to load state"}, status=status.HTTP_404_NOT_FOUND)
    ...
    def make_decision(self, request):
        ...
        try:
            workflow_id = request.data.get("workflow_id")
            request.data.get("decision_node_id")
            ...
            return Response({"success": True, "next_step": next_step, "next_decision_node": None})  # Simplified for MVP
