...
class DataProfilingViewSet(viewsets.ViewSet):
    ...
    def create(self, request):
        ...
        try:
            ...
            result = self.profiling_service.profile_uploaded_file(
                ...
                research_question=ResearchQuestion(serializer.validated_data["research_question"])
                if serializer.validated_data.get("research_question")
                else None,
            ...
            logger.info(f"User {request.user.id} profiled dataset: " f"{serializer.validated_data['file'].name}")
            ...
            return Response(
                {"error": str(e), "error_type": type(e).__name__}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def recommendations(self, request, pk=None):
        ...
        if category_filter:
            recommendations = [r for r in recommendations if r.get("test_category") == category_filter]
        ...
        return Response({"profile_id": pk, "count": len(recommendations), "recommendations": recommendations})
            ...
            return Response(
                {
                    ...
                    "overall_status": "pass" if all(r.is_met for r in results.values()) else "fail",
                }
            )
    ...
    def format_apa(self, request):
        ...
        try:
            if test_type == "t_test":
                ...
                return Response({"error": f"Unsupported test type: {test_type}"}, status=status.HTTP_400_BAD_REQUEST)
...
class AssumptionCheckingViewSet(viewsets.ViewSet):
    ...
    def check_normality(self, request):
        ...
        try:
            result = self.assumption_service.check_normality(data=data, method=method)
    ...
    def check_homogeneity(self, request):
        ...
        if not groups or len(groups) < 2:
            return Response({"error": "At least 2 groups required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        try:
            ...
            df = pd.DataFrame({"value": data_list, "group": group_labels})

            result = self.assumption_service.check_homogeneity_of_variance(data=df, groups="group", method=method)
    ...
    def check_all(self, request):
        ...
        if not test_type:
            return Response({"error": "test_type required"}, status=status.HTTP_400_BAD_REQUEST)
...
def get_async_task_status(task_id: str) -> Dict[str, Any]:
    ...
    """
    return {"task_id": task_id, "status": "pending", "progress": 0, "result": None}
