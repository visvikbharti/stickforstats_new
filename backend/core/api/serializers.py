...
from ..data_profiler import VariableType, DistributionType, DataQualityIssue
from ..test_recommender import TestCategory, ResearchQuestion
from ..services.assumption_service import AssumptionSeverity
from ..interpretation_engine import SignificanceLevel, EffectSizeMagnitude, PracticalSignificance
...
class VariableProfileSerializer(serializers.Serializer):
    ...
    variable_type = serializers.ChoiceField(choices=[(vt.value, vt.value) for vt in VariableType])

    # Statistical measures
    count = serializers.IntegerField(min_value=0)
    missing_count = serializers.IntegerField(min_value=0)
    missing_percentage = ScientificFloatField(precision=2, min_value=0, max_value=100)
    unique_count = serializers.IntegerField(min_value=0)
    unique_percentage = ScientificFloatField(precision=2, min_value=0, max_value=100)
    ...
    normality_p_value = ScientificFloatField(precision=4, min_value=0, max_value=1, required=False)

    # Distribution fitting
    distribution_type = serializers.ChoiceField(
        choices=[(dt.value, dt.value) for dt in DistributionType], required=False, allow_null=True
    )
    distribution_params = serializers.DictField(child=ScientificFloatField(precision=6), required=False)
    distribution_ks_statistic = ScientificFloatField(precision=4, min_value=0, max_value=1, required=False)
    distribution_ks_p_value = ScientificFloatField(precision=4, min_value=0, max_value=1, required=False)

    # Outliers
    outlier_count = serializers.IntegerField(min_value=0, required=False)
    outlier_percentage = ScientificFloatField(precision=2, min_value=0, max_value=100, required=False)
    outlier_indices = serializers.ListField(child=serializers.IntegerField(min_value=0), required=False)

    # Data quality
    quality_issues = serializers.ListField(
        child=serializers.ChoiceField(choices=[(dq.value, dq.value) for dq in DataQualityIssue]), required=False
    )

    # Transformations
    suggested_transformations = serializers.ListField(child=serializers.CharField(max_length=50), required=False)

    # Categorical data (if applicable)
    categories = serializers.ListField(child=serializers.CharField(), required=False)
    category_frequencies = serializers.DictField(child=serializers.IntegerField(min_value=0), required=False)
    ...
    def validate(self, data):
        ...
        if "missing_percentage" in data and "unique_percentage" in data:
            if data["missing_percentage"] > 100:
                raise ValidationError("Missing percentage cannot exceed 100%")
        ...
        if "mean" in data and "std" in data and "variance" in data:
            if data["std"] ** 2 != data["variance"]:
                ...
                if abs(data["std"] ** 2 - data["variance"]) > 0.0001:
                    logger.warning(f"Variance inconsistency: std²={data['std']**2}, " f"variance={data['variance']}")
        ...
        if "distribution_type" in data and data["distribution_type"]:
            if "distribution_params" not in data or not data["distribution_params"]:
                raise ValidationError("Distribution parameters required when distribution type specified")
...
class DatasetProfileSerializer(serializers.Serializer):
    ...
    variables = serializers.DictField(
        child=VariableProfileSerializer(), help_text="Dictionary of variable names to their profiles"
    )

    # Correlation matrix (for numerical variables)
    correlation_matrix = serializers.JSONField(required=False, help_text="Correlation matrix as nested lists or dict")

    # Missing data patterns
    missing_pattern = serializers.ChoiceField(choices=["MCAR", "MAR", "MNAR", "No missing data"], required=False)
    missing_heatmap = serializers.JSONField(required=False, help_text="Missing data pattern visualization data")

    # Multicollinearity
    multicollinearity_detected = serializers.BooleanField(default=False)
    vif_scores = serializers.DictField(child=ScientificFloatField(precision=2, min_value=0), required=False)

    # Data quality summary
    overall_quality_score = ScientificFloatField(precision=2, min_value=0, max_value=100, required=False)
    quality_warnings = serializers.ListField(child=serializers.CharField(max_length=500), required=False)

    # Statistical test suggestions
    suggested_analyses = serializers.ListField(child=serializers.CharField(max_length=100), required=False)

    # Metadata
    profiling_timestamp = serializers.DateTimeField(read_only=True)
    profiling_version = serializers.CharField(max_length=20, default="1.0.0", read_only=True)
...
class TestRecommendationSerializer(serializers.Serializer):
    ...
    test_category = serializers.ChoiceField(choices=[(tc.value, tc.value) for tc in TestCategory])

    # Recommendation strength
    confidence_score = ScientificFloatField(
        precision=3, min_value=0, max_value=1, help_text="Confidence in test appropriateness (0-1)"
    )
    priority = serializers.IntegerField(min_value=1, max_value=10, help_text="Priority ranking (1=highest)")

    # Assumptions
    assumptions_met = serializers.ListField(child=serializers.CharField(max_length=100))
    assumptions_violated = serializers.ListField(child=serializers.CharField(max_length=100))
    assumption_warnings = serializers.ListField(child=serializers.CharField(max_length=500), required=False)

    # Implementation details
    python_function = serializers.CharField(
        max_length=200, help_text="Python function to use (e.g., scipy.stats.ttest_ind)"
    )
    r_function = serializers.CharField(max_length=200, help_text="R function equivalent")

    # Parameters
    required_parameters = serializers.ListField(child=serializers.CharField(max_length=50))
    optional_parameters = serializers.DictField(child=serializers.CharField(), required=False)

    # Rationale
    rationale = serializers.CharField(help_text="Explanation of why this test is recommended")

    # Alternatives
    alternative_tests = serializers.ListField(child=serializers.CharField(max_length=100), required=False)

    # Effect size
    expected_effect_size = serializers.CharField(
        max_length=50, required=False, help_text="Expected effect size measure (e.g., Cohen's d)"
    )

    # Sample size
    min_sample_size = serializers.IntegerField(
        min_value=1, required=False, help_text="Minimum sample size for adequate power"
    )
...
class AssumptionResultSerializer(serializers.Serializer):
    ...
    severity = serializers.ChoiceField(choices=[(s.value, s.value) for s in AssumptionSeverity])

    # Statistical evidence
    test_statistic = ScientificFloatField(precision=6, required=False)
    p_value = ScientificFloatField(precision=4, min_value=0, max_value=1, required=False)
    critical_value = ScientificFloatField(precision=6, required=False)
    confidence_level = ScientificFloatField(precision=2, min_value=0, max_value=1, default=0.95)
    ...
    alternative_tests = serializers.ListField(child=serializers.CharField(max_length=100), required=False)
    transformations = serializers.ListField(child=serializers.CharField(max_length=50), required=False)
...
class InterpretationSerializer(serializers.Serializer):
    ...
    degrees_of_freedom = serializers.JSONField(required=False, help_text="Can be single value or tuple for F-tests")
    confidence_interval = serializers.ListField(
        child=ScientificFloatField(precision=4), min_length=2, max_length=2, required=False
    )
    confidence_level = ScientificFloatField(precision=2, min_value=0, max_value=1, default=0.95)
    ...
    effect_magnitude = serializers.ChoiceField(
        choices=[(em.value, em.value) for em in EffectSizeMagnitude], required=False
    )

    # Interpretations
    significance_level = serializers.ChoiceField(choices=[(sl.value, sl.value) for sl in SignificanceLevel])
    practical_significance = serializers.ChoiceField(
        choices=[(ps.value, ps.value) for ps in PracticalSignificance], required=False
    )
    ...
    common_mistakes = serializers.ListField(child=serializers.CharField(max_length=500))

    # Visualizations
    visualization_suggestions = serializers.ListField(child=serializers.CharField(max_length=200))

    # References
    references = serializers.ListField(child=serializers.CharField(max_length=500))
    further_reading = serializers.ListField(child=serializers.CharField(max_length=500), required=False)
...
class FileUploadSerializer(serializers.Serializer):
    ...
    # File specifications
    file = serializers.FileField(max_length=255, help_text="Data file (CSV, Excel, JSON, Parquet)")

    # Processing options
    target_variable = serializers.CharField(max_length=100, required=False, help_text="Dependent variable for analysis")

    research_question = serializers.ChoiceField(
        choices=[(rq.value, rq.value) for rq in ResearchQuestion], required=False, help_text="Type of research question"
    )

    # Configuration
    async_processing = serializers.BooleanField(
        default=None, required=False, help_text="Force async processing (auto-detected if not specified)"
    )

    cache_results = serializers.BooleanField(default=True, help_text="Cache results for faster subsequent access")

    # Data specifications
    delimiter = serializers.CharField(max_length=5, default=",", help_text="Delimiter for text files")

    encoding = serializers.CharField(max_length=20, default="utf-8", help_text="File encoding")

    header_row = serializers.IntegerField(min_value=0, default=0, help_text="Row number containing headers (0-indexed)")

    # Validation options
    max_rows = serializers.IntegerField(min_value=1, required=False, help_text="Maximum rows to process (for testing)")

    missing_threshold = ScientificFloatField(
        precision=2, min_value=0, max_value=100, default=50, help_text="Maximum allowed missing percentage per variable"
    )
    ...
    def validate_file(self, value):
        ...
        if value.size > max_size_mb * 1024 * 1024:
            raise ValidationError(f"File size {value.size / 1024 / 1024:.1f}MB exceeds " f"maximum {max_size_mb}MB")
        ...
        if file_ext not in valid_extensions:
            raise ValidationError(
                f"Unsupported file format '{file_ext}'. " f"Supported formats: {', '.join(valid_extensions)}"
            )
        ...
        if file_ext == ".csv" or file_ext == ".tsv":
            ...
            for pattern in dangerous_patterns:
                if first_bytes.startswith(pattern):
                    logger.warning(f"Potential formula injection detected in {value.name}")
    ...
    def validate(self, data):
        ...
        if data.get("max_rows") and data["max_rows"] > 100000:
            if not data.get("async_processing"):
                logger.info(f"Large dataset ({data['max_rows']} rows) - " "async processing recommended")
...
class ProfilingResultSerializer(serializers.Serializer):
    ...
    status = serializers.ChoiceField(choices=["completed", "processing", "error", "cached"])

    # Core results
    profile = DatasetProfileSerializer(required=False)
    recommendations = serializers.ListField(child=TestRecommendationSerializer(), required=False)

    # Metadata
    metadata = serializers.DictField(help_text="Processing metadata including timing and cache status")
    ...
    processing_time_seconds = ScientificFloatField(precision=2, required=False)
...
class BatchProfilingSerializer(serializers.Serializer):
    ...
    files = serializers.ListField(
        child=serializers.FileField(), min_length=1, max_length=10, help_text="List of files to profile (max 10)"
    )

    # Common options for all files
    common_target_variable = serializers.CharField(
        max_length=100, required=False, help_text="Common dependent variable across datasets"
    )

    compare_datasets = serializers.BooleanField(
        default=False, help_text="Generate comparative analysis across datasets"
    )

    # Processing options
    parallel_processing = serializers.BooleanField(default=True, help_text="Process files in parallel")

    max_workers = serializers.IntegerField(min_value=1, max_value=8, default=4, help_text="Maximum parallel workers")
...
class ExportReportSerializer(serializers.Serializer):
    ...
    profile_id = serializers.UUIDField(help_text="Profile ID to export")

    format = serializers.ChoiceField(
        choices=["pdf", "html", "docx", "latex", "markdown"], default="pdf", help_text="Export format"
    )

    # Content options
    include_profile = serializers.BooleanField(default=True, help_text="Include dataset profile")

    include_recommendations = serializers.BooleanField(default=True, help_text="Include test recommendations")

    include_visualizations = serializers.BooleanField(default=True, help_text="Include statistical visualizations")

    include_assumptions = serializers.BooleanField(default=True, help_text="Include assumption checks")

    include_interpretations = serializers.BooleanField(default=False, help_text="Include detailed interpretations")

    # Formatting options
    apa_formatting = serializers.BooleanField(default=True, help_text="Use APA 7th edition formatting")

    confidence_level = ScientificFloatField(
        precision=2, min_value=0.8, max_value=0.99, default=0.95, help_text="Confidence level for intervals"
    )

    # Metadata
    title = serializers.CharField(max_length=200, default="Statistical Analysis Report", help_text="Report title")

    author = serializers.CharField(max_length=100, required=False, help_text="Report author")

    organization = serializers.CharField(max_length=100, required=False, help_text="Organization name")

    include_citations = serializers.BooleanField(default=True, help_text="Include statistical references")
...
def validate_sample_size_requirements(test_type: str, n: int, k: Optional[int] = None) -> None:
    """
    ...
    if n < min_n:
        raise ValidationError(f"Sample size {n} insufficient for {test_type}. " f"Minimum required: {min_n}")
