...
class DataProfiler:
    ...
    def profile_dataset(
        self, data: Union[pd.DataFrame, np.ndarray], target_variable: Optional[str] = None
    ) -> DatasetProfile:
        """
        ...
        continuous_vars = [
            col for col, var_profile in profile.variables.items() if var_profile.type == VariableType.CONTINUOUS
        ]

        if len(continuous_vars) >= 2:
            profile.correlation_matrix = data[continuous_vars].corr()
            profile.high_correlations = self._find_high_correlations(profile.correlation_matrix)
        ...
        profile.recommended_tests = self._recommend_statistical_tests(profile, target_variable)

        # Check test assumptions
        profile.test_assumptions_met = self._check_test_assumptions(data, profile, target_variable)
    ...
    def _fit_distribution(self, series: pd.Series) -> Tuple[DistributionType, Dict[str, float]]:
        ...
        for dist_name, dist_func in distributions.items():
            try:
                ...
                ks_statistic, ks_p_value = stats.kstest(series, lambda x: dist_func.cdf(x, *params))
    ...
    def _recommend_statistical_tests(self, profile: DatasetProfile, target_variable: Optional[str] = None) -> List[str]:
        """
        ...
        categorical_vars = [
            v for v in profile.variables.values() if v.type in [VariableType.NOMINAL, VariableType.BINARY]
        ]
        ...
        # Check for normality
        [v for v in continuous_vars if v.normality_p_value and v.normality_p_value > 0.05]
        non_normal_vars = [v for v in continuous_vars if v.normality_p_value and v.normality_p_value <= 0.05]
    ...
    def _check_test_assumptions(
        self, data: pd.DataFrame, profile: DatasetProfile, target_variable: Optional[str] = None
    ) -> Dict[str, bool]:
        """
    ...
    def generate_report(self, profile: DatasetProfile) -> str:
        ...
        report.append(
            f"Missing Values: {profile.total_missing:,} ({profile.total_missing/profile.n_rows/profile.n_columns*100:.1f}%)"
        )
