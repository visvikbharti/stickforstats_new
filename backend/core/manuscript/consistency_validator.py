...
class ConsistencyValidator:
    ...
    def validate_claim(self, claim) -> ConsistencyResult:
        ...
        reported_stat: Optional[float] = getattr(claim, "statistic", None) or getattr(claim, "statistic_value", None)
        ...
        if claim_type not in self.SUPPORTED_TYPES:
            return self._skip_result(
                claim,
                note=(
                    f"Unsupported claim type '{claim_type}'; " f"supported: {', '.join(sorted(self.SUPPORTED_TYPES))}"
                ),
            )
        ...
        try:
            computed_p = self._recompute(claim_type, reported_stat, df, n, note_parts)
        ...
        if computed_p is None:
            ...
            return self._skip_result(
                claim,
                note="; ".join(note_parts) if note_parts else "Cannot recompute: insufficient information",
            )

        # ----- Compare p-values -----
        is_consistent, discrepancy, severity = self._compare_p_values(reported_p, p_comparison, computed_p)

        # Decision consistency
        reported_decision = "significant" if reported_p < self.alpha else "non_significant"
        # For inequality comparisons, infer decision from the bound
        if p_comparison == "less_than":
            reported_decision = "significant" if reported_p <= self.alpha else "non_significant"
        elif p_comparison == "greater_than":
            reported_decision = "non_significant" if reported_p >= self.alpha else "significant"

        computed_decision = "significant" if computed_p < self.alpha else "non_significant"
        ...
        if not is_decision_consistent:
            severity = self._determine_severity(discrepancy, decision_changed=True)

        # Build human-readable note
        if is_consistent:
            note_parts.append(f"Consistent (computed p = {computed_p:.6f})")
        else:
            note_parts.append(
    ...
    def _recompute_t_test(
        ...
        if resolved_df is None:
            return None
        if resolved_df <= 0:
            notes.append(f"Cannot recompute: degrees of freedom ({resolved_df}) must " f"be positive")
            return None
    ...
    def _recompute_f_test(
        ...
        """
        if statistic < 0:
            notes.append(f"F statistic ({statistic}) is negative; " f"F statistics must be non-negative")
            return None
        ...
        if df1 is None or df2 is None:
            return None
        if df1 <= 0 or df2 <= 0:
            notes.append(f"Cannot recompute: degrees of freedom ({df1}, {df2}) must " f"both be positive")
            return None
    ...
    def _recompute_chi_square(
        ...
        if statistic < 0:
            notes.append(
                f"Chi-square statistic ({statistic}) is negative; " f"chi-square statistics must be non-negative"
            )
            return None
    ...
    def _recompute_correlation(
        ...
        """
        if abs(r) > 1:
            notes.append(f"Correlation r ({r}) is outside [-1, 1]; cannot recompute")
            return None
        ...
        if n is not None and n > 2:
            ...
            if effective_df is None and n is not None:
                notes.append(f"Cannot recompute: sample size n ({n}) must be > 2 " f"for correlation")
                return None
            elif effective_df is None:
                notes.append("Cannot recompute: neither n nor df reported for " "correlation")
                return None

        if effective_df <= 0:
            notes.append(f"Cannot recompute: effective df ({effective_df}) must be " f"positive")
            return None
    ...
    def _compare_p_values(
        ...
        if reported_comparison == "less_than":
            ...
            severity = self._determine_severity(
                discrepancy,
                decision_changed=not self._same_decision(reported_p, computed_p, reported_comparison),
            )
    ...
    def _resolve_single_df(
        ...
        """
        if df is None:
            notes.append("Cannot recompute: degrees of freedom not reported")
            return None

        if isinstance(df, (int, float)):
            if df == 0:
                notes.append("Cannot recompute: degrees of freedom is zero")
                return None
            return float(df)
        ...
        notes.append(f"Cannot recompute: unrecognized df format ({type(df).__name__})")
        return None
    ...
    def _resolve_two_df(
        ...
        if isinstance(df, (list, tuple)) and len(df) >= 2:
            ...
            if df1 == 0 or df2 == 0:
                notes.append("Cannot recompute: degrees of freedom contains zero")
                return None, None
            return df1, df2

        notes.append("Cannot recompute F-test: need two df values (df1, df2), " f"got {df!r}")
        return None, None
