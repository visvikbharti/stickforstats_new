...
class MultiplicityCorrector:
    ...
    def correct(
        ...
        **kwargs,
        ...
        if result.n_tests > 20:
            result.warnings.append(f"Large number of tests ({result.n_tests}) - consider FDR control instead of FWER")

        if result.n_tests < 5 and method in [CorrectionMethod.FDR_BH, CorrectionMethod.FDR_BY]:
            result.warnings.append("Small number of tests - FWER control may be more appropriate than FDR")
    ...
    def _two_stage_fdr(
        self, p_values: np.ndarray, alpha: float, q1: float = 0.05, method: str = "bky"
    ) -> CorrectionResult:
        """
        ...
        result.warnings.append(f"Two-stage FDR: estimated {m0_hat:.1f} true nulls out of {n} tests")

        return result

    def _qvalue(
        self, p_values: np.ndarray, lambda_range: Optional[np.ndarray] = None, pi0_method: str = "smoother"
    ) -> CorrectionResult:
        """
    ...
    def sequential_correction(
        ...
        **kwargs,
        ...
        alpha_spent = self._calculate_alpha_spending(information_fractions, alpha, spending_function, **kwargs)
    ...
    def _calculate_alpha_spending(
        self, information_fractions: List[float], alpha: float, spending_function: AlphaSpendingFunction, **kwargs
    ) -> np.ndarray:
        """
    ...
    def optimal_discovery(
        self, p_values: np.ndarray, effect_sizes: Optional[np.ndarray] = None, sample_sizes: Optional[np.ndarray] = None
    ) -> CorrectionResult:
        """
...
class GroupwiseCorrector:
    ...
    def hierarchical_fdr(
        self, p_values: Dict[str, np.ndarray], hierarchy: Dict[str, List[str]], alpha: float = 0.05
        ...
        group_result = self.corrector.correct(group_p_values, method=CorrectionMethod.FDR_BH, alpha=alpha)

        # Level 2: Test within significant groups
        for i, group in enumerate(group_names):
            if group_result.rejected[i]:
                # Test within group
                within_result = self.corrector.correct(p_values[group], method=CorrectionMethod.FDR_BH, alpha=alpha)
    ...
    def tree_based_fdr(
        self, p_values: np.ndarray, tree_structure: Dict[int, List[int]], alpha: float = 0.05
    ) -> CorrectionResult:
        """
...
def recommend_correction_method(
    n_tests: int, study_type: str = "exploratory", dependence: str = "unknown"
) -> CorrectionMethod:
    """
...
def generate_correction_statement(result: CorrectionResult) -> str:
    ...
    statements = {
        ...
        CorrectionMethod.FDR_BH: (
            ...
            + (f" with an estimated FDR of {result.estimated_fdr:.3f}." if result.estimated_fdr is not None else ".")
        ),
