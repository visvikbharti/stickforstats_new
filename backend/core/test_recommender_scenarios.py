...
class ScenarioLibrary:
    ...
    def _generate_normal_two_groups(self, n1=30, n2=30, mean1=0, mean2=0, sd=1, sd1=None, sd2=None):
        ...
        df = pd.DataFrame({"group": ["A"] * n1 + ["B"] * n2, "value": np.concatenate([group1, group2])})
        return df
    ...
    def _generate_paired_data(self, n=25, correlation=0.7):
        ...
        df = pd.DataFrame({"before": data[:, 0], "after": data[:, 1]})
        return df
    ...
    def _generate_anova_data(self, groups=4, n_per_group=20):
        ...
        df = pd.DataFrame({"group": group_labels, "value": data})
        return df
    ...
    def _generate_logistic_data(self, n=200):
        ...
        df = pd.DataFrame({"x1": x1, "x2": x2, "outcome": outcome})
        return df
    ...
    def _generate_heavy_tailed(self, n=50):
        ...
        df = pd.DataFrame({"group": group, "value": values[:n]})
        return df
    ...
    def _generate_zero_inflated(self, n=150):
        ...
        df = pd.DataFrame({"predictor": predictor, "counts": counts})
        return df
    ...
    def _generate_repeated_measures(self, n_subjects=30, n_times=4):
        ...
        df = pd.DataFrame({"subject": subjects, "time": times, "measurement": measurements})
        return df
    ...
    def _generate_survival_data(self, n=100):
        ...
        df = pd.DataFrame({"time": times, "event": events, "group": group})
        return df
    ...
    def _generate_hierarchical_data(self, n_groups=20, n_per_group=15):
        ...
        df = pd.DataFrame({"school": schools, "student": students, "predictor": predictors, "outcome": outcomes})
        return df
    ...
    def validate_recommendation(
        self, scenario: TestScenario, recommended_test: str, assumption_violations: List[str]
    ) -> Dict[str, Any]:
        """
