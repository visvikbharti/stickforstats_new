...
from scipy.stats import shapiro, normaltest, anderson, levene, bartlett, fligner, jarque_bera, chi2_contingency
...
class AssumptionResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if isinstance(self.assumption_type, AssumptionType)
...
class AssumptionChecker:
    ...
    def check_normality(self, data: Union[np.ndarray, pd.Series], method: str = "shapiro") -> AssumptionResult:
        """
        ...
        if method in ["shapiro", "combined"] and len(data) <= 5000:
            try:
                ...
                results["shapiro_wilk"] = {"statistic": stat, "p_value": p_value, "is_normal": p_value > self.alpha}
            except:
        ...
        if method in ["jarque_bera", "combined"] and len(data) >= 8:
            try:
                ...
                results["jarque_bera"] = {"statistic": stat, "p_value": p_value, "is_normal": p_value > self.alpha}
            except:
        ...
        return AssumptionResult(
            ...
            details={"results": results, "skewness": skewness, "kurtosis": kurtosis_val, "sample_size": len(data)},
            suggestions=suggestions,
            warning_message=None if is_normal else "Data significantly deviates from normal distribution",
        )

    def check_homoscedasticity(self, *groups, method: str = "levene") -> AssumptionResult:
        """
        ...
        return AssumptionResult(
            ...
            is_met=is_equal_var,
            confidence=confidence,
            details={"results": results, "variances": variances, "variance_ratio": var_ratio, "n_groups": len(groups)},
            suggestions=suggestions,
            warning_message=None
            if is_equal_var
            else f"Variance ratio = {var_ratio:.2f} (>3 suggests heteroscedasticity)",
        )

    def check_independence(
        self, residuals: Optional[np.ndarray] = None, data: Optional[np.ndarray] = None
    ) -> AssumptionResult:
        """
        ...
        if not is_independent:
            ...
            suggestions.extend([TestSuggestion.USE_ROBUST_REGRESSION, TestSuggestion.USE_BOOTSTRAP])
        else:
            warning = None
    ...
    def check_sample_size(self, data, min_size=2):
        ...
        if isinstance(min_size, str):
            test_type = min_size
            test_minimums = {"ttest": 2, "t_test": 2, "anova": 3, "correlation": 3, "regression": 5}
        ...
        if not is_adequate:
            suggestions.extend(
                [TestSuggestion.INCREASE_SAMPLE, TestSuggestion.USE_BOOTSTRAP, TestSuggestion.USE_EXACT_TEST]
            )
        ...
        return AssumptionResult(
            ...
            is_met=is_adequate,
            confidence=confidence,
            details={"sample_size": n, "minimum_required": min_size, "test_type": test_type},
            suggestions=suggestions,
            warning_message=warning,
        )

    def check_sample_size_adequacy(self, n: int, n_predictors: int = 0, test_type: str = "t_test") -> AssumptionResult:
        """
        ...
        return AssumptionResult(
            ...
            is_met=is_adequate,
            confidence=confidence,
            details={"sample_size": n, "minimum_required": min_required, "test_type": test_type, "power": power},
            suggestions=suggestions,
            warning_message=warning,
        )

    def check_outliers(self, data: np.ndarray, method: str = "iqr") -> AssumptionResult:
        """
    ...
    def check_multicollinearity(self, X: np.ndarray, feature_names: Optional[List[str]] = None) -> AssumptionResult:
        """
    ...
    def check_expected_frequencies(self, contingency_table: np.ndarray, min_expected: float = 5.0) -> AssumptionResult:
        """
        ...
        suggestions = []
        if not is_met:
            suggestions.extend([TestSuggestion.USE_EXACT_TEST, TestSuggestion.INCREASE_SAMPLE])
    ...
    def comprehensive_check(
        ...
        **kwargs,
        ...
        if not all_met:
            if test_type == "independent_t_test":
                ...
                normality_violated = any(
                    not r.is_met for r in assumptions_checked if r.assumption_type == AssumptionType.NORMALITY
                )
                homoscedasticity_violated = any(
                    not r.is_met for r in assumptions_checked if r.assumption_type == AssumptionType.HOMOSCEDASTICITY
                )
        ...
        for result in assumptions_checked:
            ...
            report_lines.append(f"{status} | {result.assumption_type.value}: {result.test_name}")
