...
class RobustEstimate:
    ...
    def __str__(self) -> str:
        ...
        lines = [f"{self.method.value}: {self.value:.4f}"]
...
class RobustEstimator:
    ...
    def trimmed_mean(
        self, data: Union[np.ndarray, List[float]], trim_proportion: float = 0.2, ci_method: str = "bootstrap"
    ) -> RobustEstimate:
        """
        ...
        if ci_method == "yuen":
            ...
            ci_lower, ci_upper = self._bootstrap_ci(
                data, lambda x: self.trimmed_mean(x, trim_proportion).value, self.confidence_level
            )
        ...
        return RobustEstimate(
            ...
            metadata={"n_original": n, "n_trimmed": h, "g": g, "trim_proportion": trim_proportion},
        )

    def winsorized_mean(self, data: Union[np.ndarray, List[float]], trim_proportion: float = 0.2) -> RobustEstimate:
        """
        ...
        return RobustEstimate(
            ...
            ci_level=self.confidence_level,
            breakdown_point=trim_proportion,
            metadata={"n": n, "g": g, "trim_proportion": trim_proportion},
        )

    # ============== M-ESTIMATORS ==============

    def huber_m_estimator(
        self, data: Union[np.ndarray, List[float]], c: float = 1.345, max_iter: int = 100, tol: float = 1e-6
    ) -> RobustEstimate:
        """
        ...
        if scale == 0:
            ...
            return RobustEstimate(value=mu, method=RobustMethod.HUBER_M, converged=True, n_iterations=0)
        ...
        return RobustEstimate(
            ...
            outliers=outliers,
            weights=weights,
            metadata={"c": c, "scale": scale, "n_outliers": len(outliers)},
        )

    def tukey_biweight(
        self, data: Union[np.ndarray, List[float]], c: float = 4.685, max_iter: int = 100, tol: float = 1e-6
    ) -> RobustEstimate:
        """
        ...
        if scale == 0:
            return RobustEstimate(value=mu, method=RobustMethod.TUKEY_BIWEIGHT, converged=True, n_iterations=0)
    ...
    def median(self, data: Union[np.ndarray, List[float]]) -> RobustEstimate:
        """
    ...
    def mad(self, data: Union[np.ndarray, List[float]], constant: float = 1.4826) -> RobustEstimate:
        """
        ...
        ci_lower, ci_upper = self._bootstrap_ci(data, lambda x: self.mad(x, constant).value, self.confidence_level)
        ...
        return RobustEstimate(
            ...
            breakdown_point=0.5,
            metadata={"n": n, "raw_mad": mad_value, "constant": constant},
        )

    def hodges_lehmann(self, data: Union[np.ndarray, List[float]]) -> RobustEstimate:
        """
        ...
        ci_lower, ci_upper = self._bootstrap_ci(data, lambda x: self.hodges_lehmann(x).value, self.confidence_level)
        ...
        return RobustEstimate(
            ...
            metadata={"n": n, "n_walsh_averages": len(walsh_averages)},
        )

    # ============== ROBUST CORRELATION ==============

    def percentage_bend_correlation(
        self, x: Union[np.ndarray, List[float]], y: Union[np.ndarray, List[float]], beta: float = 0.2
    ) -> RobustEstimate:
        """
        ...
        # Weight and center x
        psi_x = np.where(np.abs(x - np.median(x)) < omega_x, x - np.median(x), omega_x * np.sign(x - np.median(x)))
        ...
        # Weight and center y
        psi_y = np.where(np.abs(y - np.median(y)) < omega_y, y - np.median(y), omega_y * np.sign(y - np.median(y)))
        ...
        ci_lower, ci_upper = self._bootstrap_ci_paired(x, y, pbcor_stat, self.confidence_level)
        ...
        return RobustEstimate(
            ...
            metadata={"n": n, "beta": beta, "omega_x": omega_x, "omega_y": omega_y},
        )

    # ============== OUTLIER DETECTION ==============

    def detect_outliers_mad(self, data: Union[np.ndarray, List[float]], threshold: float = 3.0) -> np.ndarray:
        """
    ...
    def detect_outliers_iqr(self, data: Union[np.ndarray, List[float]], k: float = 1.5) -> np.ndarray:
        """
    ...
    def _bootstrap_ci(
        self, data: np.ndarray, statistic: Callable, confidence: float, n_boot: int = 10000
    ) -> Tuple[float, float]:
        """
    ...
    def _bootstrap_ci_paired(
        self, x: np.ndarray, y: np.ndarray, statistic: Callable, confidence: float, n_boot: int = 10000
    ) -> Tuple[float, float]:
        """
...
class RobustComparison:
    ...
    @staticmethod
    def yuen_t_test(group1: np.ndarray, group2: np.ndarray, trim: float = 0.2) -> Dict[str, Any]:
        """
        ...
        if se_diff and se_diff > 0:
            ...
            df = (tm1.se**2 + tm2.se**2) ** 2 / (
                tm1.se**4 / (tm1.metadata["n_trimmed"] - 1) + tm2.se**4 / (tm2.metadata["n_trimmed"] - 1)
            )
