...
class TimeSeriesService:
    ...
    def test_stationarity(self, series: Union[pd.Series, np.ndarray], test_type: str = "both") -> Dict[str, Any]:
        """
    ...
    def make_stationary(
        self, series: Union[pd.Series, np.ndarray], method: str = "difference", difference_order: int = 1
    ...
    def seasonal_decompose(
        self, series: Union[pd.Series, np.ndarray], period: Optional[int] = None, method: str = "additive"
    ) -> Dict[str, Any]:
        """
    ...
    def acf_pacf_analysis(self, series: Union[pd.Series, np.ndarray], nlags: int = 40) -> Dict[str, Any]:
        """
    ...
    def _suggest_arima_order(self, acf_vals: np.ndarray, pacf_vals: np.ndarray, ci_width: float) -> Dict[str, int]:
    ...
    def fit_arima(
        ...
        **kwargs,
        ...
        if auto:
            ...
            # Fit specified model
            if seasonal_order:
                self.model = SARIMAX(series, order=order, seasonal_order=seasonal_order, **kwargs)
            else:
        ...
        # Compute metrics
        metrics = self._compute_metrics(series.values[len(series) - len(fitted_values) :], fitted_values, fitted_model)
    ...
    def fit_exponential_smoothing(
        ...
        **kwargs,
        ...
        self.model = ExponentialSmoothing(
            series, trend=trend, seasonal=seasonal, seasonal_periods=seasonal_periods, **kwargs
        )
        ...
        sim_forecasts = []
        for _ in range(1000):
            sim = fitted_model.simulate(nsimulations=12, anchor="end", random_state=np.random.RandomState())
    ...
    def fit_var(self, data: pd.DataFrame, maxlags: Optional[int] = None, ic: str = "aic") -> Dict[str, Any]:
        """
            ...
            for col2 in data.columns:
                if col1 != col2:
                    gc_test = grangercausalitytests(
                        stationary_data[[col1, col2]].dropna(), maxlag=optimal_lag, verbose=False
                    )
                    # Extract p-values for each lag
                    p_values = [gc_test[lag][0]["ssr_ftest"][1] for lag in range(1, optimal_lag + 1)]
                    granger_results[col1][col2] = {"min_p_value": min(p_values), "causes": min(p_values) < 0.05}
        ...
        return {
            ...
            "impulse_response": {"irf": irf.irfs, "lower_ci": irf.ci()[0], "upper_ci": irf.ci()[1]},
            "forecasts": forecasts,
            "metrics": metrics,
    ...
    def cross_validate_time_series(
        ...
        **model_kwargs,
        ...
        for fold, (train_idx, test_idx) in enumerate(tscv.split(series)):
            ...
            cv_results.append({"fold": fold, "mse": mse, "rmse": np.sqrt(mse), "mae": mae, "mape": mape})
    ...
    def _compute_metrics(self, actual: np.ndarray, predicted: np.ndarray, model: Any) -> Dict[str, float]:
        ...
        metrics = {"mse": mse, "rmse": np.sqrt(mse), "mae": mae, "mape": mape}
    ...
    def _perform_diagnostics(self, residuals: Union[pd.Series, np.ndarray]) -> Dict[str, Any]:
        ...
        diagnostics["jarque_bera"] = {"statistic": jb_stat, "p_value": jb_pvalue, "is_normal": jb_pvalue > 0.05}
        ...
        diagnostics["durbin_watson"] = {"statistic": dw_stat, "no_autocorrelation": 1.5 < dw_stat < 2.5}
    ...
    def forecast(self, steps: int = 12, confidence_level: float = 0.95) -> Dict[str, Any]:
        ...
        if self.model_type in ["ARIMA", "SARIMA"]:
            ...
            sim_forecasts = []
            for _ in range(1000):
                sim = self.model.simulate(nsimulations=steps, anchor="end", random_state=np.random.RandomState())
                sim_forecasts.append(sim)
