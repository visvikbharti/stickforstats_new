...
class LinearRegressionService:
    ...
    def fit(
        ...
        confidence_intervals = np.column_stack(
            [beta - t_critical * standard_errors, beta + t_critical * standard_errors]
        )
        ...
        diagnostics = self._calculate_diagnostics(
            X, y, beta, fitted_values, residuals, XtX_inv, sigma_squared, df_resid
        )
        ...
        anova_table = self._create_anova_table(
            ss_model, ss_resid, ss_total, df_model, df_resid, df_total, ms_model, ms_resid, f_statistic, f_p_value
        )
    ...
    def test_assumptions(self) -> Dict[str, Any]:
        ...
        self.results.fitted_values

        # 1. Normality of residuals
        # Shapiro-Wilk test
        shapiro_stat, shapiro_p = shapiro(residuals)
        results["normality_shapiro"] = {"statistic": shapiro_stat, "p_value": shapiro_p, "normal": shapiro_p > 0.05}

        # Jarque-Bera test
        jb_stat, jb_p = jarque_bera(residuals)
        results["normality_jarque_bera"] = {"statistic": jb_stat, "p_value": jb_p, "normal": jb_p > 0.05}
        ...
        if self.results.vif is not None:
            results["multicollinearity_vif"] = {
                ...
                "high_vif_features": [
                    name for name, vif in zip(self.results.feature_names, self.results.vif) if vif > 10
                ],
            }
    ...
    def _breusch_pagan_test(self, residuals: np.ndarray, X: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"statistic": np.nan, "p_value": np.nan, "df": X.shape[1] - 1, "homoscedastic": None}
    ...
    def _white_test(self, residuals: np.ndarray, X: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"statistic": np.nan, "p_value": np.nan, "df": X_white.shape[1] - 1, "homoscedastic": None}
    ...
    def _rainbow_test(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"statistic": np.nan, "p_value": np.nan, "linear": None}

    def predict(
        self, X_new: Union[np.ndarray, pd.DataFrame], confidence_level: float = 0.95, prediction_interval: bool = False
    ) -> Dict[str, np.ndarray]:
        """
    ...
    def summary(self) -> str:
        ...
        # Header
        lines.append(f"{'Variable':<20} {'Coefficient':>12} {'Std Error':>12} " f"{'t-stat':>8} {'P>|t|':>10}")
        lines.append("-" * 70)

        # Intercept
        lines.append(
            f"{'Intercept':<20} {r.intercept:>12.4f} "
        ...
        for i, name in enumerate(r.feature_names):
            lines.append(
                f"{name:<20} {r.coefficients[i]:>12.4f} "
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        r = self.results
        return {
            "coefficients": {name: float(coef) for name, coef in zip(r.feature_names, r.coefficients)},
            ...
            "diagnostics": {
                "vif": {name: float(vif) for name, vif in zip(r.feature_names, r.vif)} if r.vif is not None else None,
