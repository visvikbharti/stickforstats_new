...
class GLMService:
    ...
    def poisson_regression(
        ...
        null_log_likelihood = np.sum(
            y_arr * np.log(mu_null) - mu_null - np.log(np.array([np.math.factorial(int(yi)) for yi in y_arr]))
        )

        # Deviance
        deviance = 2 * np.sum(y_arr * np.log(np.maximum(y_arr, 1) / fitted_values) - (y_arr - fitted_values))
        ...
        confidence_intervals = np.column_stack(
            [beta - z_critical * standard_errors, beta + z_critical * standard_errors]
        )
        ...
        if overdispersion_test["overdispersed"]:
            warnings.warn(
                f"Overdispersion detected (dispersion={overdispersion_test['dispersion']:.2f}). "
    ...
    def model_diagnostics(self, results: Optional[GLMResults] = None) -> Dict[str, Any]:
        ...
        diagnostics = {"goodness_of_fit": {}, "residual_analysis": {}, "influence_measures": {}}
        ...
        diagnostics["residual_analysis"] = {
            "residuals_vs_fitted": {"x": results.fitted_values.tolist(), "y": results.residuals.tolist()},
        ...
        if self.X is not None:
            ...
            diagnostics["influence_measures"] = {
                ...
                "high_leverage_points": np.where(leverage > 2 * results.n_parameters / results.n_observations)[
                    0
                ].tolist(),
            }
    ...
    def _hosmer_lemeshow_test(self, fitted_probs, y_true, n_bins=10):
        ...
        return {"chi2_statistic": chi2_stat, "df": df, "p_value": p_value, "good_fit": p_value > 0.05}
    ...
    def _qq_plot_residuals(self, residuals):
        ...
        return {"theoretical": qq[0][0].tolist(), "sample": qq[0][1].tolist()}
    ...
    def summary(self, results: Optional[GLMResults] = None) -> str:
        ...
        for i, name in enumerate(results.feature_names):
            if results.model_type == "Logistic Regression" and results.odds_ratios is not None:
                lines.append(
                    f"{name:<20} {results.coefficients[i]:>10.4f} "
