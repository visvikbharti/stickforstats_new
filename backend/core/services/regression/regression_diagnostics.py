...
class RegressionDiagnostics:
    ...
    def residual_analysis(
        residuals: np.ndarray, fitted: np.ndarray, predictors: Optional[np.ndarray] = None
    ) -> Dict[str, Any]:
        """
        ...
        if predictors is not None:
            results["partial_residuals"] = RegressionDiagnostics._partial_residual_plots(residuals, predictors)

        # Test for patterns in residuals
        results["tests"] = {
            "runs_test": RegressionDiagnostics._runs_test(residuals),
            "heteroscedasticity": RegressionDiagnostics._goldfeld_quandt_test(residuals, fitted),
        }

        return results

    @staticmethod
    def influence_measures(X: np.ndarray, y: np.ndarray, residuals: np.ndarray, leverage: np.ndarray) -> Dict[str, Any]:
        """
        ...
        covratio = np.zeros(n)
        for i in range(n):
            covratio[i] = ((n - p - 1 + standardized_residuals[i] ** 2) / (n - p)) ** p * (1 / (1 - leverage[i]))
    ...
    def outlier_detection(
        residuals: np.ndarray, leverage: np.ndarray, cooks_distance: np.ndarray, n_predictors: int
    ) -> Dict[str, Any]:
        """
        ...
        return {"outliers": outliers, "summary": summary}
    ...
    @staticmethod
    def _calculate_lowess(x: np.ndarray, y: np.ndarray, frac: float = 0.3) -> Dict[str, List[float]]:
    ...
    @staticmethod
    def _partial_residual_plots(residuals: np.ndarray, X: np.ndarray) -> List[Dict[str, Any]]:
        ...
        for j in range(X.shape[1]):
            ...
            plots.append({"predictor_index": j, "x": X[:, j].tolist(), "y": partial_res.tolist()})
    ...
    @staticmethod
    def _goldfeld_quandt_test(residuals: np.ndarray, fitted: np.ndarray) -> Dict[str, Any]:
        """
