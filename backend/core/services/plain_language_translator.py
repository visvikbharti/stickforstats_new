...
class PlainLanguageTranslator:
    ...
    def _translate_paired_t(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        if mode == OutputMode.PLAIN_ENGLISH:
            if sig:
                summary = f"There IS a significant change between the two measurements. The change is {es['label'].lower()}."
            else:
    ...
    def _translate_one_sample_t(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        return {
            ...
            'effect_size_interpretation': es, 'is_significant': sig,
    ...
    def _translate_wilcoxon(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        return {
            'summary': summary, 'details': '', 'is_significant': sig,
    ...
    def _translate_anova(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        return {
            'summary': summary, 'details': details, 'is_significant': sig,
            'effect_size_interpretation': {'category': es_label, 'label': es_label.capitalize()},
            'recommendation': 'Run post-hoc tests to identify specific group differences.' if sig else 'No further group comparisons needed.',
        }
    ...
    def _translate_kruskal_wallis(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        sig = p < alpha
        if mode == OutputMode.PLAIN_ENGLISH:
            summary = f"At least one group IS different (non-parametric test)." if sig else "The groups are NOT significantly different."
        else:
    ...
    def _translate_pearson(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        if mode == OutputMode.PLAIN_ENGLISH:
            ...
            summary = (
                f"Pearson correlation: r = {corr:.3f}, p = {p:.4f} — "
    ...
    def _translate_chi_square(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        if mode == OutputMode.PLAIN_ENGLISH:
            ...
            summary = (
                f"Chi-square test: chi2 = {r.get('statistic', 0):.3f}, "
                f"p = {p:.4f}, Cramer's V = {v:.3f}."
            )
            details = ""
        else:
            df = r.get('degrees_of_freedom', '')
    ...
    def _translate_regression(self, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        if mode == OutputMode.PLAIN_ENGLISH:
            if sig:
                ...
                details = (
                    f"For every 1-unit increase in the predictor, the outcome changes by {slope:.3f} units."
                )
            else:
            ...
            summary = (
                f"Linear regression: b = {slope:.4f}, p = {p:.4f}, R² = {r_sq:.3f}."
            )
    ...
    def _generic_translation(self, test_type: str, r: Dict, mode: str, alpha: float) -> Dict[str, Any]:
        ...
        if mode == OutputMode.PLAIN_ENGLISH:
            summary = f"The test {'found significant results' if sig else 'did not find significant results'} (p = {p:.4f})."
        else:
