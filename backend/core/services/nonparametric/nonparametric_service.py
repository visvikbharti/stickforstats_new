...
class NonParametricService:
    ...
    def mann_whitney_u_test(
        ...
        if n1 <= 20 and n2 <= 20:
            ...
            if alternative == "two-sided":
                ...
                min(u_statistic, n1 * n2 - u_statistic)
        ...
        location_estimate, ci_lower, ci_upper = self._hodges_lehmann_estimator(x, y, confidence_level)

        # Interpretation
        interpretation = self._interpret_mann_whitney(p_value, effect_size, alternative, n1, n2)
    ...
    def wilcoxon_signed_rank_test(
        ...
        # Perform test
        if n_pairs > 0:
            result = wilcoxon(differences, alternative=alternative, zero_method=zero_method)
        ...
        ci_lower, ci_upper = self._wilcoxon_confidence_interval(differences, confidence_level)

        # Interpretation
        interpretation = self._interpret_wilcoxon(p_value, effect_size, alternative, n_pairs, zeros_present)
    ...
    def kruskal_wallis_test(
        self, *groups: Union[np.ndarray, pd.Series], compute_post_hoc: bool = True, post_hoc_method: str = "dunn"
    ) -> Dict[str, Any]:
        """
            ...
            group_stats.append(
                {
                    ...
                    "mean_rank": float(
                        np.mean(
                            rankdata(np.concatenate(clean_groups))[sum(group_sizes[:i]) : sum(group_sizes[: i + 1])]
                        )
                    ),
    ...
    def friedman_test(self, data: Union[np.ndarray, pd.DataFrame], compute_post_hoc: bool = True) -> Dict[str, Any]:
        """
    ...
    def runs_test(self, data: Union[np.ndarray, pd.Series], cutoff: Optional[float] = None) -> Dict[str, Any]:
        """
    ...
    def spearman_correlation(
        self, x: Union[np.ndarray, pd.Series], y: Union[np.ndarray, pd.Series], confidence_level: float = 0.95
    ) -> Dict[str, Any]:
        """
    ...
    def _hodges_lehmann_estimator(
        self, x: np.ndarray, y: np.ndarray, confidence_level: float
    ...
    def _wilcoxon_confidence_interval(self, differences: np.ndarray, confidence_level: float) -> Tuple[float, float]:
    ...
    def _sign_test_confidence_interval(self, data: np.ndarray, confidence_level: float) -> Tuple[float, float]:
    ...
    def _dunn_test(self, groups: List[np.ndarray], group_sizes: List[int]) -> List[Dict[str, Any]]:
    ...
    def _conover_test(self, groups: List[np.ndarray], group_sizes: List[int]) -> List[Dict[str, Any]]:
    ...
    def _nemenyi_test(self, data: np.ndarray, condition_names: List[str]) -> List[Dict[str, Any]]:
    ...
    def _interpret_mann_whitney(
        self, p_value: float, effect_size: Optional[float], alternative: str, n1: int, n2: int
    ) -> str:
        ...
        return f"There is {sig} between the two groups (p={p_value:.4f}){effect}. " f"Sample sizes: n1={n1}, n2={n2}."

    def _interpret_wilcoxon(
        self, p_value: float, effect_size: Optional[float], alternative: str, n: int, zeros: bool
    ) -> str:
        ...
        return f"The difference is {sig} (p={p_value:.4f}). " f"Sample size: n={n}{zeros_note}."

    def _interpret_kruskal_wallis(self, p_value: float, epsilon_squared: float, n_groups: int) -> str:
        """Interpret Kruskal-Wallis test results."""
        if p_value < 0.05:
            return (
                f"There is a statistically significant difference among the {n_groups} groups "
                f"(p={p_value:.4f}). Effect size (ε²={epsilon_squared:.3f}) suggests "
                f"{self._interpret_epsilon_squared(epsilon_squared)}."
            )
        else:
            return f"No statistically significant difference found among the {n_groups} groups " f"(p={p_value:.4f})."

    def _interpret_friedman(self, p_value: float, w: float, n_conditions: int) -> str:
        """Interpret Friedman test results."""
        if p_value < 0.05:
            return (
                f"There is a statistically significant difference among the {n_conditions} conditions "
                f"(p={p_value:.4f}). Kendall's W={w:.3f} indicates "
                f"{self._interpret_kendalls_w(w)}."
            )
        else:
            return (
                f"No statistically significant difference found among the {n_conditions} conditions "
                f"(p={p_value:.4f})."
            )
    ...
    def _interpret_sign_test(self, p_value: float, proportion: float, n: int) -> str:
        ...
        if p_value < 0.05:
            return (
                f"The median difference is statistically significant (p={p_value:.4f}). "
                f"Proportion positive: {proportion:.2f}, n={n}."
            )
        else:
            return (
                f"The median difference is not statistically significant (p={p_value:.4f}). "
                f"Proportion positive: {proportion:.2f}, n={n}."
            )
    ...
    def _interpret_runs_test(self, p_value: float, runs: int, expected: float) -> str:
        ...
        if p_value < 0.05:
            ...
            return (
                f"The sequence is not random (p={p_value:.4f}) with {pattern}. "
                f"Observed runs: {runs}, Expected: {expected:.1f}."
            )
        else:
            return (
                f"The sequence appears random (p={p_value:.4f}). " f"Observed runs: {runs}, Expected: {expected:.1f}."
            )
    ...
    def _interpret_correlation(self, correlation: float, p_value: float) -> str:
        ...
        if p_value < 0.05:
            ...
            return f"The correlation is not statistically significant " f"(r={correlation:.3f}, p={p_value:.4f})."
