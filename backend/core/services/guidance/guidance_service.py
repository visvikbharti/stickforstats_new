...
class GuidanceService:
    ...
    def get_analysis_recommendations(
        self, dataset_characteristics: Dict[str, Any], user_goals: Optional[Dict[str, Any]] = None
    ...
    def setup_standard_recommendation_rules(self):
        ...
        self.recommendation_rules["descriptive_statistics"] = {
            ...
            "requirements": {"numeric_columns": True},
            "parameter_generator": lambda chars, goals: {"columns": chars.get("numeric_columns", [])},
        ...
        self.recommendation_rules["correlation_analysis"] = {
            ...
            "requirements": {"min_numeric_columns": 2},
        ...
        self.recommendation_rules["t_test_independent"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) >= 1
                and len(chars.get("categorical_columns", [])) >= 1
                and any(chars.get("cardinality", {}).get(col, 0) == 2 for col in chars.get("categorical_columns", []))
            ),
            "confidence_calculator": lambda chars, goals: 0.7
            if any(
                chars.get("distribution_summary", {}).get(col, {}).get("is_normal", False)
                for col in chars.get("numeric_columns", [])
            )
            else 0.4,
            "description": "Compare means between two independent groups",
            "requirements": {"numeric_column": True, "binary_categorical": True},
            "parameter_generator": lambda chars, goals: {
                ...
                "group_column": next(
                    (
                        ...
                        if chars.get("cardinality", {}).get(col, 0) == 2
                    ),
                    None,
                ),
                "equal_variance": True,
        ...
        self.recommendation_rules["anova"] = {
            ...
            "requirements": {"numeric_column": True, "multi_categorical": True},
            "parameter_generator": lambda chars, goals: {
                "numeric_column": chars.get("numeric_columns", [])[0],
                "group_column": next(
                    (
        ...
        self.recommendation_rules["simple_linear_regression"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) >= 2
                and chars.get("correlation_summary", {}).get("max_correlation", 0) > 0.3
            ),
            "confidence_calculator": lambda chars, goals: min(
                1.0, 0.4 + 0.6 * chars.get("correlation_summary", {}).get("max_correlation", 0)
            ),
            "description": "Predict a continuous outcome using a linear model",
            "requirements": {"min_numeric_columns": 2, "correlation": True},
            "parameter_generator": lambda chars, goals: {
                # Recommend the most correlated pair for regression
                "dependent_var": chars.get("correlation_summary", {})
                .get("correlated_pairs", [{}])[0]
                .get("col1", chars.get("numeric_columns", [])[0]),
                "independent_var": chars.get("correlation_summary", {})
                .get("correlated_pairs", [{}])[0]
                .get("col2", chars.get("numeric_columns", [])[1]),
            },
            "module_recommendations": ["basic_statistics", "confidence_intervals"],
        }
        ...
        self.recommendation_rules["chi_squared"] = {
            ...
            "requirements": {"min_categorical_columns": 2},
            "parameter_generator": lambda chars, goals: {"columns": chars.get("categorical_columns", [])[:2]},
            "module_recommendations": ["probability_distributions"],
        }

        # Time series analysis
        self.recommendation_rules["time_series_analysis"] = {
            "condition": lambda chars, goals: len(chars.get("datetime_columns", [])) > 0
            and len(chars.get("numeric_columns", [])) > 0,
            "confidence_calculator": lambda chars, goals: 0.9,
            "description": "Analyze trends over time in your data",
            "requirements": {"datetime_column": True, "numeric_column": True},
            "parameter_generator": lambda chars, goals: {
        ...
        self.recommendation_rules["cluster_analysis"] = {
            ...
            "requirements": {"min_numeric_columns": 3},
            "parameter_generator": lambda chars, goals: {
        ...
        self.recommendation_rules["pca"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) >= 4
                and len(chars.get("correlation_summary", {}).get("correlated_pairs", [])) > 0
            ),
            ...
            "requirements": {"min_numeric_columns": 4, "correlated_features": True},
            "parameter_generator": lambda chars, goals: {
                "columns": chars.get("numeric_columns", []),
        ...
        self.recommendation_rules["non_parametric_tests"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) > 0
                and chars.get("distribution_summary", {})
                and not any(
                    chars.get("distribution_summary", {}).get(col, {}).get("is_normal", True)
                    for col in chars.get("distribution_summary", {})
                )
            ),
            "confidence_calculator": lambda chars, goals: 0.8,
            "description": "Use non-parametric tests for non-normally distributed data",
            "requirements": {"numeric_column": True, "not_normal": True},
            "parameter_generator": lambda chars, goals: {
                "columns": [
                    col
                    for col in chars.get("distribution_summary", {})
        ...
        self.recommendation_rules["outlier_detection"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) > 0
                and any(
                    chars.get("numeric_stats", {}).get(col, {}).get("skew", 0) > 2
                    or chars.get("numeric_stats", {}).get(col, {}).get("kurtosis", 0) > 3
                    for col in chars.get("numeric_stats", {})
                )
            ),
            "confidence_calculator": lambda chars, goals: 0.7,
            "description": "Detect and analyze outliers in your data",
            "requirements": {"numeric_column": True},
            "parameter_generator": lambda chars, goals: {
                "columns": [
                    col
                    for col in chars.get("numeric_stats", {})
                    if chars.get("numeric_stats", {}).get(col, {}).get("skew", 0) > 2
                    or chars.get("numeric_stats", {}).get(col, {}).get("kurtosis", 0) > 3
                ]
            },
        ...
        self.recommendation_rules["statistical_process_control"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) > 0 and len(chars.get("datetime_columns", [])) > 0
            ),
            ...
            "requirements": {"numeric_column": True, "datetime_column": True},
            "parameter_generator": lambda chars, goals: {
        ...
        self.recommendation_rules["random_forest"] = {
            "condition": lambda chars, goals: (
                len(chars.get("numeric_columns", [])) >= 3 and len(chars.get("categorical_columns", [])) >= 1
            ),
            "confidence_calculator": lambda chars, goals: 0.6,
            "description": "Build a random forest model for prediction or classification",
            "requirements": {"min_numeric_columns": 3, "target_column": True},
            "parameter_generator": lambda chars, goals: {
                "features": chars.get("numeric_columns", [])[:-1],
                "target": chars.get("numeric_columns", [])[-1]
                if goals is None
                else goals.get("target_column", chars.get("numeric_columns", [])[-1]),
            },
    ...
    def save_recommendations(self, user_id, dataset_id, recommendations: List[AnalysisRecommendation]) -> bool:
        ...
        try:
            ...
            GuidanceRecommendation.objects.create(
                user_id=user_id,
                context_data={"dataset_id": str(dataset_id)},
                recommendation_type="analysis_recommendations",
                recommendation={"recommendations": rec_data, "timestamp": pd.Timestamp.now().isoformat()},
            )
