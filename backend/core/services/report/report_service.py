...
class ReportService:
    ...
    def setup_custom_styles(self):
        ...
        self.custom_styles = {
            ...
            "Normal": ParagraphStyle(
                "CustomNormal", parent=self.styles["Normal"], fontSize=10, spaceBefore=6, spaceAfter=6
            ),
            "Code": ParagraphStyle(
                "Code",
            ...
            "Caption": ParagraphStyle(
                "Caption", parent=self.styles["Normal"], fontSize=8, alignment=1, italics=True  # Center
            ),
        }
    ...
    def generate_report_from_analyses(
        ...
        report_obj = Report.objects.create(
            user=user, name=title, description=description, report_type=report_format, parameters=report_config
        )
            ...
            if analysis["visualizations"]:
                ...
                for j, viz in enumerate(analysis["visualizations"]):
                    try:
                        ...
                        story.append(
                            Paragraph(f"Error rendering visualization: {str(e)}", self.custom_styles["Normal"])
                        )
    ...
    def _add_correlation_section(self, story: List, results: Dict[str, Any]):
        ...
        if "significant_correlations" in results:
            ...
            for corr in results["significant_correlations"]:
                sig_data.append([corr["var1"], corr["var2"], f"{corr['correlation']:.4f}", f"{corr['p_value']:.4f}"])
    ...
    def _add_executive_summary(self, story: List, report_data: Dict[str, Any]):
        ...
        summary_text += "statistical analyses conducted on the dataset."
    ...
    def _get_plot_description(self, plot_type: str, metadata: Dict[str, Any]) -> str:
        ...
        if plot_type == "histogram":
            ...
            return (
                "This heatmap visualizes the relationship between variables using color intensity. "
                "Darker colors typically represent higher values, while lighter colors represent lower values."
            )
        else:
