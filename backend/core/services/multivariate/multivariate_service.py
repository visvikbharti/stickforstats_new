...
class MultivariateResults:
    ...
    def summary(self) -> str:
        ...
        summary_lines = [f"Multivariate Analysis: {self.method}", "=" * 50]
...
class MultivariateService:
    ...
    def manova(self, data: pd.DataFrame, dependent_vars: List[str], independent_vars: List[str]) -> MultivariateResults:
        """
        ...
        for dep_var in dependent_vars:
            for ind_var in independent_vars:
                ...
                univariate_results[f"{dep_var}_{ind_var}"] = {"F_statistic": f_stat, "p_value": p_val}
    ...
    def discriminant_analysis(
        self, X: np.ndarray, y: np.ndarray, method: str = "linear", priors: Optional[np.ndarray] = None
    ) -> MultivariateResults:
        """
        ...
        test_statistics = {"accuracy": accuracy, "wilks_lambda": wilks_lambda, "F_statistic": F_stat}

        p_values = {"overall_model": p_value}
    ...
    def factor_analysis(
        self, X: np.ndarray, n_factors: Optional[int] = None, rotation: str = "varimax", method: str = "minres"
    ) -> MultivariateResults:
        """
        ...
        # Test statistics
        test_statistics = {"bartlett_chi_square": chi_square_value, "kmo_overall": kmo_model, "n_factors": n_factors}

        p_values = {"bartlett_test": p_value}

        effect_sizes = {"total_variance_explained": np.sum(var_explained)}
    ...
    def cluster_analysis(
        self, X: np.ndarray, method: str = "kmeans", n_clusters: Optional[int] = None, linkage_method: str = "ward"
    ) -> MultivariateResults:
        """
        ...
        if method == "kmeans":
            ...
            centers = np.array([X_std[clusters == i].mean(axis=0) for i in range(n_clusters)])

            # Calculate within-cluster sum of squares
            inertia = sum([np.sum((X_std[clusters == i] - centers[i]) ** 2) for i in range(n_clusters)])
            ...
            centers = np.array([X_std[clusters == i].mean(axis=0) for i in range(n_clusters) if i != -1])
        ...
        if n_clusters > 1 and len(np.unique(clusters)) > 1:
            ...
            cluster_means = np.array([X_std[clusters == i].mean(axis=0) for i in np.unique(clusters) if i != -1])
        ...
        p_values = {"cluster_separation": p_value}

        effect_sizes = {"within_cluster_variance": inertia / (X_std.shape[0] * X_std.shape[1]) if inertia else np.nan}
    ...
    def _find_optimal_clusters(self, X: np.ndarray, method: str = "kmeans", max_clusters: int = 10) -> int:
    ...
    def multidimensional_scaling(
        self, X: np.ndarray, n_components: int = 2, metric: bool = True, dissimilarity: str = "euclidean"
    ) -> Dict[str, Any]:
        """
        ...
        # Perform MDS
        mds = MDS(n_components=n_components, metric=metric, dissimilarity="precomputed", random_state=42)
    ...
    def mahalanobis_distance(self, X: np.ndarray, robust: bool = False) -> np.ndarray:
        """
    ...
    def hotelling_t2_test(
        self, X1: np.ndarray, X2: Optional[np.ndarray] = None, mu0: Optional[np.ndarray] = None
    ) -> Dict[str, Any]:
        """
