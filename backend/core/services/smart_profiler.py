...
from ..data_profiler import DataProfiler, DatasetProfile, VariableType
...
class SmartProfiler:
    ...
    PREPOST_PATTERNS = [
        (r"(.+)[-_\s]?(pre|before|baseline|t[0-9]*0)$", r"(.+)[-_\s]?(post|after|followup|t[0-9]*1)$"),
        (r"(.+)[-_\s]?1$", r"(.+)[-_\s]?2$"),
        (r"(pre|before|baseline)[-_\s]?(.+)", r"(post|after|followup)[-_\s]?(.+)"),
    ]
    ...
    def profile_and_recommend(self, df: pd.DataFrame, user_hint: Optional[str] = None) -> SmartProfileResult:
        """
    ...
    def _build_variable_summary(self, profile: DatasetProfile) -> Dict[str, Dict[str, Any]]:
        ...
        for name, var_prof in profile.variables.items():
            summary[name] = {
                ...
                "is_normal": (var_prof.normality_p_value > 0.05 if var_prof.normality_p_value is not None else None),
    ...
    def _infer_research_questions(
        ...
        _ = [name for name, v in profile.variables.items() if v.type == VariableType.ORDINAL]
        ...
        if len(continuous_vars) >= 2:
            for i, var1 in enumerate(continuous_vars):
                for var2 in continuous_vars[i + 1 :]:
                    both_normal = var_summary[var1].get("is_normal", False) and var_summary[var2].get(
                        "is_normal", False
                    )
    ...
    def _get_recommendations(
        ...
        for question in inferred_questions[:5]:  # Top 5 questions
            ...
            try:
                ats_profile = self.test_selector.analyze_data(data_arrays, variable_names=var_names[: len(data_arrays)])
    ...
    def _assess_data_health(self, df: pd.DataFrame, profile: DatasetProfile) -> DataHealthCard:
        ...
        if missing_pct == 0:
            ...
            issues.append(
                {"severity": "critical", "message": f"{missing_pct:.1f}% missing data — significant data loss"}
            )
        ...
        if profile.n_rows < 10:
            ...
            issues.append(
                {"severity": "warning", "message": f"Small sample (n={profile.n_rows}) — limited test options"}
            )
        else:
            strengths.append(f"Adequate sample size (n={profile.n_rows})")

        continuous_count = sum(1 for v in profile.variables.values() if v.type == VariableType.CONTINUOUS)
        ...
        # Overall score
        overall_score = round(completeness_score * 0.35 + quality_score * 0.30 + suitability_score * 0.35, 1)
