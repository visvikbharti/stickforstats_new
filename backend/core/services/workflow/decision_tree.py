...
class DecisionNode:
    ...
    def get_recommendation(self, context: Dict[str, Any]) -> Optional[DecisionOption]:
        ...
        sorted_options = sorted(evaluated_options, key=lambda x: priority[x.recommendation_level])
...
class DecisionTree:
    ...
    def get_next_decision(self, current_node_id: str, selected_option: str, context: Dict[str, Any]) -> Optional[str]:
        """
    ...
    def get_decision_path(self, target: str) -> List[str]:
        ...
        if target == "visualization_selection":
            return ["data_type_selection", "continuous_test_selection", "normality_check", "visualization_selection"]
        return []

    # Evaluation functions for dynamic recommendations

    def _evaluate_data_type(self, node: DecisionNode, context: Dict[str, Any]) -> List[DecisionOption]:
    ...
    def _evaluate_continuous_test(self, node: DecisionNode, context: Dict[str, Any]) -> List[DecisionOption]:
    ...
    def _evaluate_normality_method(self, node: DecisionNode, context: Dict[str, Any]) -> List[DecisionOption]:
...
class DecisionEngine:
    ...
    def make_decision(self, option_id: str, context: Dict[str, Any]) -> Optional[DecisionNode]:
        ...
        self.decision_history.append(
            {"node_id": self.current_node.id, "selected_option": option_id, "timestamp": pd.Timestamp.now().isoformat()}
        )

        # Get next node
        next_node_id = self.tree.get_next_decision(self.current_node.id, option_id, context)
