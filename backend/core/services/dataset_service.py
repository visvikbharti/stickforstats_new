...
class DatasetService:
    ...
    def save_uploaded_dataset(
        self, user: User, file_obj, name: str, description: str = "", has_header: bool = True, delimiter: str = ","
        ...
        try:
            ...
            if file_obj.size > user.max_dataset_size:
                return None, DatasetValidationResult(
                    is_valid=False,
                    errors=[
                        f"File size ({file_obj.size} bytes) exceeds your subscription limit ({user.max_dataset_size} bytes)"
                    ],
                    warnings=[],
                )
            ...
            return None, DatasetValidationResult(
                is_valid=False, errors=[f"Error processing dataset: {str(e)}"], warnings=[]
            )
        ...
        try:
            ...
            if df is None:
                return DatasetValidationResult(
                    is_valid=False, errors=["Unable to read file. Please check the file format."], warnings=[]
                )

            # Basic validation
            if df.empty:
                return DatasetValidationResult(is_valid=False, errors=["Dataset is empty."], warnings=[])
            ...
            for col in df.columns:
                if pd.api.types.is_numeric_dtype(df[col]):
                    ...
                    if len(unique_vals) <= 20:  # Only store if not too many
                        column_types[col] = {"type": dtype, "unique_values": unique_vals}
                        continue
                else:
            ...
            return DatasetValidationResult(
                is_valid=is_valid, errors=errors, warnings=warnings, data_info=data_info, metadata=metadata
            )

        except Exception as e:
            logger.error(f"Error validating dataset: {str(e)}")
            return DatasetValidationResult(is_valid=False, errors=[f"Error validating dataset: {str(e)}"], warnings=[])
    ...
    def _read_dataset_file(self, dataset: Dataset) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            if dataset.file_type == "csv":
                return pd.read_csv(file_path, header=0 if dataset.has_header else None, delimiter=dataset.delimiter)
    ...
    def _generate_metadata(
        self, df: pd.DataFrame, numeric_columns: List[str], categorical_columns: List[str]
    ) -> Dict[str, Any]:
        """
