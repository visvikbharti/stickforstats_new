...
class AdvancedANOVAService:
    ...
    def two_way_anova(
        ...
        for l1 in levels1:
            for l2 in levels2:
                cell_data = clean_data[(clean_data[factor1] == l1) & (clean_data[factor2] == l2)][dependent_var]
                cell_stats.append(
                    {
                        factor1: l1,
                        factor2: l2,
        ...
        ss_factor1 = np.sum([count * (mean - grand_mean) ** 2 for mean, count in zip(factor1_means, factor1_counts)])
        ...
        ss_factor2 = np.sum([count * (mean - grand_mean) ** 2 for mean, count in zip(factor2_means, factor2_counts)])
    ...
    def repeated_measures_anova(
        ...
        within_factor: str,
        dependent_var: str,
        sphericity_correction: str = "none",
    ) -> Dict[str, Any]:
        """
        ...
        results = {
            ...
            "corrected_df": {"conditions": df_conditions_corrected, "error": df_error_corrected},
            "condition_means": {
    ...
    def ancova(
        self, data: pd.DataFrame, dependent_var: str, factor: str, covariate: str, check_homogeneity_slopes: bool = True
    ) -> Dict[str, Any]:
        """
    ...
    def mixed_anova(
        self, data: pd.DataFrame, dependent_var: str, between_factor: str, within_factor: str, subject_id: str
    ) -> Dict[str, Any]:
        """
    ...
    def _tukey_hsd(
        self, data: pd.DataFrame, dep_var: str, factor: str, ms_error: float, df_error: int
    ) -> List[Dict[str, Any]]:
    ...
    def _simple_effects_analysis(
        self, data: pd.DataFrame, dep_var: str, factor1: str, factor2: str, ms_error: float, df_error: int
    ) -> Dict[str, Any]:
    ...
    def _repeated_measures_post_hoc(
        self, wide_data: pd.DataFrame, ms_error: float, df_error: int
    ) -> List[Dict[str, Any]]:
    ...
    def _test_homogeneity_slopes(self, data: pd.DataFrame, dep_var: str, factor: str, covariate: str) -> Dict[str, Any]:
        ...
        X_full = np.column_stack(
            [np.ones(len(data)), x, dummies.values, x.reshape(-1, 1) * dummies.values]  # Interaction terms
        )

        # Reduced model without interaction
        X_reduced = np.column_stack([np.ones(len(data)), x, dummies.values])
        ...
        try:
            ...
            return {"f_statistic": np.nan, "p_value": np.nan, "homogeneous": None}

    def _ancova_post_hoc(
        self, adjusted_means: pd.DataFrame, ms_error: float, df_error: int, n_total: int
    ) -> List[Dict[str, Any]]:
    ...
    def _interpret_repeated_measures(self, p_value: float, eta_squared: float, n_conditions: int) -> str:
        ...
        if p_value < 0.05:
            ...
            return (
                f"There is a statistically significant difference across the {n_conditions} conditions "
                f"(p={p_value:.4f}), with {effect} (η²={eta_squared:.3f})."
            )
        else:
            return (
                f"No statistically significant difference found across the {n_conditions} conditions "
                f"(p={p_value:.4f})."
            )
    ...
    def _interpret_ancova(self, p_value: float, eta_squared: float, regression_coef: float) -> str:
        ...
        if p_value < 0.05:
            effect = self._interpret_eta_squared(eta_squared)
            return (
                f"After controlling for the covariate, there is a statistically significant "
                f"difference between groups (p={p_value:.4f}), with {effect} (η²={eta_squared:.3f}). "
                f"The covariate coefficient is {regression_coef:.3f}."
            )
        else:
            return (
                f"After controlling for the covariate, no statistically significant "
