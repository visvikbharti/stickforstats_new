...
class PowerAnalysisService:
        ...
        if test_type == "one-sample" or test_type == "paired":
            ...
            if effect_size is not None and n is not None:
                ...
                if alternative == "two-sided":
                    ...
                    power_calc = 1 - (t.cdf(crit_val, df, nc_param) - t.cdf(-crit_val, df, nc_param))
    ...
    def effect_size_converter(self, effect_size: float, from_type: str, to_type: str, **kwargs) -> float:
        """
    ...
    def _interpret_power_analysis(self, result: Dict[str, Any]) -> str:
        ...
        if "power" in result and result["power"] < 0.8:
            return (
                f"Power is {result['power']:.2f}, below the conventional threshold of 0.80. "
                f"Consider increasing sample size or effect size."
            )
        elif "n" in result:
            if test_type == "two-sample":
                return (
                    f"Required sample size: {result.get('n1', result['n'])} per group "
                    f"(total n={result.get('n_total', result['n'] * 2)}) "
                    f"to achieve power of {result.get('power', 0.8):.2f}"
                )
            else:
                return f"Required sample size: {result['n']} " f"to achieve power of {result.get('power', 0.8):.2f}"
    ...
    def _interpret_anova_power(self, result: Dict[str, Any]) -> str:
        ...
        if "power" in result and result["power"] < 0.8:
            return (
                f"Power is {result['power']:.2f}, below the conventional threshold. "
                f"Consider increasing sample size or effect size."
            )
        elif "n_per_group" in result:
            return (
                f"Required sample size: {result['n_per_group']} per group "
    ...
    def _interpret_correlation_power(self, result: Dict[str, Any]) -> str:
        ...
        if "power" in result and result["power"] < 0.8:
            return f"Power is {result['power']:.2f}, below conventional threshold. " f"Consider increasing sample size."
        elif "n" in result:
    ...
    def _interpret_proportion_power(self, result: Dict[str, Any]) -> str:
        ...
        if "power" in result and result["power"] < 0.8:
            ...
            return (
                f"Required sample size: {result['n1']} and {result['n2']} "
