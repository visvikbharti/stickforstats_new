...
COVARIANCE_STRUCTURES = {
    ...
    "ar1": {"name": "AR(1)", "description": "First-order autoregressive", "parameters": 2},
    "unstructured": {"name": "Unstructured", "description": "Free covariance matrix", "parameters": "varies"},
}
...
class LMMResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "convergence": {"converged": self.converged, "warnings": self.warnings},
            "fit_statistics": {
            ...
            "residual": {"variance": round(self.residual_variance, 6), "std": round(self.residual_std, 6)},
    ...
    if compute_blups:
        try:
            ...
            for group, effects in random_effects.items():
                blups[str(group)] = {col: float(val) for col, val in effects.items()}
        except Exception:
            pass
...
def fit_random_intercept_model(
    data: pd.DataFrame, outcome: str, fixed_effects: List[str], grouping_var: str
) -> LMMResult:
    """
...
def generate_model_equation(
    outcome: str, fixed_effects: List[str], grouping_var: str, random_slope: Optional[str] = None
) -> Dict[str, str]:
    """
    ...
    # Level 2 equations
    level2_intercept = "β_0j = γ_00 + u_0j"

    # Combined/composite form
    if random_slope:
        level2_slope = "β_1j = γ_10 + u_1j"
