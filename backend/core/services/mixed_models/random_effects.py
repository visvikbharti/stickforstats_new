...
def caterpillar_plot_data(
    ...
    if not blups:
        return {
            'error': 'No random effects available',
            'groups': [],
            'effects': [],
            'ci_lower': [],
            'ci_upper': []
...
def group_specific_predictions(
    lmm_result,
    data: pd.DataFrame,
    grouping_var: str,
    x_var: str = None
...
def shrinkage_plot_data(
    ...
    return {
        ...
        'shrinkage_ratio': [
            round(abs(b) / abs(o), 4) if abs(o) > 0.001 else 1.0
            for o, b in zip(ols_estimates, blup_estimates)
        ],
        'interpretation': (
            'Points below the diagonal line show shrinkage toward zero. '
            'Smaller groups experience more shrinkage.'
        )
    }
