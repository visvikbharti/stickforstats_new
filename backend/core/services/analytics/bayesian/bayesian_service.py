...
class BayesianService:
    ...
    def check_availability(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": (
                "Full Bayesian analysis is available."
                if self.pymc_available
                else "Install PyMC3 and ArviZ for full Bayesian analysis capabilities. " "Run: pip install pymc3 arviz"
            ),
        }

    @safe_operation
    def perform_bayesian_ttest(
        self,
        data: pd.DataFrame,
        group_col: str,
        value_col: str,
        ...
        if len(group1_data) < 3 or len(group2_data) < 3:
            raise ValueError(
                f"Each group must have at least 3 observations, but found {len(group1_data)} and {len(group2_data)}"
            )

        # Check if PyMC3 is available for full Bayesian analysis
        if self.pymc_available:
            return self._perform_pymc_ttest(group1_data, group2_data, groups, rope_width, credible_interval, n_samples)
        else:
            return self._perform_compatibility_ttest(group1_data, group2_data, groups, rope_width, credible_interval)

    def _perform_pymc_ttest(
        self,
        ...
        with pm.Model():
            ...
            # Likelihood
            _ = pm.Normal("group1_obs", mu=group1_mean, sigma=group1_std, observed=group1_data)
            _ = pm.Normal("group2_obs", mu=group2_mean, sigma=group2_std, observed=group2_data)
            ...
            pm.Deterministic("effect_size", diff / pooled_std)
        ...
        results["interpretation"] = self._interpret_bayesian_ttest(
            results["probabilities"]["direction"], results["probabilities"]["in_rope"], results["statistics"]
        )

        return results

    def _perform_compatibility_ttest(
        self,
        group1_data: np.ndarray,
        group2_data: np.ndarray,
        groups: np.ndarray,
    ...
    def _interpret_bayesian_ttest(
        self, prob_direction: float, prob_rope: float, summary: Dict[str, Any]
    ) -> Dict[str, str]:
        ...
        context = (
            f"The estimated difference between groups is {difference:.2f} " f"[{diff_lower:.2f}, {diff_upper:.2f}]. "
        )
        ...
        return {"main": main_interp, "context": context, "guidance": guidance}

    @safe_operation
    def perform_bayesian_correlation(
        self,
        data: pd.DataFrame,
        var1: str,
        var2: str,
        ...
        # Check if PyMC3 is available for full Bayesian analysis
        if self.pymc_available:
            return self._perform_pymc_correlation(x, y, var1, var2, rope_width, credible_interval, n_samples)
        else:
            return self._perform_compatibility_correlation(x, y, var1, var2, rope_width, credible_interval)

    def _perform_pymc_correlation(
        self,
        x: np.ndarray,
        y: np.ndarray,
        var1: str,
        ...
        # Extract posterior samples
        posterior_samples = {"rho": rho_samples.tolist()}
    ...
    def _perform_compatibility_correlation(
        self, x: np.ndarray, y: np.ndarray, var1: str, var2: str, rope_width: float, credible_interval: float
    ) -> Dict[str, Any]:
        """
        ...
        r_samples = np.array([stats.pearsonr(x[idx], y[idx])[0] for idx in indices])
        ...
        results = {
            ...
            "statistics": {
                ...
                "frequentist": {"r": float(r), "p_value": float(p_value), "n": int(n)},
            },
            "probabilities": {
                "direction": float(prob_direction),
                "in_rope": float(prob_rope),
            ...
            "bayes_factor": float(bf10),
            "samples": {"rho": r_samples.tolist()},
            "compatibility_mode": True,
            "analysis_type": "bayesian_correlation",
        }

        # Add interpretation
        results["interpretation"] = self._interpret_bayesian_correlation(prob_direction, prob_rope, bf10, r)
    ...
    def _interpret_bayesian_correlation(
        self, prob_direction: float, prob_rope: float, bf10: float, rho: float
    ) -> Dict[str, str]:
        ...
        return {"main": main_interp, "guidance": guidance}

    @safe_operation
    def perform_bayesian_regression(
        self,
        data: pd.DataFrame,
        target: str,
        predictors: List[str],
        ...
        # Check if PyMC3 is available for full Bayesian analysis
        if self.pymc_available:
            return self._perform_pymc_regression(X, y, target, predictors, credible_interval, n_samples)
        else:
            return self._perform_compatibility_regression(X, y, target, predictors, credible_interval)

    def _perform_pymc_regression(
        self,
        X: pd.DataFrame,
        y: pd.Series,
        target: str,
        ...
        with pm.Model():
            ...
            # Likelihood
            _ = pm.Normal("likelihood", mu=mu, sigma=sigma, observed=y.values)
            ...
            pickle.dump(
                {
                    ...
                    "model_info": {
                        ...
                        "date_created": datetime.now().isoformat(),
                        "parameters": {"credible_interval": credible_interval, "n_samples": n_samples},
                        "X_std_params": {"mean": X.mean().to_dict(), "std": X.std().to_dict()},
                    },
                },
                f,
            )
    ...
    def _perform_compatibility_regression(
        self, X: pd.DataFrame, y: pd.Series, target: str, predictors: List[str], credible_interval: float
    ) -> Dict[str, Any]:
        """
        ...
        intercept_ci = (model.intercept_ - t_critical * se[0], model.intercept_ + t_critical * se[0])

        coef_cis = []
        for i, coef in enumerate(model.coef_):
            coef_cis.append((coef - t_critical * se[i + 1], coef + t_critical * se[i + 1]))
    ...
    def _interpret_bayesian_regression(
        self,
        coef_probabilities: Dict[str, Dict[str, float]],
        r_squared: float,
        ...
        significant_predictors = [
            (pred, probs) for pred, probs in coef_probabilities.items() if probs.get("significant", 0) > 0.75
        ]
        ...
        return {"main": main_interp, "coefficient_details": coef_details, "guidance": guidance}
    ...
    def get_model_info(self, model_id: str) -> Dict[str, Any]:
        ...
        return {"model_id": model_id, "model_info": model_data["model_info"]}
