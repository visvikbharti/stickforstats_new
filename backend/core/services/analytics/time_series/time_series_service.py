...
class TimeSeriesService:
    ...
    def perform_decomposition(
        self, series: pd.Series, period: Optional[int] = None, model: str = "additive"
    ) -> Dict[str, Any]:
        """
    ...
    def _interpret_decomposition(self, trend: float, seasonal: float, residual: float) -> Dict[str, str]:
        """Create interpretations for decomposition results."""
        # Trend interpretation
        if trend > 0.5:
            trend_interp = (
                "Strong trend component (>50% of variance), indicating a " "significant long-term movement in the data."
            )
        elif trend > 0.3:
            trend_interp = (
            ...
            trend_interp = (
                "Weak trend component (10-30% of variance), indicating " "a slight long-term movement in the data."
            )
        else:
            trend_interp = (
                "Minimal trend component (<10% of variance), indicating " "little to no long-term movement in the data."
            )

        # Seasonal interpretation
        if seasonal > 0.5:
            seasonal_interp = (
                "Strong seasonal component (>50% of variance), indicating " "significant cyclical patterns in the data."
            )
        elif seasonal > 0.3:
            seasonal_interp = (
            ...
            seasonal_interp = (
                "Weak seasonal component (10-30% of variance), indicating " "slight cyclical patterns in the data."
            )
        else:
            seasonal_interp = (
        ...
        if trend > 0.3 and seasonal > 0.3:
            ...
            model_rec = (
                "Consider ARIMA models to capture trend component. " "Differencing may be needed if non-stationary."
            )
        elif seasonal > 0.3:
            model_rec = "Consider seasonal models like SARIMA or ETS to capture " "the strong seasonal patterns."
        else:
            model_rec = (
    ...
    @safe_operation
    def analyze_acf_pacf(self, series: pd.Series, nlags: int = 40, alpha: float = 0.05) -> Dict[str, Any]:
        """
        ...
        sig_acf_lags = [
            i
            for i, (val, lower, upper) in enumerate(zip(acf_values, acf_lower, acf_upper))
            if (val > upper or val < lower) and i > 0
        ]

        sig_pacf_lags = [
            i
            for i, (val, lower, upper) in enumerate(zip(pacf_values, pacf_lower, pacf_upper))
            if (val > upper or val < lower) and i > 0
        ]
        ...
        interpretation = self._interpret_acf_pacf(acf_values, pacf_values, sig_acf_lags, sig_pacf_lags)
        ...
        return {
            ...
            "suggested_orders": {"p": arima_p, "q": arima_q},
            "interpretation": interpretation,
        }

    def _interpret_acf_pacf(
        self, acf_values: np.ndarray, pacf_values: np.ndarray, sig_acf_lags: List[int], sig_pacf_lags: List[int]
    ) -> Dict[str, str]:
        ...
        acf_decaying = all(abs(acf_simple[i]) >= abs(acf_simple[i + 1]) for i in range(len(acf_simple) - 1))

        pacf_decaying = all(abs(pacf_simple[i]) >= abs(pacf_simple[i + 1]) for i in range(len(pacf_simple) - 1))

        acf_cutoff = len(sig_acf_lags) < len(acf_simple) / 2
        pacf_cutoff = len(sig_pacf_lags) < len(pacf_simple) / 2

        acf_seasonal = any(
            acf_simple[i] > acf_simple[i - 1] and acf_simple[i] > acf_simple[i + 1]
            for i in range(1, len(acf_simple) - 1)
        )

        pacf_seasonal = any(
            pacf_simple[i] > pacf_simple[i - 1] and pacf_simple[i] > pacf_simple[i + 1]
        ...
        if acf_decaying and pacf_cutoff:
            pattern_description = (
                "The ACF shows a gradual decay pattern, while the PACF shows " "a more abrupt cutoff after a few lags. "
            )
            model_suggestion = (
        ...
        # ARMA process pattern: Both ACF and PACF decay
            ...
            pattern_description = "Both the ACF and PACF show gradual decay patterns. "
            model_suggestion = (
        ...
        # White noise pattern: No significant lags
            ...
            pattern_description = (
                "Neither the ACF nor PACF show significant lags, suggesting " "that the series may be white noise. "
            )
            model_suggestion = (
    ...
    @safe_operation
    def fit_arima_model(
        self, series: pd.Series, order: Tuple[int, int, int], seasonal_order: Optional[Tuple[int, int, int, int]] = None
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "order": order_str,
            "metrics": {"aic": float(aic), "bic": float(bic)},
            "residuals": {
                "mean": residual_mean,
                "std": residual_std,
            ...
            "interpretation": self._interpret_model_fit(
                model_type, aic, bic, residual_mean, residual_std, residuals_white_noise
            ),
        }

    def _interpret_model_fit(
        self, model_type: str, aic: float, bic: float, residual_mean: float, residual_std: float, is_white_noise: bool
    ) -> Dict[str, str]:
    ...
    def fit_exponential_smoothing(
        ...
        return {
            ...
            "metrics": {"aic": float(aic), "bic": float(bic)},
            "residuals": {"mean": residual_mean, "std": residual_std},
            "interpretation": {
    ...
    @safe_operation
    def forecast(self, model_id: str, steps: int, confidence_level: float = 0.95) -> Dict[str, Any]:
        """
                    ...
                    models.append(
                        {
                            ...
                            "metrics": {"aic": model_info.get("aic"), "bic": model_info.get("bic")},
                        }
                    )
