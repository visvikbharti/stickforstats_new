...
class MLService:
    ...
    def __init__(self):
        ...
        self.regression_models = {
            ...
            "lasso": {
                ...
                "params": {"alpha": [0.01, 0.1, 1.0, 10.0], "max_iter": [1000, 2000, 5000]},
        ...
        self.classification_models = {
            "logistic_regression": {
                ...
                "params": {"C": [0.1, 1, 10], "solver": ["lbfgs", "liblinear"], "max_iter": [1000, 2000]},
        ...
        self.clustering_models = {
            "kmeans": {
                ...
                "params": {"n_clusters": [2, 3, 4, 5, 6, 7, 8], "init": ["k-means++", "random"]},
        ...
        if handle_categorical:
            ...
            if not categorical_cols.empty:
                ...
                for col in categorical_cols:
                    ...
                    # Store encoder
                    categorical_encoders[col] = {"encoder": encoder, "encoded_names": encoded_names}
        ...
        if y.dtype in ["object", "category"] or y.nunique() < 10:
            ...
            if handle_categorical:
                ...
                preprocessing_steps.append(
                    f"Label encoded target '{target}' with {len(target_encoder.classes_)} classes"
                )
    ...
    def train_model(
        ...
        if perform_gridsearch and param_grid:
            ...
            cv_results = {"mean_test_score": np.mean(cv_scores), "std_test_score": np.std(cv_scores, ddof=1)}
    ...
    def _extract_feature_importance(self, model: Any, feature_names: List[str]) -> Dict[str, float]:
        ...
        try:
            if hasattr(model, "feature_importances_"):
                ...
                feature_importance = {name: float(value) for name, value in zip(feature_names, importance)}
            elif hasattr(model, "coef_"):
                coef = model.coef_
                # Handle different shapes of coefficients
                if len(coef.shape) == 1:
                    # Single target or binary classification
                    feature_importance = {name: float(value) for name, value in zip(feature_names, np.abs(coef))}
                else:
                    # Multi-class or multi-target
                    avg_importance = np.mean(np.abs(coef), axis=0)
                    feature_importance = {name: float(value) for name, value in zip(feature_names, avg_importance)}
        except:
    ...
    def _evaluate_regression(
        self, y_true: np.ndarray, y_pred: np.ndarray, X_test: pd.DataFrame, model: Any
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "predictions": {"true": y_true.tolist(), "predicted": y_pred.tolist()},
        }
    ...
    def _evaluate_classification(
        ...
        results = {
            "metrics": {"accuracy": float(accuracy), "class_report": class_report},
    ...
    def predict(self, model_uuid: str, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        if preprocessing.get("feature_scaler"):
            ...
            X = pd.DataFrame(feature_scaler.transform(X[feature_names]), columns=feature_names, index=X.index)
        else:
        ...
        if task_type == "classification" and preprocessing.get("target_encoder"):
            ...
            if hasattr(model, "predict_proba"):
                ...
                probabilities = {"values": y_proba.tolist(), "classes": target_encoder.classes_.tolist()}

            return {"predictions": y_pred.tolist(), "probabilities": probabilities}
        elif task_type == "clustering":
            # For clustering, also return distances to cluster centers if possible
            if hasattr(model, "transform"):
                distances = model.transform(X)
                return {"clusters": y_pred.tolist(), "distances": distances.tolist()}
            else:
                return {"clusters": y_pred.tolist()}
        else:
            # For regression
