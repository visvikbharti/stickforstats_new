...
from .types import TestType, AnalysisOptions, VariableSelection
...
class AdvancedStatisticalAnalysisService:
    ...
    def analyze(
        self, data: pd.DataFrame, test_type: TestType, variables: VariableSelection, options: AnalysisOptions
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_inputs(self, data: pd.DataFrame, test_type: TestType, variables: VariableSelection) -> None:
        """
    ...
    def _run_one_way_anova(
        self, data: pd.DataFrame, variables: VariableSelection, options: AnalysisOptions
    ) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary containing analysis results
        """
        results = {"anova_results": {}, "descriptive_stats": {}, "assumption_checks": {}, "post_hoc": {}}
        ...
        if options.assumption_checks:
            ...
            for group in data[group_var].unique():
                ...
                if len(group_data) > 3:  # Need at least 4 observations for Shapiro-Wilk
                    ...
                    normality_results[str(group)] = {"statistic": stat, "p_value": p, "normal": p > options.alpha}
        ...
        if options.post_hoc:
            # Tukey HSD
            tukey_results = pairwise_tukeyhsd(data[dep_var].dropna(), data[group_var].dropna(), alpha=options.alpha)
            ...
            results["post_hoc"] = {"tukey_hsd": tukey_dict}

        # Calculate descriptive statistics
        if options.include_descriptive:
            desc_stats = (
                data.groupby(group_var)[dep_var]
                .agg(
                    [
                        "count",
                        "mean",
            ...
            desc_stats = desc_stats.rename(columns={"<lambda_0>": "q1", "<lambda_1>": "q3"})
    ...
    def _run_two_way_anova(
        ...
        Returns:
            Dictionary containing analysis results
        """
        results = {"anova_results": {}, "descriptive_stats": {}, "assumption_checks": {}, "interaction_analysis": {}}
        ...
        # Run ANOVA
        anova_results = pg.anova(data=data, dv=dep_var, between=[factor_a, factor_b], detailed=True)
        ...
        if options.assumption_checks:
            ...
            for a_level in data[factor_a].unique():
                for b_level in data[factor_b].unique():
                    group_data = data[(data[factor_a] == a_level) & (data[factor_b] == b_level)][dep_var].dropna()

                    group_key = f"{a_level}_{b_level}"
                    if len(group_data) > 3:  # Need at least 4 observations for Shapiro-Wilk
                        stat, p = stats.shapiro(group_data)
                        normality_results[group_key] = {"statistic": stat, "p_value": p, "normal": p > options.alpha}
        ...
        if options.include_descriptive:
            desc_stats = (
                data.groupby([factor_a, factor_b])[dep_var]
        ...
        if not interaction_row.empty and interaction_row["p-unc"].iloc[0] < options.alpha:
            ...
            results["interaction_analysis"] = {"has_significant_interaction": True, "simple_effects": simple_effects}
        else:
            results["interaction_analysis"] = {"has_significant_interaction": False}
    ...
    def _run_manova(self, data: pd.DataFrame, variables: VariableSelection, options: AnalysisOptions) -> Dict[str, Any]:
        """
        Run MANOVA analysis.
        ...
        Returns:
            Dictionary containing analysis results
        """
        results = {"manova_results": {}, "descriptive_stats": {}, "assumption_checks": {}, "univariate_tests": {}}
        ...
        manova_results = pg.multivariate_tests(data=data, dv=dep_vars, between=group_var)
        ...
        if options.assumption_checks:
            ...
            for dep_var in dep_vars:
                ...
                for group in data[group_var].unique():
                    ...
                    if len(group_data) > 3:  # Need at least 4 observations for Shapiro-Wilk
                        stat, p = stats.shapiro(group_data)
                        var_normality[str(group)] = {"statistic": stat, "p_value": p, "normal": p > options.alpha}
        ...
        if options.include_descriptive:
            ...
            for dep_var in dep_vars:
                desc_summary = (
                    data.groupby(group_var)[dep_var].agg(["count", "mean", "std", "min", "median", "max"]).reset_index()
                )
                desc_stats[dep_var] = desc_summary.to_dict("records")
    ...
    def _run_kruskal_wallis(
        ...
        Returns:
            Dictionary containing analysis results
        """
        results = {"kruskal_results": {}, "descriptive_stats": {}, "post_hoc": {}}
        ...
        results["kruskal_results"] = {"h_statistic": h_stat, "p_value": p_value, "significant": p_value < options.alpha}
        ...
        if p_value < options.alpha and options.post_hoc:
            try:
                ...
                dunn_results = sp.posthoc_dunn(
                    posthoc_data, val_col=dep_var, group_col=group_var, p_adjust=options.correction_method
                )
        ...
        if options.include_descriptive:
            ...
            for group in data[group_var].unique():
                ...
                if len(group_data) > 0:
                    ...
                    group_ranks = [
                        all_ranks[all_data.index.get_loc(idx)] for idx in group_indices if idx in all_data.index
                    ]

                    mean_ranks.append({group_var: group, "mean_rank": np.mean(group_ranks) if group_ranks else np.nan})

            # Convert to dictionary
            results["descriptive_stats"] = {"summary": desc_stats.to_dict("records"), "mean_ranks": mean_ranks}
    ...
    def _run_friedman(
        ...
        Returns:
            Dictionary containing analysis results
        """
        results = {"friedman_results": {}, "descriptive_stats": {}, "post_hoc": {}}
        ...
        # Run Friedman test
        try:
            friedman_result = pg.friedman(data=data, dv=dep_var, within=within_var, subject=subject_var)
            ...
            if friedman_result.loc[0, "p-unc"] < options.alpha and options.post_hoc:
                try:
                    posthoc = pg.pairwise_tests(
                        data=data, dv=dep_var, within=within_var, subject=subject_var, parametric=False
                    )
            ...
            if options.include_descriptive:
                desc_stats = (
                    data.groupby(within_var)[dep_var]
                    .agg(
                        [
                            "count",
                            "mean",
    ...
    def _run_repeated_measures_anova(
        ...
        try:
            ...
            if len(variables.independent_variables) > 1:
                ...
                rm_anova = pg.rm_anova(data=data, dv=dep_var, within=within_factors, subject=subject_var, detailed=True)
            else:
                rm_anova = pg.rm_anova(data=data, dv=dep_var, within=within_var, subject=subject_var, detailed=True)
            ...
            if len(variables.independent_variables) == 1:
                sphericity = pg.sphericity(data=data, dv=dep_var, within=within_var, subject=subject_var)
    ...
    def _run_mixed_anova(
        ...
        try:
            mixed_anova = pg.mixed_anova(
                data=data, dv=dep_var, between=between_var, within=within_var, subject=subject_var, detailed=True
            )
                    ...
                    for idx, row in mixed_anova.iterrows():
                        if row["Source"] not in ["Residual", "Sphericity Corrections"] and row["p-unc"] < options.alpha:
                            ...
                            if source == between_var:
                                posthoc = pg.pairwise_tukey(data=data, dv=dep_var, between=between_var)
                            ...
                            # Interaction effect
                                    ...
                                    if len(subset) > 0:
                                        try:
                                            simple_rm = pg.rm_anova(
                                                data=subset, dv=dep_var, within=within_var, subject=subject_var
                                            )
                                            simple_effects[f"{within_var}_at_{between_level}"] = simple_rm.to_dict(
                                                "records"
                                            )
                                            ...
                                            if simple_rm.loc[0, "p-unc"] < options.alpha:
                                                ...
                                                simple_effects[
                                                    f"{within_var}_at_{between_level}_posthoc"
                                                ] = posthoc.to_dict("records")
                                        except Exception as e:
                                            logger.warning(
                                                f"Error in simple effect for {within_var} at {between_level}: {e}"
                                            )
                                ...
                                wide_data = data.pivot(
                                    index=[subject_var, between_var], columns=within_var, values=dep_var
                                )
                                for within_level in wide_data.columns:
                                    ...
                                    if len(level_data) > 0:
                                        try:
                                            simple_anova = pg.anova(data=level_data, dv=dep_var, between=between_var)
                                            simple_effects[f"{between_var}_at_{within_level}"] = simple_anova.to_dict(
                                                "records"
                                            )
                                            ...
                                            if simple_anova.loc[0, "p-unc"] < options.alpha:
                                                posthoc = pg.pairwise_tukey(
                                                    data=level_data, dv=dep_var, between=between_var
                                                )
                                                simple_effects[
                                                    f"{between_var}_at_{within_level}_posthoc"
                                                ] = posthoc.to_dict("records")
                                        except Exception as e:
                                            logger.warning(
                                                f"Error in simple effect for {between_var} at {within_level}: {e}"
                                            )
            ...
            if options.include_descriptive:
                desc_stats = (
                    data.groupby([between_var, within_var])[dep_var]
                    .agg(["count", "mean", "std", "min", "median", "max"])
                    .reset_index()
                )
