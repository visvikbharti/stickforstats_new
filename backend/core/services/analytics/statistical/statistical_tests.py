...
from core.guardian import GuardianIntegratedService, GuardianEnrichedResult
...
class StatisticalTestService:
    ...
    def run_t_test(self, data: pd.DataFrame, test_type: str, variables: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        """
        Run a t-test.
    ...
    def run_correlation_test(
        self, data: pd.DataFrame, test_type: str, variables: Dict[str, Any], **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def run_chi_square_test(
        ...
        if test_type == "independence":
            ...
            if "variable1" not in variables or "variable2" not in variables:
                raise ValueError(
                    "Chi-square test of independence requires 'variable1' and 'variable2' in variables dict"
                )
            ...
            if len(expected_probs) != len(observed):
                raise ValueError(
                    f"Length of expected_probs ({len(expected_probs)}) does not match number of categories ({len(observed)})"
                )
...
class GuardianStatisticalTestService(GuardianIntegratedService):
    ...
    def run_t_test_guarded(
        ...
        **kwargs,
        ...
        return self.execute_with_guardian(
            test_type="t_test", data=guardian_data, compute_function=compute, alpha=alpha, expert_mode=expert_mode
        )
    ...
    def run_correlation_guarded(
        ...
        **kwargs,
        ...
        if x_var and y_var:
            mask = ~(data[x_var].isna() | data[y_var].isna())
            guardian_data = [data.loc[mask, x_var].values, data.loc[mask, y_var].values]
        else:
            guardian_data = data
    ...
    def run_chi_square_guarded(
        ...
        **kwargs,
        ...
        return self.execute_with_guardian(
            test_type="chi_square", data=guardian_data, compute_function=compute, alpha=alpha, expert_mode=expert_mode
        )
    ...
    def run_nonparametric_guarded(
        ...
        **kwargs,
        ...
        guardian_type_map = {"mann_whitney": "mann_whitney", "wilcoxon": "mann_whitney", "kruskal": "kruskal_wallis"}
    ...
    def _prepare_data_for_guardian(
        self, data: pd.DataFrame, test_type: str, variables: Dict[str, Any]
    ) -> List[np.ndarray]:
        """
        ...
        if test_type in ["independent", "mann_whitney"]:
            ...
            if var1 and var2:
                mask = ~(data[var1].isna() | data[var2].isna())
                return [data.loc[mask, var1].values, data.loc[mask, var2].values]
