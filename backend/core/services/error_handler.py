...
class ErrorHandler:
    ...
    def api_exception_handler(exc, context):
        ...
        if response is None:
            ...
            response = Response(
                {"error": user_friendly_message, "detail": str(exc), "type": type(exc).__name__},
...
def try_except(func: Callable, default_return: Any = None, error_message: Optional[str] = None) -> Any:
    """
...
class ApiErrorResponse:
    ...
    def validation_error(errors: Dict) -> JsonResponse:
        ...
        return JsonResponse({"error": "Validation failed", "validation_errors": errors}, status=400)
