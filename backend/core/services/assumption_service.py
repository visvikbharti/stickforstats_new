...
class AssumptionCheckingService:
    ...
    def check_all_assumptions(
        ...
        for assumption in assumptions:
            if assumption == "normality":
                ...
                results["homogeneity_of_variance"] = self.check_homogeneity_of_variance(data, groups)
            elif assumption == "homoscedasticity":
                results["homoscedasticity"] = self.check_homoscedasticity(data, dependent_var, independent_vars)
                ...
                results["linearity"] = self.check_linearity(data, dependent_var, independent_vars)
            elif assumption == "no_multicollinearity":
                results["no_multicollinearity"] = self.check_multicollinearity(data, independent_vars)
    ...
    def check_normality(
        self, data: Union[pd.Series, np.ndarray, pd.DataFrame], variable: Optional[str] = None, method: str = "auto"
    ) -> AssumptionResult:
        """
        ...
        viz_data = {
            ...
            "theoretical_quantiles": stats.norm.ppf(np.linspace(0.01, 0.99, len(series))).tolist(),
            "histogram_bins": 30,
            "kde": True,
        }
    ...
    def check_homogeneity_of_variance(
        self, data: pd.DataFrame, groups: Union[str, List[str]], method: str = "levene"
    ) -> AssumptionResult:
        """
        ...
        if isinstance(groups, str):
            group_col = groups
            group_data = [group_df.dropna().values for _, group_df in data.groupby(group_col)]
        else:
            # Multiple grouping variables
            group_data = [group_df.dropna().values for _, group_df in data.groupby(groups)]
        ...
        alternatives = []
        if not is_met:
            alternatives.extend(["Welch's ANOVA", "Brown-Forsythe test", "Kruskal-Wallis test", "Robust ANOVA methods"])
    ...
    def check_independence(
        self, data: Union[pd.DataFrame, np.ndarray], method: str = "durbin_watson"
    ) -> AssumptionResult:
        """
        ...
        return AssumptionResult(
            ...
            p_value=p_value,
            sample_size=n,
            details={"test_name": test_name, "interpretation": "2.0 indicates no autocorrelation"},
            recommendation=recommendation,
            alternative_tests=alternatives,
            test_reference=test_ref,
        )

    def check_linearity(self, data: pd.DataFrame, dependent_var: str, independent_vars: List[str]) -> AssumptionResult:
        """
        ...
        for var in independent_vars:
            ...
            corr, p_value = stats.pearsonr(data[var].dropna(), data[dependent_var].dropna())

            # Calculate Spearman correlation for comparison
            spearman_corr, _ = stats.spearmanr(data[var].dropna(), data[dependent_var].dropna())
        ...
        transformations = []
        if not is_met:
            transformations.extend(["Polynomial terms", "Log transformation", "Spline functions", "Interaction terms"])
        ...
        return AssumptionResult(
            ...
            details={
                ...
                "linearity_per_var": {var: float(lin) for var, lin in zip(independent_vars, linearities)},
            },
            recommendation=recommendation,
            transformations=transformations,
        )

    def check_multicollinearity(self, data: pd.DataFrame, independent_vars: List[str]) -> AssumptionResult:
        """
    ...
    def check_normality_of_differences(
        self, data: pd.DataFrame, var1: str = None, var2: str = None
    ) -> AssumptionResult:
    ...
    def check_normality_of_residuals(
        self, data: pd.DataFrame, dependent_var: str, independent_vars: List[str]
    ) -> AssumptionResult:
    ...
    def check_homoscedasticity(
        ...
        lm_statistic = n * (
            1
            - np.sum((residuals_squared - predicted) ** 2) / np.sum((residuals_squared - residuals_squared.mean()) ** 2)
        )
    ...
    def check_expected_frequencies(
        self, contingency_table: pd.DataFrame, min_expected: float = 5.0
    ) -> AssumptionResult:
    ...
    def check_distribution_shape(self, data: pd.DataFrame, groups: str) -> AssumptionResult:
    ...
    def generate_assumption_report(self, results: Dict[str, AssumptionResult]) -> str:
        """
