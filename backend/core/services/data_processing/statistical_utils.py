...
class StatisticalUtilsService:
    ...
    def perform_inferential_tests(data: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        for col in numeric_cols:
            ...
            ks_test = try_except(
                lambda: stats.kstest((col_data - col_data.mean()) / col_data.std(), "norm"),
                {"statistic": None, "p_value": None},
            )
            ...
            results[col] = {
                "normality_tests": {
                    "shapiro": {
                        ...
                        if shapiro_test["statistic"] is not None
                        else None,
                        "p_value": float(shapiro_test["p_value"]) if shapiro_test["p_value"] is not None else None,
                        "is_normal": float(shapiro_test["p_value"]) > 0.05
                        if shapiro_test["p_value"] is not None
                        else None,
                    },
                    "kolmogorov_smirnov": {
    ...
    def analyze_distributions(data: pd.DataFrame, bins: str = "auto") -> Dict[str, Dict[str, Any]]:
        ...
        for col in numeric_cols:
            ...
            # Compute histogram
            hist_result = try_except(lambda: np.histogram(col_data, bins=bins), (np.array([]), np.array([])))

            # Compute KDE (Kernel Density Estimation)
            x_points = np.linspace(col_data.min(), col_data.max(), 100)
            kde_result = try_except(lambda: stats.gaussian_kde(col_data)(x_points), np.array([]))

            distribution_info[col] = {
                "histogram_data": {"counts": hist_result[0].tolist(), "bins": hist_result[1].tolist()},
                "kde_data": {"x": x_points.tolist(), "y": kde_result.tolist()},
            }
    ...
    def compute_correlation_matrix(data: pd.DataFrame, method: str = "pearson") -> Dict[str, Any]:
        ...
        if numeric_data.empty or numeric_data.shape[1] < 2:
            return {"error": "Not enough numerical columns for correlation analysis", "matrix": {}, "columns": []}

        corr_matrix = numeric_data.corr(method=method)

        # Convert to Python native types for JSON serialization
        result = {"method": method, "matrix": corr_matrix.to_dict(), "columns": numeric_data.columns.tolist()}
