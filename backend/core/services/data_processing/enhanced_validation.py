...
class EnhancedDataValidator:
    ...
    def add_rule(
        ...
        """
        self.rules.append({"check": check_func, "message": message, "level": level, "context": context or {}})
        return self
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        for rule in self.rules:
            try:
                ...
                if not check_result:
                    messages.append(
                        ValidationMessage(message=rule["message"], level=rule["level"], context=rule["context"])
                    )
                ...
                messages.append(
                    ValidationMessage(
                        message=f"Validation error: {str(e)}", level="error", context={"exception": str(e)}
                    )
                )
                is_valid = False
    ...
    def add_common_rules(self):
        ...
        self.add_rule(lambda df: df is not None and not df.empty, "Dataset is empty", "error")

        # Check for reasonable dimensions
        self.add_rule(lambda df: df.shape[0] > 1, "Dataset must have at least 2 rows", "error")

        # Check for columns
        self.add_rule(lambda df: df.shape[1] > 0, "Dataset must have at least 1 column", "error")
    ...
    def add_anova_rules(self):
        ...
        self.add_rule(
            lambda df: any(
                not is_numeric_dtype(df[col]) or (is_numeric_dtype(df[col]) and len(df[col].unique()) <= 10)
        ...
        def check_anova_assumptions(df):
            ...
            categorical_cols = [
                col
                for col in df.columns
                if not is_numeric_dtype(df[col]) or (is_numeric_dtype(df[col]) and len(df[col].unique()) <= 10)
            ]
        ...
        self.add_rule(check_anova_assumptions, "Some numeric columns might violate ANOVA assumptions", "warning")
    ...
    def add_regression_rules(self):
        ...
        self.add_rule(check_multicollinearity, "Potential multicollinearity detected among numeric columns", "warning")
    ...
    def add_time_series_rules(self):
        ...
        self.add_rule(has_datetime_column, "Time series analysis requires a datetime column", "error")
        ...
        self.add_rule(check_regular_intervals, "Some datetime columns have irregular time intervals", "warning")
    ...
    def add_clustering_rules(self):
        ...
        self.add_rule(check_column_ranges, "Some numeric columns have very different ranges", "warning")
    ...
    def add_pca_rules(self):
        ...
        self.add_rule(check_variability, "Some numeric columns have very low variance", "warning")
...
def validate_file_upload(file_data: BytesIO, file_name: str) -> ValidationResult:
    ...
    try:
        ...
        if file_ext == "csv":
            ...
            return ValidationResult(
                is_valid=False,
                messages=[ValidationMessage(message=f"Unsupported file format: {file_ext}", level="error")],
                data=None,
            )
        ...
        if data is None or data.empty:
            return ValidationResult(
                is_valid=False,
                messages=[ValidationMessage(message="Failed to load data or data is empty", level="error")],
                data=None,
            )
        ...
        return ValidationResult(
            ...
            messages=[
                ValidationMessage(
                    message=f"Error processing file: {str(e)}", level="error", context={"exception": str(e)}
                )
            ],
            data=None,
        )
