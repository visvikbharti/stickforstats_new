...
class DataValidator:
    ...
    def validate(self, data: pd.DataFrame) -> ValidationResult:
        ...
        return ValidationResult(is_valid=is_valid, messages=messages, data=data, metadata=metadata)
    ...
    def validate_file(self, file_obj: BytesIO, file_type: str = None) -> ValidationResult:
        ...
        try:
            ...
            if file_type is None:
                ...
                if filename.endswith(".csv"):
                    ...
                    return ValidationResult(
                        is_valid=False,
                        messages=[
                            ValidationMessage(message="Unable to determine file type. Please specify.", level="error")
                        ],
                        data=None,
                    )
            ...
            if file_type == "csv":
                ...
                return ValidationResult(
                    is_valid=False,
                    messages=[ValidationMessage(message=f"Unsupported file type: {file_type}", level="error")],
                    data=None,
                )
            ...
            return ValidationResult(
                ...
                messages=[
                    ValidationMessage(
                        message=f"Error loading file: {str(e)}", level="error", context={"exception": str(e)}
                    )
                ],
                data=None,
            )
    ...
    def add_standard_rules(self):
        ...
        self.add_rule(lambda df: len(df) > 0, "Dataset is empty", "error")
    ...
    def add_t_test_rules(self, paired: bool = False, equal_variance: bool = True):
        ...
        if paired:
            self.add_rule(lambda df: len(df.columns) >= 2, "Paired t-test requires at least two columns", "error")
            ...
            self.add_rule(
                lambda df: any("group" in col.lower() for col in df.columns)
                or df.select_dtypes(include=["object", "category"]).shape[1] > 0,
    ...
    def add_anova_rules(self):
        ...
        self.add_rule(
            ...
            and any(not pd.api.types.is_numeric_dtype(df[col]) for col in df.columns),
        ...
        self.add_rule(check_group_sizes, "ANOVA performs best with balanced group sizes", "warning")
    ...
    def add_normality_rules(self, columns: Optional[List[str]] = None):
        ...
        self.add_rule(check_normality, "Some columns may not be normally distributed", "warning")

        return self

    def add_preprocessing_recommendation(
        self, condition: Callable[[pd.DataFrame], bool], recommendation: str, context: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        self.preprocessing_recommendations.append(
            {"condition": condition, "recommendation": recommendation, "context": context or {}}
        )
        return self
        ...
        for rec in self.preprocessing_recommendations:
            try:
                if rec["condition"](data):
                    ...
                    recommendations.append({"recommendation": rec["recommendation"], "context": context})
            except Exception as e:
