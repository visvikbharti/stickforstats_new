...
class EffectSizeCalculator:
    ...
    @staticmethod
    def hedges_g(mean1: float, mean2: float, sd1: float, sd2: float, n1: int, n2: int) -> EffectSizeInfo:
        """
...
class PowerAnalyzer:
    ...
    def calculate_power(
        ...
        **kwargs,
        ...
        if test_type in [TestType.ONE_SAMPLE_T, TestType.TWO_SAMPLE_T, TestType.PAIRED_T]:
            return self._calculate_t_test_power(test_type, effect_size, sample_size, alpha, alternative, **kwargs)
        elif test_type in [TestType.ONE_WAY_ANOVA, TestType.FACTORIAL_ANOVA]:
            return self._calculate_anova_power(test_type, effect_size, sample_size, alpha, **kwargs)
        elif test_type in [TestType.PEARSON_CORRELATION, TestType.SPEARMAN_CORRELATION]:
            return self._calculate_correlation_power(test_type, effect_size, sample_size, alpha, alternative, **kwargs)
        elif test_type in [TestType.LINEAR_REGRESSION, TestType.MULTIPLE_REGRESSION]:
            return self._calculate_regression_power(test_type, effect_size, sample_size, alpha, **kwargs)
        elif test_type == TestType.CHI_SQUARE_INDEPENDENCE:
            return self._calculate_chi_square_power(effect_size, sample_size, alpha, **kwargs)
        else:
    ...
    def calculate_sample_size(
        ...
        **kwargs,
        ...
        if test_type in [TestType.ONE_SAMPLE_T, TestType.TWO_SAMPLE_T, TestType.PAIRED_T]:
            return self._calculate_t_test_sample_size(test_type, effect_size, power, alpha, alternative, **kwargs)
        elif test_type == TestType.ONE_WAY_ANOVA:
            return self._calculate_anova_sample_size(effect_size, power, alpha, **kwargs)
        elif test_type in [TestType.PEARSON_CORRELATION]:
            return self._calculate_correlation_sample_size(effect_size, power, alpha, alternative)
        else:
    ...
    def _calculate_t_test_power(
        ...
        if STATSMODELS_AVAILABLE:
            ...
            power = self._manual_t_test_power(effect_size, sample_size, alpha, alternative, test_type)
    ...
    def _calculate_t_test_sample_size(
        self, test_type: TestType, effect_size: float, power: float, alpha: float, alternative: str, ratio: float = 1.0
    ) -> PowerAnalysisResult:
        """Calculate sample size for t-tests"""

        if STATSMODELS_AVAILABLE:
            if test_type == TestType.ONE_SAMPLE_T:
                n = tt_solve_power(effect_size=effect_size, power=power, alpha=alpha, alternative=alternative)
            elif test_type == TestType.TWO_SAMPLE_T:
                n = tt_ind_solve_power(
                    effect_size=effect_size, power=power, alpha=alpha, ratio=ratio, alternative=alternative
                )
            ...
            # Manual calculation
            n = self._manual_t_test_sample_size(effect_size, power, alpha, alternative, test_type, ratio)
        ...
        achieved_result = self.calculate_power(test_type, effect_size, sample_size, alpha, alternative, ratio=ratio)
    ...
    def _calculate_anova_sample_size(
        self, effect_size: float, power: float, alpha: float, n_groups: int = 3, **kwargs  # Cohen's f
    ) -> PowerAnalysisResult:
        """Calculate sample size for ANOVA"""

        # Use iterative search to find sample size
        def power_diff(n):
            result = self._calculate_anova_power(TestType.ONE_WAY_ANOVA, effect_size, int(n), alpha, n_groups)
        ...
        achieved_result = self._calculate_anova_power(TestType.ONE_WAY_ANOVA, effect_size, sample_size, alpha, n_groups)
    ...
    def _calculate_correlation_power(
        ...
        **kwargs,
        ...
        # Power calculation
        if alternative == "two-sided":
            power = stats.norm.cdf(-z_crit + abs(z_r) / se) + stats.norm.cdf(-z_crit - abs(z_r) / se)
    ...
    def _calculate_correlation_sample_size(
        self, effect_size: float, power: float, alpha: float, alternative: str
    ) -> PowerAnalysisResult:
        ...
        achieved_result = self._calculate_correlation_power(
            TestType.PEARSON_CORRELATION, effect_size, n, alpha, alternative
        )
    ...
    def _calculate_regression_power(
        ...
        **kwargs,
        ...
        if df_error < 10 * n_predictors:
            result.warnings.append(f"Sample size may be too small. Recommend n > {10 * (n_predictors + 1)}")

        return result

    def _calculate_chi_square_power(
        self, effect_size: float, sample_size: int, alpha: float, df: Optional[int] = None, **kwargs  # Cohen's w
    ) -> PowerAnalysisResult:
    ...
    def _manual_t_test_sample_size(
        self, effect_size: float, power: float, alpha: float, alternative: str, test_type: TestType, ratio: float = 1.0
    ) -> float:
        ...
        def power_diff(n):
            ...
            calculated_power = self._manual_t_test_power(effect_size, sample_size, alpha, alternative, test_type)
        ...
        **kwargs,
        ...
        if effect_sizes is not None and sample_sizes is None:
            ...
            for es in effect_sizes:
                ...
                results.append({"effect_size": es, "sample_size": n, "power": result.power, "alpha": alpha})
            ...
            for n in sample_sizes:
                ...
                results.append({"effect_size": es, "sample_size": int(n), "power": result.power, "alpha": alpha})
        ...
        for param, (min_val, max_val) in vary_ranges.items():
            ...
            for val in values:
                if param == "effect_size":
                    result = self.calculate_power(test_type, val, base_sample_size, alpha, **kwargs)
                    results.append(
                        {
                    ...
                    result = self.calculate_power(test_type, base_effect_size, int(val), alpha, **kwargs)
                    results.append(
                        {
                    ...
                    result = self.calculate_power(test_type, base_effect_size, base_sample_size, val, **kwargs)
                    results.append(
                        {
                            "parameter": "alpha",
                            "value": val,
    ...
    def compare_tests(
        self, test_types: List[TestType], effect_size: float, sample_size: int, alpha: Optional[float] = None, **kwargs
    ) -> pd.DataFrame:
        """
        ...
        for test_type in test_types:
            try:
                result = self.calculate_power(test_type, effect_size, sample_size, alpha, **kwargs)
                results.append(
                    {
...
def optimal_allocation(
    total_n: int, group_costs: Optional[List[float]] = None, group_variances: Optional[List[float]] = None
) -> Dict[str, int]:
    """
