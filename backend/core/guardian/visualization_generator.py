...
class VisualizationGenerator:
    ...
    def generate_all_diagnostics(
        self, data: np.ndarray, violations: List[Dict[str, Any]], test_type: str
    ) -> Dict[str, str]:
        """
    ...
    def generate_qq_plot(self, data: np.ndarray) -> str:
        ...
        ax.plot(theoretical_quantiles, theoretical_line, "r--", linewidth=2, label="Normal distribution", alpha=0.8)
        ...
        ax.fill_between(
            theoretical_quantiles, lower_band, upper_band, alpha=0.1, color="red", label="95% confidence envelope"
        )
    ...
    def generate_histogram(self, data: np.ndarray) -> str:
        ...
        ax.plot(x_range, normal_curve, "r--", linewidth=2, label=f"Normal(μ={mu:.2f}, σ={sigma:.2f})", alpha=0.8)

        # Add vertical lines for mean and median
        mean_val = np.mean(data_clean)
        median_val = np.median(data_clean)
        ax.axvline(mean_val, color="green", linestyle="--", linewidth=1.5, label=f"Mean = {mean_val:.2f}", alpha=0.7)
        ax.axvline(
            median_val, color="orange", linestyle="--", linewidth=1.5, label=f"Median = {median_val:.2f}", alpha=0.7
        )

        ax.set_xlabel("Value", fontsize=10)
        ax.set_ylabel("Density", fontsize=10)
        ax.set_title("Distribution Analysis: Histogram with Normal Overlay", fontsize=12, fontweight="bold")
    ...
    def generate_boxplot(self, data: np.ndarray) -> str:
        ...
        _ = ax.boxplot(
            ...
            flierprops=dict(marker="o", markerfacecolor="red", markersize=6, alpha=0.6, markeredgecolor="darkred"),
        )

        # Add strip plot for individual points (jittered)
        y_values = data_clean
        x_values = np.random.normal(1, 0.04, size=len(y_values))
        ax.scatter(x_values, y_values, alpha=0.3, s=20, color="black", label="Individual data points")
    ...
    def generate_group_comparison(self, data: List[np.ndarray]) -> str:
        ...
        parts = ax2.violinplot(
            cleaned_groups, positions=range(1, len(cleaned_groups) + 1), showmeans=True, showmedians=True
        )
        ...
        fig.text(0.5, 0.02, summary, ha="center", fontsize=8, bbox=dict(boxstyle="round", facecolor="wheat", alpha=0.3))
    ...
    def _fig_to_base64(self, fig) -> str:
        ...
        buffer = io.BytesIO()
        fig.savefig(buffer, format="png", dpi=self.dpi, bbox_inches="tight", facecolor="white", edgecolor="none")
        buffer.seek(0)
