...
from .guardian_core import GuardianReport
...
class GuardianReportGenerator:
    ...
    def _build_header(self, report: GuardianReport) -> List:
        ...
        # Title
        title = Paragraph("Guardian Validation Report", self.styles["CustomTitle"])
        ...
        subtitle = Paragraph(
            f"<font size=10>Generated: {timestamp}</font><br/>" f"<font size=10>StickForStats Platform v1.0</font>",
        ...
        status_para = Paragraph(
            f"<font color='{status_color}' size=12><b>{status_text}</b></font>", self.styles["GuardianBodyText"]
        )
    ...
    def _build_assumption_checks(self, report: GuardianReport) -> List:
        ...
        for assumption in report.assumptions_checked:
            ...
            results_data.append([assumption.replace("_", " ").title(), status, stat, pval, severity])
        ...
        for plot_key, plot_title in plot_titles.items():
            if plot_key in report.visual_evidence and report.visual_evidence[plot_key]:
                try:
                    ...
                    # Add title
                    elements.append(Paragraph(f"<b>{plot_title}</b>", self.styles["SubsectionHeader"]))
    ...
    def _build_effect_sizes(self, report: GuardianReport) -> List:
        ...
        if effect_report.get("recommendations"):
            ...
            for recommendation in effect_report["recommendations"]:
                elements.append(Paragraph(f"• {recommendation}", self.styles["GuardianBodyText"]))
            elements.append(Spacer(1, 0.2 * inch))
    ...
    def _build_violations_detail(self, report: GuardianReport) -> List:
        ...
        for i, violation in enumerate(report.violations, 1):
            # Violation header
            severity_color = {"critical": "#d32f2f", "warning": "#f57c00", "minor": "#fbc02d"}.get(
                violation.severity, "#666666"
            )
    ...
    def _build_recommendations(self, report: GuardianReport) -> List:
        ...
        if report.alternative_tests:
            elements.append(
                Paragraph("Recommended Alternative Tests with Step-by-Step Guidance", self.styles["SubsectionHeader"])
            )
            ...
            for alt in report.alternative_tests:
                if alt in test_guidance:
                    ...
                    alt_text = (
                        ...
                        "<b>Why Recommended:</b><br/>" + guidance["why"] + "<br/><br/>"
                        "<b>How to Run in StickForStats:</b><br/>" + guidance["how"] + "<br/><br/>"
    ...
    def _build_footer(self) -> List:
        ...
        """.format(
            report_id=self._generate_report_id(), timestamp=datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        )
