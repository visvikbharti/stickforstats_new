...
class DataFingerprinter:
    ...
    def fingerprint_dataframe(
        self, df: pd.DataFrame, name: Optional[str] = None, include_index: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def fingerprint_array(self, arr: np.ndarray, name: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def fingerprint_dict(self, data_dict: Dict[str, Any], name: Optional[str] = None) -> Dict[str, Any]:
        """
    ...
    def verify_dataframe(
        self, df: pd.DataFrame, fingerprint: Dict[str, Any], strict: bool = True
    ) -> Tuple[bool, List[str]]:
        """
        ...
        if list(df.columns) != fingerprint["columns"]:
            differences.append("Columns mismatch")
        ...
        # Check hash
        current_fingerprint = self.fingerprint_dataframe(df, include_index=fingerprint.get("include_index", False))
    ...
    def verify_array(self, arr: np.ndarray, fingerprint: Dict[str, Any]) -> Tuple[bool, List[str]]:
        """
    ...
    def compare_fingerprints(self, fp1: Dict[str, Any], fp2: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "metadata" in fp1 and "metadata" in fp2:
            ...
            if meta1.get("n_rows") != meta2.get("n_rows"):
                comparison["differences"].append(f"Row count: {meta1.get('n_rows')} vs {meta2.get('n_rows')}")
