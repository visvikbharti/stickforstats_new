...
class MethodsGenerator:
    ...
    def _generate_reproducibility_statement(self, bundle: Any) -> None:
        ...
        if bundle.checksum:
            text += "Data integrity was verified using SHA-256 checksums. "
    ...
    def _generate_references(self) -> str:
        ...
        sorted_citations = sorted(unique_citations.values(), key=lambda c: (c.authors.split(",")[0], c.year))
    ...
    def generate_limitations(self, bundle: Any) -> List[str]:
        ...
        for fingerprint in bundle.data_fingerprints.values():
            if fingerprint.missing_count > 0:
                ...
                if missing_pct > 5:
                    limitations.append(
                        f"Missing data ({missing_pct:.1f}%) may introduce bias " f"if not missing completely at random."
                    )
                break
