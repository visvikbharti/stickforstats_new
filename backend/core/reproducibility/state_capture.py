...
class StateCapture:
    ...
    def capture_test_recommender(self, recommender) -> Dict[str, Any]:
        ...
        if hasattr(recommender, "recommendation_history"):
            state["recommendations"] = self._serialize_recommendations(recommender.recommendation_history)

        # Capture data characteristics
        if hasattr(recommender, "data_characteristics"):
            state["data_characteristics"] = self._make_serializable(recommender.data_characteristics)

        # Capture assumption check results
        if hasattr(recommender, "assumption_results"):
            state["assumption_checks"] = self._serialize_assumption_checks(recommender.assumption_results)

        # Capture decision tree state
        if hasattr(recommender, "decision_tree"):
            state["decision_tree"] = self._serialize_decision_tree(recommender.decision_tree)
    ...
    def capture_hypothesis_registry(self, registry) -> Dict[str, Any]:
        ...
        if hasattr(registry, "hypotheses"):
            state["registered_hypotheses"] = [self._serialize_hypothesis(h) for h in registry.hypotheses]

        # Capture test results
        if hasattr(registry, "test_results"):
            state["test_results"] = [self._serialize_test_result(r) for r in registry.test_results]
    ...
    def capture_power_analysis(self, power_analyzer) -> Dict[str, Any]:
        ...
        if hasattr(power_analyzer, "calculation_history"):
            state["calculations"] = [
                self._serialize_power_calculation(calc) for calc in power_analyzer.calculation_history
            ]

        # Capture sample size determinations
        if hasattr(power_analyzer, "sample_size_history"):
            state["sample_size_determinations"] = [
                self._serialize_sample_size(ss) for ss in power_analyzer.sample_size_history
            ]

        # Capture power curves
        if hasattr(power_analyzer, "power_curves"):
            state["power_curves"] = self._serialize_power_curves(power_analyzer.power_curves)
    ...
    def capture_effect_sizes(self, effect_calculator) -> Dict[str, Any]:
        ...
        if hasattr(effect_calculator, "effect_history"):
            state["calculated_effects"] = [
                self._serialize_effect_size(effect) for effect in effect_calculator.effect_history
            ]

        # Capture confidence intervals
        if hasattr(effect_calculator, "ci_history"):
            state["confidence_intervals"] = self._serialize_confidence_intervals(effect_calculator.ci_history)
    ...
    def capture_robust_estimators(self, robust_estimator) -> Dict[str, Any]:
        ...
        if hasattr(robust_estimator, "estimation_history"):
            state["estimations"] = [
                self._serialize_robust_estimation(est) for est in robust_estimator.estimation_history
            ]

        # Capture outlier detections
        if hasattr(robust_estimator, "outlier_history"):
            state["outlier_detections"] = self._serialize_outlier_detections(robust_estimator.outlier_history)

        # Capture bootstrap results
        if hasattr(robust_estimator, "bootstrap_history"):
            state["bootstrap_results"] = [
                self._serialize_bootstrap_result(boot) for boot in robust_estimator.bootstrap_history
            ]
    ...
    def capture_all_modules(self, modules: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        complete_state = {"capture_timestamp": datetime.now().isoformat(), "module_states": {}}
    ...
    def restore_module_state(self, module_name: str, module_instance: Any, state: Dict[str, Any]) -> bool:
        """
    ...
    def _make_serializable(self, obj: Any, max_depth: int = 5, depth: int = 0) -> Any:
        ...
        if obj is None or isinstance(obj, (bool, int, float, str)):
            ...
            return {key: self._make_serializable(value, max_depth, depth + 1) for key, value in obj.items()}
