...
class DistributionConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.channel_layer.group_add(self.distribution_group_name, self.channel_name)
    ...
    async def disconnect(self, close_code):
        ...
        await self.channel_layer.group_discard(self.distribution_group_name, self.channel_name)
    ...
    async def update_parameters(self, data):
        ...
        if distribution:
            ...
            await self.channel_layer.group_send(
                self.distribution_group_name, {"type": "distribution_parameters_updated", "parameters": parameters}
            )
    ...
    async def calculate_pmf_pdf(self, data):
        ...
        if distribution:
            ...
            await self.send(text_data=json.dumps({"type": "pmf_pdf_calculated", "result": result}))
    ...
    async def calculate_cdf(self, data):
        ...
        if distribution:
            ...
            # Send result directly to the client
            await self.send(text_data=json.dumps({"type": "cdf_calculated", "result": result}))
    ...
    async def calculate_probability(self, data):
        ...
        if distribution:
            ...
            # Send result directly to the client
            await self.send(text_data=json.dumps({"type": "probability_calculated", "result": result}))
    ...
    async def generate_random_sample(self, data):
        ...
        if distribution:
            ...
            # Send result directly to the client
            await self.send(text_data=json.dumps({"type": "random_sample_generated", "result": result}))
    ...
    async def distribution_parameters_updated(self, event):
        ...
        await self.send(text_data=json.dumps({"type": "parameters_updated", "parameters": parameters}))
    ...
    def get_distribution(self):
        ...
        try:
            return Distribution.objects.select_related("project").get(id=self.distribution_id, project__user=user)
    ...
    def perform_pmf_pdf_calculation(self, distribution, x_values):
        ...
        """
        return DistributionService.calculate_pmf_pdf(distribution.distribution_type, distribution.parameters, x_values)
    ...
    def perform_cdf_calculation(self, distribution, x_values):
        ...
        """
        return DistributionService.calculate_cdf(distribution.distribution_type, distribution.parameters, x_values)
    ...
    def perform_sample_generation(self, distribution, sample_size, seed):
        ...
        return DistributionService.generate_random_sample(
            distribution.distribution_type, distribution.parameters, sample_size, seed
        )
