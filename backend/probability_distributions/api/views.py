...
from .serializers import (
    DistributionProjectSerializer,
    DistributionSerializer,
    ProbabilityCalculationSerializer,
    RandomSampleSerializer,
    BinomialApproximationRequestSerializer,
    DataFittingRequestSerializer,
    ProcessCapabilityRequestSerializer,
    ClinicalTrialSimulationSerializer,
)
...
class DistributionProjectViewSet(viewsets.ModelViewSet):
    ...
    def duplicate(self, request, pk=None):
        ...
        new_project = DistributionProject.objects.create(
            user=request.user, name=f"Copy of {original_project.name}", description=original_project.description
        )
        ...
        for dist in original_project.distributions.all():
            ...
            for viz in dist.visualizations.all():
                DistributionVisualization.objects.create(
                    distribution=new_dist, visualization_type=viz.visualization_type, settings=viz.settings
                )
...
class DistributionViewSet(viewsets.ModelViewSet):
    ...
    def calculate_pmf_pdf(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            result = DistributionService.calculate_pmf_pdf(
                distribution.distribution_type, distribution.parameters, x_values
            )
    ...
    def calculate_probability(self, request, pk=None):
        ...
        if serializer.is_valid():
            ...
            x_value = (
                serializer.validated_data.get("x_values", [0])[0] if serializer.validated_data.get("x_values") else None
            )
...
class DistributionUtilityViewSet(viewsets.ViewSet):
    ...
    def clinical_trial_simulation(self, request):
        ...
        if serializer.is_valid():
            ...
            result = DistributionService.simulate_clinical_trial(
                treatment_effect, control_mean, control_sd, treatment_sd, n_control, n_treatment, n_simulations, seed
            )
    ...
    def poisson_process(self, request):
        ...
        if not rate or not time_period:
            return Response(
                {"error": "Rate and time_period are required parameters"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if request.data.get("save", False):
            ...
            if project_id:
                ...
                ApplicationSimulation.objects.create(
                    project=project,
                    name="Poisson Process Simulation",
                    application_type="NETWORK_TRAFFIC",
                    parameters={"rate": rate, "time_period": time_period, "num_simulations": num_simulations},
                    results=result,
                )
