...
class CreateDistributionView(APIView):
    ...
    def post(self, request):
        ...
        if not distribution_type:
            return Response(
                {'error': 'distribution_type is required'},
    ...
    def post(self, request):
        ...
        try:
            result = DistributionService.calculate_probability(
                distribution_type, parameters, probability_type,
    ...
    def post(self, request):
        ...
        try:
            result = DistributionService.generate_random_sample(
                distribution_type, parameters, sample_size, seed
            )
            return Response(result)
        except (ValueError, TypeError) as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class CompareApproximationsView(APIView):
    ...
    def post(self, request):
        ...
        try:
            result = DistributionService.compare_binomial_approximations(
                n, p, approximation_types
            )
            return Response(result)
        except (ValueError, TypeError) as e:
            return Response({'error': str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class ProcessCapabilityView(APIView):
    ...
    def post(self, request):
        ...
        if not data or lsl is None or usl is None:
            return Response(
                {'error': 'data, lsl, and usl are required'},
                status=status.HTTP_400_BAD_REQUEST
            )

        try:
            result = DistributionService.calculate_process_capability(
                data, lsl, usl, target
            )
            return Response(result)
        except (ValueError, TypeError) as e:
...
class SimulatePoissonProcessView(APIView):
    ...
    def post(self, request):
        ...
        if rate is None or time_period is None:
            return Response(
                {'error': 'rate and time_period are required'},
                status=status.HTTP_400_BAD_REQUEST
            )
