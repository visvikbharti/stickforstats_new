...
class PCAAnalysisConsumer(AsyncWebsocketConsumer):
    ...
    async def pca_status(self, event):
        ...
        await self.send(text_data=json.dumps({"type": "pca_status", "data": event["message"]}))

    async def pca_progress(self, event):
        """Handle PCA progress updates."""
        # Send message to WebSocket
        await self.send(text_data=json.dumps({"type": "pca_progress", "data": event["message"]}))

    async def new_visualization(self, event):
        """Handle new visualization creation updates."""
        # Send message to WebSocket
        await self.send(text_data=json.dumps({"type": "new_visualization", "data": event["message"]}))
