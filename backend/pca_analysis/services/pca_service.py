...
class PCAService:
    ...
    def prepare_data_matrix(project_id):
        ...
        data_matrix = pd.DataFrame(index=sample_ids, columns=gene_ids, dtype=float)

        # Fill the matrix with expression values
        expressions = ExpressionValue.objects.filter(sample__project_id=project_id).select_related("sample", "gene")
        ...
        try:
            ...
            if data_matrix.isnull().values.any():
                ...
                data_matrix_imputed = pd.DataFrame(
                    imputer.fit_transform(data_matrix), index=data_matrix.index, columns=data_matrix.columns
                )
            else:
            ...
            if project.scaling_method == "STANDARD":
                ...
                data_matrix_scaled = (data_matrix_imputed - data_matrix_imputed.min()) / (
                    data_matrix_imputed.max() - data_matrix_imputed.min()
                )
            else:  # 'NONE'
            ...
            pca_df = pd.DataFrame(
                data=pca_result, columns=[f"PC{i+1}" for i in range(pca.n_components_)], index=data_matrix_scaled.index
            )
            ...
            loadings_df = pd.DataFrame(
                loadings, columns=[f"PC{i+1}" for i in range(pca.n_components_)], index=data_matrix_scaled.columns
            )

            # Calculate gene contributions - Matching lines 524-531 in original code
            gene_contribution = pd.DataFrame(
                index=loadings_df.index, columns=["PC1_contribution", "PC2_contribution", "Total_contribution"]
            )
            ...
            if loadings_df.shape[1] >= 2:
                ...
                gene_contribution["Total_contribution"] = (
                    gene_contribution["PC1_contribution"] + gene_contribution["PC2_contribution"]
                )
    ...
    def generate_visualization_data(
        ...
        try:
            ...
            if include_gene_loadings and pca_result.loadings:
                ...
                gene_contributions = (
                    GeneContribution.objects.filter(pca_result=pca_result)
                    .select_related("gene")
                    .order_by("-total_contribution")[:top_genes_count]
                )
