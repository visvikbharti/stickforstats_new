...
from pca_analysis.models import PCAProject, Sample, SampleGroup, Gene, ExpressionValue
...
class DataProcessorService:
    ...
    def import_data(project_id, df):
        ...
        for group_name in unique_groups:
            group, _ = SampleGroup.objects.get_or_create(project=project, name=group_name)
        ...
        for column in df.columns:
            ...
            sample = Sample.objects.create(
                project=project, name=column, group=group_objects[group_name], replicate_id=replicate_id
            )
        ...
        for gene_name in df.index:
            gene = Gene.objects.create(project=project, name=gene_name)
            ...
            for column, sample in sample_objects.items():
                ...
                if not pd.isna(df.loc[gene_name, column]):
                    expression_values.append(
                        ExpressionValue(
                            project=project, sample=sample, gene=gene, value=float(df.loc[gene_name, column])
                        )
                    )
    ...
    def create_demo_data(project):
        ...
        for group_name in group_names:
            ...
            for rep in range(1, n_samples_per_group + 1):
                sample = Sample.objects.create(
                    project=project, name=f"{group_name}_R{rep}", group=group, replicate_id=f"R{rep}"
                )
        ...
        for sample, group_name in sample_objects:
            ...
            for gene, val in zip(gene_objects, base):
                expression_values.append(ExpressionValue(project=project, sample=sample, gene=gene, value=float(val)))
