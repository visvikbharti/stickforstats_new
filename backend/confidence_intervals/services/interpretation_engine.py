...
class InterpretationEngine:
    ...
    def __init__(self):
        ...
        self.effect_size_thresholds = {"small": 0.2, "medium": 0.5, "large": 0.8, "very_large": 1.2}

    def interpret_mean_ci(
        self,
        mean: float,
        ci_lower: float,
        ci_upper: float,
        ...
        # APA format
        interpretations["apa_format"] = f"M = {mean:.2f}, {confidence_pct}% CI [{ci_lower:.2f}, {ci_upper:.2f}]"
    ...
    def interpret_proportion_ci(
        self, proportion: float, ci_lower: float, ci_upper: float, confidence_level: float, successes: int, trials: int
    ) -> Dict[str, str]:
        """
        ...
        # APA format
        interpretations["apa_format"] = f"p = {proportion:.3f}, {confidence_pct}% CI [{ci_lower:.3f}, {ci_upper:.3f}]"
    ...
    def interpret_bootstrap_ci(
        self, estimate: float, ci_lower: float, ci_upper: float, confidence_level: float, n_iterations: int
    ) -> Dict[str, str]:
        """
    ...
    def interpret_comparison_ci(
        ...
        if ci_lower > 0 or ci_upper < 0:
            interpretations["recommendation"] = "There is a statistically significant difference between the groups. "
            if cohens_d:
