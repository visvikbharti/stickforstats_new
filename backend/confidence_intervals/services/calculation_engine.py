...
class ConfidenceIntervalCalculator:
    ...
    def calculate_mean_ci(
        self, data: Union[List[float], np.ndarray, pd.Series], confidence_level: float = 0.95, method: str = "t"
    ) -> Dict:
        """
        ...
        if not (self.min_confidence_level <= confidence_level <= self.max_confidence_level):
            raise ValueError(
                f"Confidence level must be between {self.min_confidence_level} and {self.max_confidence_level}"
            )
    ...
    def calculate_proportion_ci(
        self, successes: int, trials: int, confidence_level: float = 0.95, method: str = "wilson"
    ) -> Dict:
        """
