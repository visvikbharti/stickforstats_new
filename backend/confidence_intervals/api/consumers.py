    ...
    async def receive(self, text_data):
        ...
        if action == "start_bootstrap_simulation":
            ...
            await self.channel_layer.group_send(
                self.group_name, {"type": "simulation_cancelled", "message": "Simulation was cancelled"}
            )
    ...
    def run_simulation_chunk(
        ...
        for _ in range(chunk_size):
            ...
            result = BootstrapService.bootstrap_ci(
                data=sample, confidence_level=confidence_level, n_resamples=n_bootstrap, method=method
            )
