...
class ConfidenceIntervalCalculationViewSet(viewsets.ViewSet):
    ...
    def calculate_mean(self, request):
        ...
        if not data:
            return Response({"error": "Data array is required"}, status=status.HTTP_400_BAD_REQUEST)

        # Perform calculation
        result = self.calculator.calculate_mean_ci(data=data, confidence_level=confidence_level, method=method)
        ...
        logging_service.log_calculation(
            ...
            parameters={"n": len(data), "confidence_level": confidence_level, "method": method},
            result=result,
        )
    ...
    def calculate_proportion(self, request):
        ...
        if successes is None or trials is None:
            return Response({"error": "Both 'successes' and 'trials' are required"}, status=status.HTTP_400_BAD_REQUEST)

        # Perform calculation
        result = self.calculator.calculate_proportion_ci(
            successes=successes, trials=trials, confidence_level=confidence_level, method=method
        )
    ...
    def compare_groups(self, request):
        ...
        if not group1 or not group2:
            return Response({"error": "Both group1 and group2 are required"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def validate_data(self, request):
        ...
        validation_result = {"valid": True, "warnings": [], "errors": [], "statistics": {}}
