...
class IntervalDataSerializer(serializers.ModelSerializer):
    class Meta:
        model = IntervalData
        fields = ["id", "project", "name", "data_type", "data", "description", "created_at", "updated_at"]
...
class IntervalResultSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "project", "interval_type", "data_source", "parameters", "results", "created_at", "updated_at"]
        read_only_fields = ["id", "created_at", "updated_at"]
...
class EducationalResourceSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "title", "content_type", "content", "category", "order", "created_at", "updated_at"]
        read_only_fields = ["id", "created_at", "updated_at"]
...
class CalculateIntervalSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if interval_type == "MEAN_Z":
            if not (
                ("numeric_data" in data)
                or ("sample_mean" in data and "population_std" in data and "sample_size" in data)
            ):
            ...
            if not (
                ("numeric_data" in data) or ("sample_mean" in data and "sample_std" in data and "sample_size" in data)
            ):
                raise serializers.ValidationError(
            ...
            if not (
                (("data_id" in data) and ("data_id_2" in data))
                or (("numeric_data" in data) and ("numeric_data_2" in data))
                or (
                    ("sample_mean" in data)
                    and ("sample_mean_2" in data)
                    and ("sample_std" in data)
                    and ("sample_std_2" in data)
                    and ("sample_size" in data)
                    and ("sample_size_2" in data)
                )
            ):
                raise serializers.ValidationError(
