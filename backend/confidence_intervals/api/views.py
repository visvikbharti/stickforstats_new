...
class IntervalDataViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        project = get_object_or_404(
            ConfidenceIntervalProject, id=self.request.data.get("project"), user=self.request.user
        )
...
class EducationalResourceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.basename == "ci-case-studies":
            return queryset.filter(content_type="EXAMPLE", category="APPLICATIONS").order_by("order", "title")
...
class ConfidenceIntervalCalculationViewSet(viewsets.ViewSet):
    ...
    def calculate(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Project not found or you don't have permission"}, status=status.HTTP_404_NOT_FOUND
            )
        ...
        if "data_id" in data and data.get("data_id"):
            try:
                ...
                return Response(
                    {"error": "Data not found or you don't have permission"}, status=status.HTTP_404_NOT_FOUND
                )
        ...
        try:
            ...
            if interval_type == "MEAN_Z":
                ...
                if numeric_data is not None:
                    result = IntervalService.mean_t_interval(data=numeric_data, confidence_level=confidence_level)
                else:
                ...
                result = IntervalService.proportion_wald_interval(
                    successes=successes, sample_size=sample_size, confidence_level=confidence_level
                )
                ...
                if numeric_data is not None:
                    result = IntervalService.variance_interval(data=numeric_data, confidence_level=confidence_level)
                else:
                    return Response(
                        {"error": "Variance interval requires raw data"}, status=status.HTTP_400_BAD_REQUEST
                    )
            ...
            # Difference intervals
                ...
                if "data_id_2" in data and data.get("data_id_2"):
                    try:
                        interval_data_2 = IntervalData.objects.get(id=data.get("data_id_2"), project__user=request.user)
                ...
                if "data_id" in data and "data_id_2" in data:
                    try:
                        interval_data_1 = IntervalData.objects.get(id=data.get("data_id"), project__user=request.user)
                        interval_data_2 = IntervalData.objects.get(id=data.get("data_id_2"), project__user=request.user)
                ...
                result = IntervalService.difference_proportions_interval(
                    x1=x1, n1=n1, x2=x2, n2=n2, confidence_level=confidence_level, method=data.get("method", "wald")
                )
            ...
            # Bootstrap intervals
                ...
                if interval_type == "BOOTSTRAP_SINGLE":
                    if numeric_data is None:
                        return Response({"error": "Bootstrap requires raw data"}, status=status.HTTP_400_BAD_REQUEST)
                    ...
                    if "data_id_2" in data:
                        try:
                            interval_data_2 = IntervalData.objects.get(
                                id=data.get("data_id_2"), project__user=request.user
                            )
                            # Extract numeric data from the data field
                            if "values" in interval_data_2.data:
                                numeric_data_2 = np.array(interval_data_2.data["values"])
                            ...
                            return Response({"error": "Second data set not found"}, status=status.HTTP_404_NOT_FOUND)
                    ...
                    if numeric_data is None or numeric_data_2 is None:
                        return Response(
                            {"error": "Bootstrap difference requires two data sets"}, status=status.HTTP_400_BAD_REQUEST
                        )
                ...
                return Response(
                    {"error": f"Unknown interval type: {interval_type}"}, status=status.HTTP_400_BAD_REQUEST
                )
...
class CoverageSimulationViewSet(viewsets.ViewSet):
    ...
    def simulate(self, request):
        ...
        try:
            ...
            true_param = (
                dist_params.get("mean", 0)
                if distribution.upper() == "NORMAL"
                else dist_params.get("p", 0.5)
            ...
            for i in range(n_simulations):
                ...
                if interval_type == "MEAN_Z":
                    ...
                    result = IntervalService.mean_z_interval(
                        data=sample, confidence_level=confidence_level, sigma=pop_std
                    )
                elif interval_type == "MEAN_T":
                    result = IntervalService.mean_t_interval(data=sample, confidence_level=confidence_level)
    ...
    def calculate_pmf_pdf(self, request):
        ...
        try:
            ...
            if distribution == "NORMAL":
                ...
                return Response(
                    {"error": f"Unsupported distribution: {distribution}"}, status=status.HTTP_400_BAD_REQUEST
                )
