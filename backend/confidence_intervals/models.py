...
class ConfidenceIntervalProject(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="confidence_interval_projects"
    )
...
class IntervalData(models.Model):
    ...
    data_type = models.CharField(
        max_length=50,
        choices=[("NORMAL", "Normal"), ("BINOMIAL", "Binomial"), ("POISSON", "Poisson"), ("CUSTOM", "Custom")],
    )
...
class IntervalResult(models.Model):
    ...
    data_source = models.ForeignKey(
        IntervalData, on_delete=models.CASCADE, related_name="interval_results", null=True, blank=True
    )  # Optional if derived from theoretical parameters
    name = models.CharField(max_length=255)
    description = models.TextField(blank=True)
