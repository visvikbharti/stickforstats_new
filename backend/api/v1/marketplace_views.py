...
class MarketplaceListView(APIView):
    ...
    def get(self, request):
        ...
        if org:
            ...
            installed_ids = set(
                PluginInstallation.objects.filter(organization=org, is_active=True).values_list("plugin_id", flat=True)
            )
...
class PluginDetailView(APIView):
    ...
    def get(self, request, plugin_slug):
        ...
        if org:
            ...
            is_installed = PluginInstallation.objects.filter(plugin=p, organization=org, is_active=True).exists()
...
class InstalledPluginsView(APIView):
    ...
    def get(self, request):
        ...
        installations = PluginInstallation.objects.filter(organization=org, is_active=True).select_related(
            "plugin", "installed_by"
        )
