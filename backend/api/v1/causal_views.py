...
def dag_create(request):
    ...
    try:
        ...
        if not edges:
            return Response(
                {'success': False, 'error': 'edges parameter is required'},
...
def propensity_scores(request):
    ...
    try:
        ...
        return Response({
            'success': True,
            'results': sanitize_for_json(result),
            'method': 'propensity_scores'
        }, status=status.HTTP_200_OK)

    except ValueError as e:
        return Response(
            {'success': False, 'error': str(e), 'error_type': 'validation_error'},
            status=status.HTTP_400_BAD_REQUEST
        )
    except Exception as e:
...
def treatment_effects(request):
    ...
    try:
        ...
        if method == 'doubly_robust' and covariates:
            result = doubly_robust_estimator(
                data=df,
                treatment=treatment,
                outcome=outcome,
                covariates=covariates
            ...
            result = {
                'ate': sanitize_for_json(ate_result),
                'att': sanitize_for_json(att_result)
            }
...
def did_parallel_trends(request):
    ...
    try:
        ...
        result = test_parallel_trends_simple(
            ...
            outcome=outcome,
            treatment_group=treatment,
            post_period=treatment_time or time,
            time_var=time
        )
