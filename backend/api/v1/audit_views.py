...
class AuditSummaryView(APIView):
    ...
    def get(self, request):
        ...
        try:
                ...
                field_breakdown.append(
                    {
                        ...
                        "reproducibility_score": float(item["avg_reproducibility"])
                        if item["avg_reproducibility"]
                        else 0,
            ...
            trend_audits = StatisticalAudit.objects.filter(timestamp__gte=trend_start, timestamp__lte=end_date)
            ...
            for i in range(30):
                ...
                day_data = trend_audits.filter(timestamp__gte=day_start, timestamp__lt=day_end).aggregate(
                    analyses=Count("id"), violations=Sum("violations_detected")
                )
            ...
            for aspect in method_aspects:
                # For SQLite compatibility, simplify the query
                compliance_rate = 100  # Default to 100
                methodology_metrics.append({"aspect": aspect, "compliance": compliance_rate})
            ...
            response_data = {
                "summary": {
                    ...
                    "methodologyScore": float(summary_data["avg_methodology_score"])
                    if summary_data["avg_methodology_score"]
                    else 0,
                    "reproducibilityScore": float(summary_data["avg_reproducibility_score"])
                    if summary_data["avg_reproducibility_score"]
                    else 0,
                },
            ...
            logger.error(f"Error in AuditSummaryView: {str(e)}", exc_info=True)
            # Return null if there's an error (same as no data)
            return Response(None, status=status.HTTP_200_OK)
    ...
    def post(self, request):
        ...
        try:
            ...
            audit = StatisticalAudit.objects.create(
                ...
                methodology_score=Decimal(str(data.get("methodology_score", 0)))
                if data.get("methodology_score")
                else None,
                reproducibility_score=Decimal(str(data.get("reproducibility_score", 0)))
                ...
                statistical_power=Decimal(str(data.get("statistical_power", 0)))
                if data.get("statistical_power")
                else None,
            ...
            logger.error(f"Error in AuditRecordView: {str(e)}", exc_info=True)
...
class AuditMetricsView(APIView):
    ...
    def get(self, request, metric_type):
        ...
        try:
            ...
            audits = StatisticalAudit.objects.filter(timestamp__gte=start_date, timestamp__lte=end_date)
            ...
            if metric_type == "fields":
                ...
                data = (
                    audits.values("field")
                    .annotate(
                        total=Count("id"), avg_score=Avg("methodology_score"), violations=Sum("violations_detected")
                    )
                    .order_by("-total")
                )
                ...
                data = (
                    audits.values("test_type")
                    .annotate(total=Count("id"), failures=Sum("assumptions_failed"), avg_power=Avg("statistical_power"))
                    .order_by("-total")
                )
                ...
                for i in range(30):
                    ...
                    day_audits = audits.filter(timestamp__gte=day_start, timestamp__lt=day_end)

                    timeline_data.append(
                        {
                            "date": day_start.strftime("%Y-%m-%d"),
                            "count": day_audits.count(),
                            "avg_score": day_audits.aggregate(Avg("methodology_score"))["methodology_score__avg"] or 0,
                        }
                    )
            ...
            logger.error(f"Error in AuditMetricsView: {str(e)}", exc_info=True)
...
def audit_health_check(request):
    ...
    try:
        ...
        logger.error(f"Error in audit_health_check: {str(e)}", exc_info=True)
        return Response(
