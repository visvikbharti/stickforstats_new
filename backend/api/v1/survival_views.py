...
def kaplan_meier_analysis(request):
    ...
    try:
        ...
        return Response(
            {"success": True, "results": sanitized_results, "method": "kaplan_meier"}, status=status.HTTP_200_OK
        )
...
def cox_regression(request):
    ...
    try:
        ...
        if missing:
            return Response(
                {"error": f'Missing required parameters: {", ".join(missing)}'}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not isinstance(covariate_cols, list) or len(covariate_cols) == 0:
            return Response(
                {"error": "covariate_cols must be a non-empty list of column names"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"success": True, "results": sanitized_results, "method": "cox_proportional_hazards"},
...
def predict_survival(request):
    ...
    try:
        ...
        # Make predictions
        results = survival_service.predict_survival(model_id=model_id, new_data=df, times=times)

        logger.info(f"Survival predictions made for {len(df)} subjects using model {model_id}")

        return Response(
            {"success": True, "results": results, "method": "survival_prediction"}, status=status.HTTP_200_OK
        )
...
def survival_tutorial(request):
    ...
    try:
        ...
        # Create DataFrame
        example_data = pd.DataFrame({"duration": observed_duration, "event": event, "treatment": treatment, "age": age})

        # Perform Kaplan-Meier analysis by treatment group
        km_results = survival_service.kaplan_meier_analysis(
            data=example_data, duration_col="duration", event_col="event", group_col="treatment", confidence_level=0.95
        )

        # Perform Cox regression
        cox_results = survival_service.cox_proportional_hazards(
            data=example_data, duration_col="duration", event_col="event", covariate_cols=["age"], confidence_level=0.95
        )
