...
def detect_missing_patterns(request):
    ...
    try:
        ...
        result = missing_data_handler.analyze_missing_patterns(df, perform_tests=perform_tests)

        logger.info("Missing pattern detection completed")

        return Response({"success": True, "results": result, "method": "pattern_detection"}, status=status.HTTP_200_OK)
...
def perform_littles_mcar_test(request):
    ...
    try:
        ...
        logger.info("Little's MCAR test completed ")

        return Response({"success": True, "results": result, "method": "littles_mcar_test"}, status=status.HTTP_200_OK)
...
def impute_missing_data(request):
    ...
    try:
        ...
        # Perform imputation
        result = missing_data_handler.impute_data(df, method=method, column_strategies=column_strategies, **parameters)
        ...
        return Response({"success": True, "results": response_data, "method": "imputation"}, status=status.HTTP_200_OK)
...
def multiple_imputation(request):
    ...
    try:
        ...
        result = missing_data_handler.mice_imputation(
            df, n_imputations=n_imputations, max_iter=max_iter, random_state=random_state
        )

        logger.info("MICE imputation completed ")

        return Response({"success": True, "results": result, "method": "mice_imputation"}, status=status.HTTP_200_OK)
...
def knn_imputation(request):
    ...
    try:
        ...
        result = missing_data_handler.knn_imputation(df, n_neighbors=n_neighbors, weights=weights, metric=metric)
        ...
        response_data = {
            "imputed_data": imputed_data,
            "method": "knn",
            "parameters": {"n_neighbors": n_neighbors, "weights": weights, "metric": metric},
        }

        logger.info("KNN imputation completed ")

        return Response(
            {"success": True, "results": response_data, "method": "knn_imputation"}, status=status.HTTP_200_OK
        )
...
def em_algorithm_imputation(request):
    ...
    try:
        ...
        result = missing_data_handler.em_imputation(df, max_iter=max_iter, tol=tol)

        logger.info("EM algorithm imputation completed ")

        return Response({"success": True, "results": result, "method": "em_algorithm"}, status=status.HTTP_200_OK)
...
def visualize_missing_patterns(request):
    ...
    try:
        ...
        result = missing_data_handler.visualize_missing(df, visualization_types=visualization_types)

        logger.info("Missing data visualization completed ")

        return Response({"success": True, "results": result, "method": "visualization"}, status=status.HTTP_200_OK)
...
def compare_imputation_methods(request):
    ...
    try:
        ...
        # Compare methods
        result = missing_data_handler.compare_methods(df, methods=methods, metrics=metrics)

        logger.info("Imputation method comparison completed ")

        return Response({"success": True, "results": result, "method": "comparison"}, status=status.HTTP_200_OK)
...
def get_imputation_methods_info(request):
    ...
    try:
        ...
        logger.info("Imputation methods info retrieved ")

        return Response({"success": True, "methods": methods_info}, status=status.HTTP_200_OK)
