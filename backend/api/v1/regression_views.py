...
from core.missing_data_handler import MissingDataHandler, ImputationMethod
    ...
    def post(self, request):
        ...
        try:
            ...
            if np.isnan(X).any() or np.isnan(y).any():
                ...
                missing_info = {"had_missing": True, "method_used": handle_method}
            else:
            ...
            if method == "linear" or method == "multiple":
                ...
                result = hp_regression.lasso_regression(
                    X, y, alpha=params.get("alpha", 1.0), feature_names=feature_names
                )
                ...
                result = hp_regression.quantile_regression(
                    X, y, quantile=params.get("quantile", 0.5), feature_names=feature_names
                )

            elif method == "robust":
                result = hp_regression.robust_regression(
                    X, y, method=params.get("robust_method", "huber"), feature_names=feature_names
                )
            ...
            if options.get("include_visualization", False):
                ...
                plots["residual_plot"] = viz_system.create_regression_diagnostics(
                    y, result.predictions, result.residuals, result.diagnostics.leverage
                )
                ...
                if hasattr(result, "feature_importance") and result.feature_importance:
                    plots["feature_importance"] = viz_system.create_feature_importance_plot(result.feature_importance)
...
class RegressionModelComparisonView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            metrics_to_compare = adapted_data.get("comparison_metrics", ["r_squared", "mse", "aic", "bic"])
            ...
            for model_config in models:
                ...
                if method == "linear":
                    ...
                    result = hp_regression.polynomial_regression(
                        X, y, degree=params.get("degree", 2), feature_names=feature_names
                    )
                else:
                    continue

                # Extract metrics
                model_metrics = {"method": method, "parameters": params}
            ...
            for metric in metrics_to_compare:
                if metric in ["r_squared", "adjusted_r_squared"]:
                    # Higher is better
                    best_model = max(comparison_results, key=lambda x: float(x.get(metric, 0)))
                else:
                    # Lower is better (mse, aic, bic)
                    best_model = min(comparison_results, key=lambda x: float(x.get(metric, float("inf"))))
    ...
    def _generate_recommendation(self, results: list, best_models: dict) -> str:
        ...
        return (
            f"Based on the comparison, {best_overall} performs best on most metrics. "
...
class MissingDataAnalysisView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            analysis = handler.analyze_missing_data(data, column_names=column_names, perform_tests=perform_tests)
            ...
            response_data = {
                ...
                "pattern": {"type": analysis.missing_pattern.value, "confidence": str(analysis.pattern_confidence)},
                "by_column": {
                    col: {"count": info["count"], "percentage": str(info["percentage"]), "pattern": info["pattern"]}
            ...
            if perform_tests and analysis.little_mcar_test:
                response_data["little_mcar_test"] = {k: str(v) for k, v in analysis.little_mcar_test.items()}

            # Add visualization data
            if analysis.missing_heatmap_data is not None:
                response_data["visualization"] = {
                    "heatmap": analysis.missing_heatmap_data.tolist(),
                    "correlations": analysis.missing_correlations.to_dict()
                    if analysis.missing_correlations is not None
                    else None,
                }
