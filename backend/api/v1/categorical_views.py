...
def chi_square_independence_test(request):
    ...
    try:
        ...
        if "contingency_table" not in data:
            return Response(
                {"error": "Missing required parameter: contingency_table"}, status=status.HTTP_400_BAD_REQUEST
            )
...
def chi_square_goodness_of_fit(request):
    ...
    try:
        ...
        if "observed" not in data:
            return Response({"error": "Missing required parameter: observed"}, status=status.HTTP_400_BAD_REQUEST)
...
def fishers_exact_test(request):
    ...
    try:
        ...
        if table.shape != (2, 2):
            return Response(
                {"error": "Fisher's exact test requires a 2x2 contingency table"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Perform calculation
        result = categorical_calculator.fisher_exact_test(table, alternative=alternative)
...
def mcnemar_test(request):
    ...
    try:
        ...
        if table.shape != (2, 2):
            return Response(
                {"error": "McNemar test requires a 2x2 contingency table"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"success": True, "results": result.to_dict(), "precision": "50 decimal places", "method": "mcnemar"},
            status=status.HTTP_200_OK,
        )
...
def cochrans_q_test(request):
    ...
    try:
        ...
        # Perform calculation
        result = categorical_calculator.cochrans_q(test_data, alpha=alpha)

        logger.info(f"Cochran's Q test completed for user {request.user.id}")

        return Response(
            {"success": True, "results": result.to_dict(), "precision": "50 decimal places", "method": "cochrans_q"},
            status=status.HTTP_200_OK,
        )
...
def g_test(request):
    ...
    try:
        ...
        observed = np.array(data.get("observed"))
        _ = data.get("alpha", 0.05)  # Keep for potential future use
        ...
        return Response(
            {"success": True, "results": result.to_dict(), "precision": "50 decimal places", "method": "g_test"},
            status=status.HTTP_200_OK,
        )
...
def binomial_test(request):
    ...
    try:
        ...
        # Perform calculation
        result = categorical_calculator.binomial_test(successes, n, p, alternative=alternative)

        logger.info(f"Binomial test completed for user {request.user.id}")

        return Response(
            {"success": True, "results": result.to_dict(), "precision": "50 decimal places", "method": "binomial_test"},
            status=status.HTTP_200_OK,
        )
...
def multinomial_test(request):
    ...
    try:
        ...
        # Perform calculation
        result = categorical_calculator.multinomial_test(observed, expected_probs=expected_probs, alpha=alpha)
...
def categorical_effect_sizes(request):
    ...
    try:
        ...
        result = categorical_calculator.calculate_effect_sizes(contingency_table, requested_sizes)
