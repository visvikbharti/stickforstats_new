...
class JournalAnalyticsOverviewView(APIView):
    ...
    def get(self, request):
        ...
        submissions = ManuscriptSubmission.objects.filter(journal=journal, submitted_at__gte=since)
...
class JournalAnalyticsIssuesView(APIView):
    ...
    def get(self, request):
        ...
        submissions = ManuscriptSubmission.objects.filter(journal=journal, submitted_at__gte=since).exclude(
            review_report__isnull=True
        )
...
class JournalAnalyticsTrendsView(APIView):
    ...
    def get(self, request):
        ...
        weekly = (
            ManuscriptSubmission.objects.filter(journal=journal, submitted_at__gte=since, sqs_score__isnull=False)
            .annotate(week=TruncWeek("submitted_at"))
            .values("week")
            .annotate(avg_score=Avg("sqs_score"), count=Count("id"))
            .order_by("week")
        )
...
class JournalAnalyticsComparisonView(APIView):
    ...
    def get(self, request):
        ...
        def period_stats(start, end):
            qs = ManuscriptSubmission.objects.filter(journal=journal, submitted_at__gte=start, submitted_at__lt=end)
