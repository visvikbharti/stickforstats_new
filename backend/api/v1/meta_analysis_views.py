...
from core.meta_analysis import run_meta_analysis, get_meta_analysis_engine, StudyData, EffectSizeConverter
    ...
    def post(self, request):
        ...
        try:
            ...
            if not studies or len(studies) < 2:
                return Response(
                    {"success": False, "error": "At least 2 studies are required for meta-analysis"},
            ...
            for i, study in enumerate(studies):
                ...
                if "effect_size" not in study:
                    return Response(
                        {"success": False, "error": f"Study {study_num} is missing effect_size"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                try:
                    ...
                    return Response(
                        {"success": False, "error": f"Study {study_num} has invalid effect_size (must be a number)"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                ...
                if "standard_error" not in study:
                    return Response(
                        {"success": False, "error": f"Study {study_num} is missing standard_error"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
                try:
                    ...
                    if standard_error <= 0:
                        return Response(
                            {"success": False, "error": f"Study {study_num} has invalid standard_error (must be > 0)"},
                            status=status.HTTP_400_BAD_REQUEST,
                        )
            ...
            try:
                ...
                if not (0.001 <= alpha <= 0.5):
                    return Response(
                        {"success": False, "error": "Alpha must be between 0.001 and 0.5"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            except (TypeError, ValueError):
            ...
            # Run meta-analysis
            results = run_meta_analysis(studies=studies, model=model, method=method, alpha=alpha)
            ...
            return Response(
                {"success": False, "error": f"Meta-analysis failed: {str(e)}"},
...
def convert_effect_size(request):
    ...
    try:
        ...
        if value is None:
            return Response({"success": False, "error": "Value is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if from_type == "d":
            if to_type == "r":
                ...
                if n:
                    ...
                    return Response(
                        {"success": False, "error": "Sample size (n) required for d to g conversion"},
                        status=status.HTTP_400_BAD_REQUEST,
                    )
            elif to_type == "log_or":
        ...
        if converted is None:
            return Response(
                {"success": False, "error": f"Conversion from {from_type} to {to_type} not supported"},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
def calculate_effect_size_se(request):
    ...
    try:
        ...
        if effect_size is None:
            return Response({"success": False, "error": "effect_size is required"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        if effect_type == "d":
            if not n1 or not n2:
                return Response(
                    {"success": False, "error": "n1 and n2 required for Cohen's d SE calculation"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            if not n:
                return Response(
                    {"success": False, "error": "n required for correlation SE calculation"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"success": False, "error": f"Unsupported effect type: {effect_type}"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        return Response(
            {"success": True, "effect_type": effect_type, "effect_size": effect_size, "standard_error": float(se)}
        )
...
def publication_bias_test(request):
    ...
    try:
        ...
        if len(studies) < 3:
            return Response(
                {"success": False, "error": "At least 3 studies required for publication bias tests"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response({"success": True, "eggers_test": eggers, "beggs_test": beggs})
...
def sensitivity_analysis(request):
    ...
    try:
        ...
        if len(studies) < 3:
            return Response(
                {"success": False, "error": "At least 3 studies required for sensitivity analysis"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        return Response({"success": True, "sensitivity_results": results})
...
def subgroup_analysis(request):
    ...
    try:
        ...
        if len(studies) < 2:
            return Response(
                {"success": False, "error": "At least 2 studies required"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if not has_subgroups:
            return Response(
                {"success": False, "error": "No subgroups found in study data"}, status=status.HTTP_400_BAD_REQUEST
            )
