...
class FlexibleDataField(serializers.Field):
    ...
    def to_internal_value(self, data):
        ...
        # Handle list input
            ...
            raise serializers.ValidationError("Data must be a comma-separated string or a list of numbers")
...
class TTestRequestSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if test_type in ["two_sample", "paired"] and "data2" not in data:
            raise serializers.ValidationError(f"{test_type} test requires data2")

        # For paired test, ensure equal lengths
        if test_type == "paired" and "data2" in data:
            if len(data["data1"]) != len(data["data2"]):
                raise serializers.ValidationError("Paired test requires data1 and data2 to have equal length")
...
class ComparisonRequestSerializer(serializers.Serializer):
    ...
    SUPPORTED_TESTS = ["t_test", "anova", "correlation", "chi_square", "linear_regression", "mann_whitney", "wilcoxon"]
...
class DataImportSerializer(serializers.Serializer):
    ...
    def validate_file(self, value):
        ...
        if value.size > 100 * 1024 * 1024:
            raise serializers.ValidationError("File size exceeds maximum (100MB)")
        ...
        if not any(file_name.endswith(ext) for ext in allowed_extensions):
            raise serializers.ValidationError(f"Unsupported file format. Allowed: {', '.join(allowed_extensions)}")
...
class ANOVARequestSerializer(serializers.Serializer):
    ...
    POST_HOC_TESTS = ["tukey", "bonferroni", "scheffe", "games_howell", "dunnett", "sidak", "holm", "lsd"]
    ...
    # For two-way ANOVA
    factor1_levels = serializers.ListField(child=serializers.CharField(), required=False)
    factor2_levels = serializers.ListField(child=serializers.CharField(), required=False)
    ...
    # Post-hoc tests
    post_hoc = serializers.ChoiceField(choices=POST_HOC_TESTS, required=False, allow_null=True)

    # Multiple comparison correction
    correction = serializers.ChoiceField(choices=CORRECTIONS, default="none", required=False)
    ...
    def validate(self, data):
        ...
        if "data" in data and "groups" not in data:
            ...
            raise serializers.ValidationError("Either 'groups' or 'data' parameter is required")
        ...
        for i, group in enumerate(groups):
            if len(group) < 2:
                raise serializers.ValidationError(f"Group {i+1} has insufficient data (minimum 2 required)")
            # Check for NaN/Inf
            if any(np.isnan(x) or np.isinf(x) for x in group):
                raise serializers.ValidationError(f"Group {i+1} contains NaN or Inf values")

        # Two-way ANOVA validation
        if anova_type == "two_way":
            if "factor1_levels" not in data or "factor2_levels" not in data:
                raise serializers.ValidationError("Two-way ANOVA requires factor1_levels and factor2_levels")
            expected_groups = len(data["factor1_levels"]) * len(data["factor2_levels"])
            if len(groups) != expected_groups:
                raise serializers.ValidationError(f"Two-way ANOVA expects {expected_groups} groups")

        # MANOVA validation
        if anova_type == "manova":
            if "dependent_variables" not in data:
                raise serializers.ValidationError("MANOVA requires dependent_variables")
            ...
            for dv in data["dependent_variables"]:
                if len(dv) != sum(len(g) for g in groups):
                    raise serializers.ValidationError("Dependent variables must match total observations")

        # Post-hoc test validation
        if data.get("post_hoc") and len(groups) < 3:
            raise serializers.ValidationError("Post-hoc tests require at least 3 groups")
...
class RegressionRequestSerializer(serializers.Serializer):
    ...
    y = serializers.ListField(child=serializers.FloatField(), validators=[DataArrayValidator()])
    parameters = serializers.DictField(required=False, default=dict)
    options = serializers.DictField(required=False, default=dict)
...
class HighPrecisionNumberField(serializers.Field):
    ...
    def to_internal_value(self, data):
        ...
        try:
            ...
            raise serializers.ValidationError("Invalid numeric value")
...
class CategoricalRequestSerializer(serializers.Serializer):
    ...
    expected_frequencies = serializers.ListField(child=serializers.FloatField(min_value=0), required=False)
    ...
    def validate(self, data):
        ...
        if "contingency_table" not in data and "data" not in data:
            raise serializers.ValidationError("Either 'contingency_table' or 'data' must be provided")
        ...
        if "contingency_table" in data:
            ...
            if test_type == "fishers_exact" and (len(table) != 2 or len(table[0]) != 2):
                raise serializers.ValidationError("Fisher's exact test requires a 2x2 contingency table")

            if test_type == "mcnemar" and (len(table) != 2 or len(table[0]) != 2):
                raise serializers.ValidationError("McNemar's test requires a 2x2 contingency table")
    ...
    def validate(self, data):
        ...
        if test_type in ["mann_whitney", "wilcoxon", "sign_test"]:
            if test_type == "mann_whitney" and not ("data1" in data and "data2" in data):
                raise serializers.ValidationError("Mann-Whitney U test requires data1 and data2")
            if test_type == "wilcoxon" and data.get("paired"):
                if not ("data1" in data and "data2" in data):
                    raise serializers.ValidationError("Paired Wilcoxon test requires data1 and data2")
                if len(data["data1"]) != len(data["data2"]):
                    raise serializers.ValidationError("Paired samples must have equal length")

        elif test_type in ["kruskal_wallis", "friedman"]:
            if "groups" not in data:
                raise serializers.ValidationError(f"{test_type} test requires 'groups' data")
            if len(data["groups"]) < 3:
                raise serializers.ValidationError(f"{test_type} test requires at least 3 groups")
...
class ANCOVARequestSerializer(serializers.Serializer):
    ...
    groups = FlexibleGroupsField(help_text="List of groups, each containing numerical data points")
    covariates = FlexibleCovariatesField(help_text="List of covariates, each containing numerical data points")
    group_names = serializers.ListField(
        child=serializers.CharField(), required=False, help_text="Optional names for each group"
    )
    covariate_names = serializers.ListField(
        child=serializers.CharField(), required=False, help_text="Optional names for each covariate"
    )
    dependent_variable_name = serializers.CharField(
        required=False, default="Dependent Variable", help_text="Name of the dependent variable"
    )
    alpha = serializers.FloatField(
        required=False, default=0.05, min_value=0.001, max_value=0.999, help_text="Significance level for tests"
    )
    check_homogeneity_slopes = serializers.BooleanField(
        required=False, default=True, help_text="Check homogeneity of regression slopes assumption"
    )
    post_hoc = serializers.ChoiceField(
        choices=["tukey", "bonferroni", "scheffe", "games_howell", "dunnett", "fisher_lsd", "newman_keuls", "duncan"],
        required=False,
        help_text="Post-hoc test to perform",
    )
    options = serializers.DictField(
        child=serializers.JSONField(), required=False, help_text="Additional options for ANCOVA analysis"
    )
    ...
    def validate_groups(self, value):
        ...
        for i, group in enumerate(value):
            if len(group) < 2:
                raise serializers.ValidationError(f"Group {i+1} must have at least 2 data points")
...
class MissingDataRequestSerializer(serializers.Serializer):
    ...
    missingness_type = serializers.ChoiceField(choices=MISSINGNESS_TESTS, required=False)
    ...
    def validate(self, data):
        ...
        if method == "knn" and "parameters" in data:
            ...
            if "n_neighbors" in params:
                ...
                if not isinstance(n, int) or n < 1:
                    raise serializers.ValidationError("n_neighbors must be a positive integer")
        ...
        if method == "mice" and "parameters" in data:
            ...
            if "n_iterations" in params:
                ...
                if not isinstance(n, int) or n < 1:
                    raise serializers.ValidationError("n_iterations must be a positive integer")
