        ...
        try:
            ...
            if method == "auto" or options.get("auto_select", False):
                ...
                if method == "pearson":
                    ...
                    return Response(
                        {"error": f"Unknown correlation method: {method}"}, status=status.HTTP_400_BAD_REQUEST
                    )
...
class AutomaticTestSelectorView(APIView):
    ...
    def post(self, request):
        ...
        """
        from core.automatic_test_selector import AutomaticTestSelector, ResearchQuestion, generate_user_guidance
        ...
        try:
            ...
            recommendation = selector.recommend_test(data_profile, research_question, context)
            ...
            return Response(
                {"error": "Test selection failed", "message": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
