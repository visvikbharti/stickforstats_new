...
class RegressionDataSerializer(serializers.Serializer):
    ...
    y = serializers.ListField(child=serializers.FloatField(), help_text="Dependent variable vector")
    feature_names = serializers.ListField(
        child=serializers.CharField(max_length=100), required=False, help_text="Names of features"
    )
...
class RegressionParametersSerializer(serializers.Serializer):
    ...
    confidence_level = serializers.FloatField(
        default=0.95, min_value=0.5, max_value=0.999, help_text="Confidence level for intervals"
    )
    ...
    robust_standard_errors = serializers.BooleanField(
        default=False, help_text="Use heteroscedasticity-robust standard errors"
    )
    alpha = serializers.FloatField(default=1.0, min_value=0, help_text="Regularization strength for Ridge/Lasso")
    degree = serializers.IntegerField(default=2, min_value=1, max_value=10, help_text="Polynomial degree")
    quantile = serializers.FloatField(
        default=0.5, min_value=0.01, max_value=0.99, help_text="Quantile for quantile regression"
    )
    cv_folds = serializers.IntegerField(
        default=5, min_value=2, max_value=20, help_text="Number of cross-validation folds"
    )
    include_interaction = serializers.BooleanField(
        default=True, help_text="Include interaction terms in polynomial regression"
    )
    regularization = serializers.ChoiceField(
        choices=["none", "l1", "l2", "elastic_net"], default="none", required=False, help_text="Regularization type"
    )
    logistic_type = serializers.ChoiceField(
        choices=["binary", "multinomial"], default="binary", help_text="Type of logistic regression"
    )
    robust_method = serializers.ChoiceField(
        choices=["huber", "ransac", "theil_sen"], default="huber", help_text="Robust regression method"
    )
    stepwise_method = serializers.ChoiceField(
        choices=["forward", "backward", "both"], default="both", help_text="Stepwise selection method"
    )
    alpha_in = serializers.FloatField(
        default=0.05, min_value=0.001, max_value=0.5, help_text="P-value threshold for adding features"
    )
    alpha_out = serializers.FloatField(
        default=0.10, min_value=0.001, max_value=0.5, help_text="P-value threshold for removing features"
    )
...
class RegressionOptionsSerializer(serializers.Serializer):
    ...
    include_diagnostics = serializers.BooleanField(default=True, help_text="Include diagnostic statistics")
    include_visualization = serializers.BooleanField(default=False, help_text="Include visualization data")
    compare_with_standard = serializers.BooleanField(default=False, help_text="Compare with standard precision")
...
class RegressionInputSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Check dimensions
        if len(X) != len(y):
            raise serializers.ValidationError("Number of samples in X must match length of y")
        ...
        if "feature_names" in regression_data:
            ...
            if len(regression_data["feature_names"]) != n_features:
                raise serializers.ValidationError("Number of feature names must match number of features")
        ...
        if method == "logistic":
            ...
            if params.get("logistic_type") == "binary" and len(unique_values) > 2:
                raise serializers.ValidationError("Binary logistic regression requires binary target variable")
...
class RegressionResultSerializer(serializers.Serializer):
    ...
    coefficients = serializers.DictField(child=serializers.CharField())
...
class ModelComparisonInputSerializer(serializers.Serializer):
    ...
    data = RegressionDataSerializer()
    models = serializers.ListField(child=serializers.DictField(), help_text="List of models to compare")
    comparison_metrics = serializers.ListField(
        child=serializers.CharField(), default=["r_squared", "mse", "aic", "bic"], help_text="Metrics to compare"
    )
...
class MissingDataSerializer(serializers.Serializer):
    ...
    data = serializers.ListField(child=serializers.ListField(), help_text="Data matrix with potential missing values")
    column_names = serializers.ListField(child=serializers.CharField(), required=False, help_text="Column names")
    perform_tests = serializers.BooleanField(default=True, help_text="Perform statistical tests for missingness")


class ImputationInputSerializer(serializers.Serializer):
    """Serializer for data imputation input."""

    data = serializers.ListField(child=serializers.ListField(), help_text="Data with missing values")
    method = serializers.ChoiceField(
        choices=[
            "drop",
            "mean",
    ...
    column_strategies = serializers.DictField(
        child=serializers.CharField(), required=False, help_text="Column-specific imputation strategies"
    )
    parameters = serializers.DictField(required=False, help_text="Method-specific parameters")
