        ...
        try:
            ...
            if options.get("check_assumptions", True):
                ...
                if data2:
                    ...
                    if test_type == "two_sample":
                        variance_result = assumption_checker.check_homoscedasticity(np.array(data1), np.array(data2))
            ...
            if test_type == "one_sample":
                ...
                if not data2 or len(data1) != len(data2):
                    return Response(
                        {"error": "Paired test requires two arrays of equal length"}, status=status.HTTP_400_BAD_REQUEST
                    )
            ...
            if options.get("validate_results", True) and not skip_validation:
                ...
                validation_suite = validator.validate_t_test(
                    np.array(data1), np.array(data2) if data2 else None, test_type, **parameters
                )
            ...
            logger.info(
                f"Successfully calculated {test_type} t-test with {len(str(hp_result['t_statistic']).split('.')[-1])} decimal precision"
            )
...
class ComparisonView(APIView):
    ...
    def post(self, request):
        ...
        comparison_results = {"test_name": test_name, "high_precision": {}, "standard_precision": {}, "improvement": {}}
    ...
    def post(self, request):
        ...
        try:
            ...
            if file_name.endswith(".csv"):
                ...
                return Response({"error": f"Unsupported file format: {file_name}"}, status=status.HTTP_400_BAD_REQUEST)
            ...
            return Response(
                {"status": "success", "summary": summary, "message": f"Successfully imported {file_name}"},
            ...
            return Response(
                {"error": "Failed to import file", "message": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            if options.get("check_assumptions", True):
                ...
                homogeneity_result = assumption_checker.check_homoscedasticity(*[np.array(g) for g in groups])
                ...
                # Check sample sizes
                assumptions["sample_sizes"] = {f"group_{i+1}": len(group) for i, group in enumerate(groups)}
            ...
            if options.get("generate_visualizations", True):
                ...
                viz_data = {"group_means": {}, "group_stds": {}, "group_sizes": {}, "group_data": groups}
            ...
            # Log successful calculation
            logger.info(
                f"Successfully calculated {anova_type} ANOVA with {len(str(result.f_statistic).split('.')[-1])} decimal precision"
            )
