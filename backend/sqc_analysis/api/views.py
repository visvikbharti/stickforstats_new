...
from sqc_analysis.models import (
    ...
    EconomicDesignAnalysis,
    SPCImplementationPlan,
)
from sqc_analysis.api.serializers import (
    ControlChartAnalysisSerializer,
    ProcessCapabilityAnalysisSerializer,
    AcceptanceSamplingPlanSerializer,
    MeasurementSystemAnalysisSerializer,
    EconomicDesignAnalysisSerializer,
    SPCImplementationPlanSerializer,
    ControlChartRequestSerializer,
    ProcessCapabilityRequestSerializer,
    AcceptanceSamplingRequestSerializer,
    MeasurementSystemAnalysisRequestSerializer,
    EconomicDesignRequestSerializer,
    SPCImplementationRequestSerializer,
)
...
class ControlChartViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ControlChartAnalysis.objects.filter(analysis_session__user=user).select_related(
            "analysis_session", "analysis_result"
        )
    ...
    def create(self, request):
        ...
        session = AnalysisSession.objects.create(
            ...
            module="sqc",
            status="in_progress",
            configuration={"analysis_type": "control_chart", "chart_type": chart_type},
        )
    ...
    def plot_data(self, request, pk=None):
        ...
        if not control_chart.analysis_result:
            return Response(
                {"error": "No analysis result found for this control chart."}, status=status.HTTP_404_NOT_FOUND
            )
...
class ProcessCapabilityViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter queryset to only show user's own analyses."""
        user = self.request.user
        return ProcessCapabilityAnalysis.objects.filter(analysis_session__user=user).select_related(
            "analysis_session", "analysis_result"
        )
    ...
    def create(self, request):
        ...
        data = serializer.validated_data
        parameter_column = data["parameter_column"]
        session_name = data.get("session_name", "Process Capability Analysis")
        ...
        session = AnalysisSession.objects.create(
            ...
            module="sqc",
            status="in_progress",
            configuration={"analysis_type": "process_capability", "parameter_column": parameter_column},
        )
        ...
        try:
            ...
            send_notification.delay(
                ...
                message="Your process capability analysis has been completed.",
                notification_type="success",
...
class AcceptanceSamplingViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter queryset to only show user's own analyses."""
        user = self.request.user
        return AcceptanceSamplingPlan.objects.filter(analysis_session__user=user).select_related(
            "analysis_session", "analysis_result"
        )
    ...
    def create(self, request):
        ...
        session = AnalysisSession.objects.create(
            ...
            module="sqc",
            status="in_progress",
            configuration={"analysis_type": "acceptance_sampling", "plan_type": plan_type},
        )
        ...
        try:
            ...
            if plan_type == "single":
                ...
                result = sampling_service.calculate_skip_lot_plan(
                    base_plan=base_plan, frequency_index=data.get("frequency_index", 2)
                )
    ...
    def oc_curve(self, request, pk=None):
        ...
        if not sampling_plan.analysis_result:
            return Response(
                {"error": "No analysis result found for this sampling plan."}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def compare_plans(self, request):
        ...
        try:
            ...
            for plan_id in plan_ids:
                plan = AcceptanceSamplingPlan.objects.get(id=plan_id, analysis_session__user=request.user)
...
class MeasurementSystemAnalysisViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter queryset to only show user's own analyses."""
        user = self.request.user
        return MeasurementSystemAnalysis.objects.filter(analysis_session__user=user).select_related(
            "analysis_session", "analysis_result"
        )
    ...
    def create(self, request):
        ...
        session = AnalysisSession.objects.create(
            ...
            module="sqc",
            status="in_progress",
            configuration={"analysis_type": "msa", "msa_type": msa_type},
        )
        ...
        try:
            ...
            if msa_type == "gage_rr":
                ...
                if not part_column or part_column not in df.columns:
                    raise ValidationError({"part_column": "Part column is required for gage R&R analysis."})

                if not operator_column or operator_column not in df.columns:
                    raise ValidationError({"operator_column": "Operator column is required for gage R&R analysis."})
                ...
                if not part_column or part_column not in df.columns:
                    raise ValidationError({"part_column": "Part column is required for attribute agreement analysis."})

                if not operator_column or operator_column not in df.columns:
                    raise ValidationError(
                        {"operator_column": "Operator column is required for attribute agreement analysis."}
                    )
                ...
                if not time_column or time_column not in df.columns:
                    raise ValidationError({"time_column": "Time column is required for stability analysis."})
        ...
        try:
            ...
            if msa_type == "gage_rr":
                ...
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="Gage R&R Analysis Results",
                    analysis_type="msa_gage_rr",
                    parameters=data,
                    result_summary={
                ...
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="Attribute Agreement Analysis Results",
                    analysis_type="msa_attribute_agreement",
                    parameters=data,
                    result_summary={
                ...
                result = msa_service.analyze_linearity_bias(
                    data=df, reference_col=data["reference_column"], measurement_col=data["parameter_column"]
                )

                # Create analysis result
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="Linearity & Bias Analysis Results",
                    analysis_type="msa_linearity_bias",
                    parameters=data,
                    result_summary={
                ...
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="Stability Analysis Results",
                    analysis_type="msa_stability",
                    parameters=data,
                    result_summary={
...
class EconomicDesignViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter queryset to only show user's own analyses."""
        user = self.request.user
        return EconomicDesignAnalysis.objects.filter(analysis_session__user=user).select_related(
            "analysis_session", "analysis_result"
        )
    ...
    def create(self, request):
        ...
        session_name = data.get("session_name", "Economic Design Analysis")
        ...
        session = AnalysisSession.objects.create(
            ...
            module="sqc",
            status="in_progress",
            configuration={"analysis_type": "economic_design", "chart_type": chart_type},
        )
        ...
        try:
            ...
            send_notification.delay(
                ...
                message="Your economic design analysis has been completed.",
                notification_type="success",
...
class SPCImplementationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter queryset to only show user's own analyses."""
        user = self.request.user
        return SPCImplementationPlan.objects.filter(analysis_session__user=user).select_related(
            "analysis_session", "analysis_result"
        )
    ...
    def create(self, request):
        ...
        session_name = data.get("session_name", "SPC Implementation Plan")
        ...
        session = AnalysisSession.objects.create(
            ...
            module="sqc",
            status="in_progress",
            configuration={"analysis_type": "spc_implementation", "plan_type": plan_type},
        )
        ...
        try:
            if plan_type == "roadmap":
                ...
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="SPC Implementation Roadmap",
                    analysis_type="spc_implementation_roadmap",
                    parameters=data,
                    result_summary={
                ...
                result = impl_service.create_control_plan(control_plan_items=control_plan_items, metadata=metadata)
                ...
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="SPC Control Plan",
                    analysis_type="spc_control_plan",
                    parameters=data,
                    result_summary={
                    ...
                    result_detail={"control_plan": result, "evaluation": evaluation},
                ...
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="SPC Maturity Assessment",
                    analysis_type="spc_maturity_assessment",
                    parameters=data,
                    result_summary={
                ...
                result = impl_service.get_case_study(industry=industry, focus_area=focus_area)

                # Create analysis result
                analysis_result = AnalysisResult.objects.create(
                    session=session,
                    name="SPC Case Study",
                    analysis_type="spc_case_study",
                    parameters=data,
                    result_summary={
    ...
    def assessment_report(self, request, pk=None):
        ...
        if plan.plan_type != "maturity_assessment":
            return Response(
                {"error": "This action is only available for maturity assessments."}, status=status.HTTP_400_BAD_REQUEST
            )

        if not plan.analysis_result:
            return Response({"error": "No analysis result found for this plan."}, status=status.HTTP_404_NOT_FOUND)
...
class QuickControlChartView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if not measurements:
                return Response(
                    {"status": "error", "error": {"code": "NO_DATA", "message": "No measurements provided"}},
            ...
            if chart_type == "i_mr":
                ...
                result = chart_service.analyze_control_chart(data=data_list, chart_type="imr")
                ...
                result = chart_service.analyze_control_chart(data=df, chart_type="xbar_r", subgroup_size=subgroup_size)
            else:
                # Handle other chart types
                data_list = [{"Measurement": value} for value in measurements]
                result = chart_service.analyze_control_chart(data=data_list, chart_type=chart_type)

            # Generate creative visualizations
            visualizations = {}

            # Control chart with animated points
            visualizations["control_chart"] = self._create_control_chart_svg(result, chart_type)
            ...
            if isinstance(out_of_control, dict):
                for chart_type_key, indices in out_of_control.items():
                    for idx in indices:
                        violations.append({"index": idx, "type": f"Out of control ({chart_type_key})"})

            # Violation highlights
            if violations:
                visualizations["violations_plot"] = self._create_violations_plot(result)

            # Process capability preview
            visualizations["capability_preview"] = self._create_capability_preview(measurements, result)
            ...
            return Response(
                {"status": "error", "error": {"code": "CHART_ERROR", "message": str(e)}},
    ...
    def _create_control_chart_svg(self, result, chart_type):
        ...
        ax.fill_between(x, center_line + 2 * zone_a / 3, ucl, alpha=0.1, color="yellow", label="Zone A")
        ax.fill_between(
            x, center_line + zone_a / 3, center_line + 2 * zone_a / 3, alpha=0.1, color="green", label="Zone B"
        )
        ax.fill_between(x, center_line, center_line + zone_a / 3, alpha=0.1, color="blue", label="Zone C")

        # Mirror zones below
        ax.fill_between(x, lcl, center_line - 2 * zone_a / 3, alpha=0.1, color="yellow")
        ax.fill_between(x, center_line - 2 * zone_a / 3, center_line - zone_a / 3, alpha=0.1, color="green")
        ax.fill_between(x, center_line - zone_a / 3, center_line, alpha=0.1, color="blue")
...
class QuickProcessCapabilityView(APIView):
        ...
        try:
            ...
            if not lsl and not usl:
                return Response(
                    {
                        "status": "error",
                        "error": {"code": "NO_SPECS", "message": "At least one specification limit required"},
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            # Prepare dataframe
            _ = pd.DataFrame({"value": measurements})

            # Calculate capability
            result = capability_service.calculate_capability_indices(data=measurements, lsl=lsl, usl=usl, target=target)

            # Generate creative visualizations
            visualizations = {}

            # Capability histogram with curve fit
            visualizations["capability_plot"] = self._create_capability_plot(measurements, result, lsl, usl, target)

            # Process performance dashboard
            visualizations["performance_dashboard"] = self._create_performance_dashboard(result)

            # Animated gauge chart
            visualizations["gauge_chart"] = self._create_gauge_chart(result.get("Cpk", result.get("cpk", 0)))

            # Interactive what-if analysis
            visualizations["what_if_analysis"] = self._create_what_if_analysis(measurements, lsl, usl)
            ...
            return Response(
                {
                    ...
                    "data": {
                        "results": {
                            ...
                            "percent_out_of_spec": result.get("below_lsl_percent", 0)
                            + result.get("above_usl_percent", 0),
                        },
                        "visualizations": visualizations,
            ...
            return Response(
                {"status": "error", "error": {"code": "CAPABILITY_ERROR", "message": str(e)}},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def _create_capability_plot(self, measurements, result, lsl, usl, target):
        ...
        # Histogram
        n, bins, patches = ax.hist(measurements, bins=30, density=True, alpha=0.7, color="lightblue", edgecolor="black")
...
class QuickAcceptanceSamplingView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {
                    ...
                    "data": {
                        "results": {
                            ...
                            "rejection_number": result.get("acceptance_number", 0) + 1
                            if result.get("acceptance_number") is not None
                            else 1,
            ...
            return Response(
                {"status": "error", "error": {"code": "SAMPLING_ERROR", "message": str(e)}},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def _create_oc_curve(self, result):
        ...
        if quality_levels and probabilities:
            ...
            ax.scatter([aql], [aql_prob], color="green", s=100, zorder=5, label=f"AQL ({aql}%, {aql_prob:.3f})")
            ax.scatter([ltpd], [ltpd_prob], color="red", s=100, zorder=5, label=f"LTPD ({ltpd}%, {ltpd_prob:.3f})")
...
class QuickMSAView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if msa_type == "gage_rr":
                ...
                return Response(
                    {"status": "error", "error": {"code": "INVALID_TYPE", "message": f"Invalid MSA type: {msa_type}"}},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {"status": "error", "error": {"code": "MSA_ERROR", "message": str(e)}},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def _perform_gage_rr(self, data):
        ...
        if not measurements:
            return Response(
                {"status": "error", "error": {"code": "NO_DATA", "message": "No measurement data provided"}},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        for part, operators in measurements.items():
            for operator, values in operators.items():
                for value in values:
                    data_list.append({"Part": part, "Operator": operator, "Measurement": value})

        df = pd.DataFrame(data_list)

        # Perform analysis
        result = msa_service.calculate_gauge_rr_anova(
            data=df, parts_col="Part", operators_col="Operator", measurements_col="Measurement"
        )
    ...
    def _create_components_chart(self, result):
        ...
        # Create bar chart
        bars = ax.bar(components.keys(), components.values(), color=["green", "orange", "red"], alpha=0.7)
    ...
    def _perform_bias_analysis(self, data):
        ...
        return Response(
            {
                "status": "success",
                "data": {"results": {}, "visualizations": {}, "interpretation": "Bias analysis completed"},
            }
        )
    ...
    def _perform_stability_analysis(self, data):
        ...
        return Response(
            {
                "status": "success",
                "data": {"results": {}, "visualizations": {}, "interpretation": "Stability analysis completed"},
            }
        )
...
class SQCSimulationView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"status": "success", "data": {"simulations": simulations}})
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({"status": "success", "data": result, "message": f"{simulation_type} simulation completed"})

        except Exception as e:
            logger.error(f"Error in SQC simulation: {str(e)}")
            return Response(
                {"status": "error", "error": {"code": "SIMULATION_ERROR", "message": str(e)}},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class SQCDemoDataView(APIView):
    ...
    def get(self, request):
        ...
        return Response({"status": "success", "data": {"demos": demos}})
        ...
        try:
            ...
            if demo_type == "manufacturing_process":
                ...
                return Response(
                    {
                        "status": "error",
                        "error": {"code": "UNKNOWN_DEMO", "message": f"Unknown demo type: {demo_type}"},
                    },
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            return Response(
                {
                    "status": "success",
                    "data": {"dataset": data.to_dict("records"), "metadata": metadata},
            ...
            return Response(
                {"status": "error", "error": {"code": "DEMO_ERROR", "message": str(e)}},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
    ...
    def _generate_electronics_msa_data(self):
        ...
        # Operator biases
        operator_bias = {"Operator_A": 0.0, "Operator_B": 0.2, "Operator_C": -0.1}
        ...
        for part in parts:
            for operator in operators:
                for replicate in range(1, replicates + 1):
                    ...
                    measurement = true_values[part] + operator_bias[operator] + np.random.normal(0, repeatability_std)

                    data_list.append(
                        {"Part": part, "Operator": operator, "Trial": replicate, "Measurement": measurement}
                    )
    ...
    def _generate_pharmaceutical_data(self):
        ...
        metadata = {
            ...
            "specifications": {"tablet_weight": {"lsl": 95.0, "usl": 105.0, "target": 100.0}},
            "units": "mg",
