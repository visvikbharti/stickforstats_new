...
class AnalysisResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "name", "analysis_type", "parameters", "result_summary", "interpretation", "created_at"]
...
class AnalysisSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ["id", "name", "description", "module", "status", "created_at", "updated_at"]
...
class AcceptanceSamplingPlanSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            "rejection_number",
            "second_sample_size",
            "second_acceptance_number",
            "second_rejection_number",
            "standard_used",
            "aql",
            "ltpd",
            "producer_risk",
...
class ControlChartRequestSerializer(serializers.Serializer):
    ...
    chart_type = serializers.ChoiceField(choices=["xbar_r", "xbar_s", "i_mr", "p", "np", "c", "u"], required=True)
    ...
    rule_set = serializers.ChoiceField(
        choices=["western_electric", "nelson", "custom"], required=False, default="western_electric"
    )
...
class ProcessCapabilityRequestSerializer(serializers.Serializer):
    ...
    transformation_method = serializers.ChoiceField(
        choices=["none", "box_cox", "johnson", "log"], required=False, default="none"
    )
    session_name = serializers.CharField(required=False, max_length=255)
    ...
    def validate(self, data):
        ...
        if not data.get("lower_spec_limit") and not data.get("upper_spec_limit"):
            raise serializers.ValidationError("At least one specification limit (lower or upper) must be provided.")
        return data
...
class AcceptanceSamplingRequestSerializer(serializers.Serializer):
    ...
    plan_type = serializers.ChoiceField(
        choices=["single", "double", "multiple", "sequential", "skip_lot", "continuous", "ansi_z14"], required=True
    )
    ...
    def validate(self, data):
        ...
        if plan_type in ["single", "double", "multiple", "skip_lot", "ansi_z14"]:
            if not data.get("lot_size"):
                raise serializers.ValidationError({"lot_size": f"Lot size is required for {plan_type} sampling plan."})
...
class MeasurementSystemAnalysisRequestSerializer(serializers.Serializer):
    ...
    msa_type = serializers.ChoiceField(
        choices=["gage_rr", "attribute_agreement", "bias", "linearity", "stability"], required=True
    )
    parameter_column = serializers.CharField(required=True)
    ...
    time_column = serializers.CharField(required=False, allow_blank=True)
    method = serializers.ChoiceField(choices=["anova", "range"], required=False, default="anova")
    attribute_type = serializers.ChoiceField(choices=["binary", "ordinal", "nominal"], required=False, default="binary")
    session_name = serializers.CharField(required=False, max_length=255)
    ...
    def validate(self, data):
        ...
        if msa_type in ["gage_rr", "attribute_agreement"]:
            if not data.get("part_column"):
                raise serializers.ValidationError({"part_column": f"Part column is required for {msa_type} analysis."})
        ...
        if msa_type == "stability":
            if not data.get("time_column"):
                raise serializers.ValidationError({"time_column": "Time column is required for stability analysis."})
...
class EconomicDesignRequestSerializer(serializers.Serializer):
    ...
    chart_type = serializers.ChoiceField(
        choices=["xbar", "xbar_r", "xbar_s", "i_mr", "p", "np", "c", "u"], required=True
    )
    # Process parameters
...
class SPCImplementationRequestSerializer(serializers.Serializer):
    ...
    plan_type = serializers.ChoiceField(
        choices=["roadmap", "control_plan", "maturity_assessment", "case_study"], required=True
    )
    industry = serializers.CharField(required=False, default="manufacturing")
    organization_size = serializers.ChoiceField(
        choices=["small", "medium", "large", "enterprise"], required=False, default="medium"
    )
    implementation_scope = serializers.ChoiceField(
        choices=["pilot", "department", "division", "organization"], required=False, default="department"
    )
    existing_quality_system = serializers.ChoiceField(
        choices=["none", "basic", "iso9001", "six_sigma", "lean"], required=False, default="basic"
    )
    process_complexity = serializers.ChoiceField(choices=["low", "medium", "high"], required=False, default="medium")
    focus_area = serializers.CharField(required=False)

    # For control plans
    control_plan_items = serializers.ListField(child=serializers.DictField(), required=False, default=list)
    ...
    def validate(self, data):
        ...
        if plan_type == "case_study" and not data.get("focus_area"):
            raise serializers.ValidationError({"focus_area": "Focus area is required for case study."})
