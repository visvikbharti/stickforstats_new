...
class ControlChartAnalysis(models.Model):
    ...
    analysis_session = models.ForeignKey(
        AnalysisSession, on_delete=models.CASCADE, related_name="control_chart_analyses"
    )
    analysis_result = models.OneToOneField(
        AnalysisResult, on_delete=models.CASCADE, related_name="control_chart_detail", null=True, blank=True
    )
...
class ProcessCapabilityAnalysis(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    analysis_session = models.ForeignKey(
        AnalysisSession, on_delete=models.CASCADE, related_name="process_capability_analyses"
    )
    analysis_result = models.OneToOneField(
        AnalysisResult, on_delete=models.CASCADE, related_name="process_capability_detail", null=True, blank=True
    )
    ...
    transformation_method = models.CharField(
        max_length=20,
        choices=[
            ("none", "None"),
            ("box_cox", "Box-Cox"),
...
class AcceptanceSamplingPlan(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    analysis_session = models.ForeignKey(
        AnalysisSession, on_delete=models.CASCADE, related_name="acceptance_sampling_plans"
    )
    analysis_result = models.OneToOneField(
        AnalysisResult, on_delete=models.CASCADE, related_name="acceptance_sampling_detail", null=True, blank=True
    )
...
class MeasurementSystemAnalysis(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    analysis_session = models.ForeignKey(
        AnalysisSession, on_delete=models.CASCADE, related_name="measurement_system_analyses"
    )
    analysis_result = models.OneToOneField(
        AnalysisResult, on_delete=models.CASCADE, related_name="msa_detail", null=True, blank=True
    )

    # MSA type
    msa_type = models.CharField(
        max_length=30,
        choices=[
            ("gage_rr", "Gage R&R"),
