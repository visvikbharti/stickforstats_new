...
class ProcessCapabilityService:
    ...
    def non_normal_capability_analysis(
        ...
        if distribution == "auto":
            ...
            try:
                ...
                distribution_fits["boxcox"] = {"p_value": 0, "error": str(e)}
            ...
            try:
                ...
                distribution_fits["yeojohnson"] = {"p_value": 0, "error": str(e)}
        ...
        if distribution == "boxcox":
            ...
            transformed_target = self._transform_boxcox(target if target is not None else (lsl + usl) / 2, lambda_param)
            ...
            transformed_target = self._transform_yeojohnson(
                target if target is not None else (lsl + usl) / 2, lambda_param
            )
            ...
            for dist_name in distributions:
                try:
                    ...
                    if ks_p > best_kstest:
                        best_kstest = ks_p
                        best_dist = {"name": dist_name, "params": params, "ks_p": ks_p}
                except Exception:
            ...
            if best_dist:
                ...
                result["transformation"] = "none"
                result["capability_indices"] = self._calculate_percentile_capability(values, lsl, usl, target)
    ...
    def _calculate_percentile_capability(
        self, data: np.ndarray, lsl: float, usl: float, target: Optional[float] = None
    ) -> Dict[str, float]:
        """Calculate capability indices based on percentiles."""
        if target is None:
            target = (lsl + usl) / 2
