...
class ControlChartService:
    ...
    def calculate_xbar_r_chart(
        ...
        try:
            ...
            d4_values = {2: 3.267, 3: 2.575, 4: 2.282, 5: 2.115, 6: 2.004, 7: 1.924, 8: 1.864, 9: 1.816, 10: 1.777}
            ...
            if detect_rules:
                x_violations = self._detect_rule_violations(xbar_values, xbar, x_ucl, x_lcl, rule_set)
                r_violations = self._detect_rule_violations(range_values, rbar, r_ucl, r_lcl, rule_set)
            ...
            plot_data = {
                ...
                "subgroups": [{"name": label, "values": values} for label, values in zip(subgroup_labels, subgroups)],
            }
    ...
    def calculate_xbar_s_chart(
        ...
        try:
            ...
            a3_values = {2: 2.659, 3: 1.954, 4: 1.628, 5: 1.427, 6: 1.287, 7: 1.182, 8: 1.099, 9: 1.032, 10: 0.975}
            ...
            if detect_rules:
                x_violations = self._detect_rule_violations(xbar_values, xbar, x_ucl, x_lcl, rule_set)
                s_violations = self._detect_rule_violations(s_values, sbar, s_ucl, s_lcl, rule_set)
            ...
            interpretation = self._generate_xbar_s_interpretation(
                xbar_values, s_values, xbar, sbar, x_ucl, x_lcl, s_ucl, s_lcl, x_violations, s_violations, process_stats
            )
    ...
    def calculate_i_mr_chart(
        ...
        try:
            ...
            moving_ranges = [
                abs(individual_values[i] - individual_values[i - 1]) for i in range(1, len(individual_values))
            ]
            ...
            if detect_rules:
                i_violations = self._detect_rule_violations(individual_values, x_bar, i_ucl, i_lcl, rule_set)
                # For MR chart, we need to handle the offset in indices
                mr_violations_raw = self._detect_rule_violations(moving_ranges, mr_bar, mr_ucl, mr_lcl, rule_set)
        ...
        try:
            ...
            if detect_rules:
                ...
                if variable_sample_size:
                    ...
                    violations = self._detect_rule_violations(z_scores, 0, 3, -3, rule_set)
                else:
                    # Fixed control limits
                    violations = self._detect_rule_violations(p_values, p_bar, p_ucl[0], p_lcl[0], rule_set)
            ...
            interpretation = self._generate_p_chart_interpretation(
                p_values, defective_counts, sample_sizes, p_bar, p_ucl, p_lcl, violations, process_stats
            )
    ...
    def _detect_rule_violations(
        self, values: List[float], center_line: float, ucl: float, lcl: float, rule_set: str = "western_electric"
    ) -> List[Tuple[int, int]]:
        """
        ...
        if rule_set == "western_electric":
            ...
            for i in range(1, n - 1):
                ...
                beyond_2sigma_count = sum((v >= zone_a_lower or v <= zone_b_lower_neg) for v in window)
            ...
            for i in range(2, n - 2):
                ...
                beyond_1sigma_count = sum((v >= zone_b_lower or v <= zone_c_lower_neg) for v in window)
            ...
            for i in range(14, n):
                ...
                in_zone_c = sum((v < zone_b_lower and v > zone_c_lower_neg) for v in window)
                if in_zone_c == 15:
            ...
            for i in range(7, n):
                window = values_array[i - 7 : i + 1]
                not_in_zone_c = all(v >= zone_b_lower or v <= zone_c_lower_neg for v in window)
    ...
    def _generate_xbar_r_interpretation(
        ...
        if x_out_of_control or len(x_violations) > 0:
            ...
            if violation_subgroups:
                interpretation.append(
                    f"Subgroups {', '.join(map(str, sorted(violation_subgroups)))} show pattern violations on the X-bar chart."
                )
        else:
        ...
        if r_out_of_control or len(r_violations) > 0:
            ...
            if violation_subgroups:
                interpretation.append(
                    f"Subgroups {', '.join(map(str, sorted(violation_subgroups)))} show pattern violations on the R chart."
                )
        else:
        ...
        if x_out_of_control or r_out_of_control or len(x_violations) > 0 or len(r_violations) > 0:
            interpretation.append(
                "Recommended Action: Investigate the special causes identified and take corrective action."
            )
        else:
    ...
    def _generate_xbar_s_interpretation(
        ...
        if s_out_of_control or len(s_violations) > 0:
            ...
            if violation_subgroups:
                interpretation.append(
                    f"Subgroups {', '.join(map(str, sorted(violation_subgroups)))} show pattern violations on the S chart."
                )
        else:
            interpretation.append("The process variation appears to be in statistical control.")
        ...
        interpretation.append(
            "The X-bar S chart is particularly suitable for this data as it provides better estimation of process variation with larger subgroup sizes."
        )
    ...
    def _generate_i_mr_interpretation(
        ...
        if autocorrelation is not None:
            ...
            if abs(autocorrelation) > 0.5:
                interpretation.append(
                    "WARNING: Strong autocorrelation detected. I-MR chart assumptions may be violated."
                )
                ...
                interpretation.append(
                    "CAUTION: Moderate autocorrelation detected. Results should be interpreted with care."
                )
        ...
        if i_out_of_control or len(i_violations) > 0:
            ...
            if violation_points:
                interpretation.append(
                    f"Points {', '.join(map(str, sorted(violation_points)))} show pattern violations on the Individuals chart."
                )
        else:
        ...
        if mr_out_of_control or len(mr_violations) > 0:
            ...
            if violation_points:
                interpretation.append(
                    f"Points {', '.join(map(str, sorted(violation_points)))} show pattern violations on the Moving Range chart."
                )
        else:
            interpretation.append("The process variation appears to be in statistical control.")
    ...
    def _generate_p_chart_interpretation(
        ...
        if process_stats["variable_sample_size"]:
            interpretation.append(
                "This chart uses variable sample sizes, resulting in different control limits for each subgroup."
            )
        ...
        if out_of_control or len(violations) > 0:
            ...
            if violation_points:
                interpretation.append(
                    f"Subgroups {', '.join(map(str, sorted(violation_points)))} show pattern violations."
                )
            ...
            if low_points:
                ...
                interpretation.append(
                    f"Subgroups {', '.join(map(str, low_subgroups))} show improved (lower) defect rates."
                )
        else:
        ...
        if p_bar > 0.1:
            interpretation.append(
                "NOTE: The defect rate is greater than 10%. Consider implementing process improvements."
            )
