...
def generate_process_data(
    mean: float = 10.0, std_dev: float = 1.0, n_samples: int = 100, random_state: Optional[int] = None
) -> np.ndarray:
    """
...
def generate_process_data_with_shifts(
    ...
    if shift_point is not None and 0 < shift_point < n_samples:
        data[shift_point:] = np.random.normal(
            loc=mean + shift_size * std_dev, scale=std_dev, size=n_samples - shift_point
        )
...
def generate_subgrouped_data(data: np.ndarray, subgroup_size: int = 5) -> pd.DataFrame:
    """
...
def calculate_control_limits(
    data: pd.DataFrame, control_type: str = "xbar_r", subgroup_size: int = 5
    ...
    if control_type == "xbar_r":
        ...
        # R chart limits
        limits["r"] = {"center": r_bar, "lcl": constants["D3"][n] * r_bar, "ucl": constants["D4"][n] * r_bar}
        ...
        # S chart limits
        limits["s"] = {"center": s_bar, "lcl": constants["B3"][n] * s_bar, "ucl": constants["B4"][n] * s_bar}
        ...
        limits["mr"] = {"center": mr_mean, "lcl": 0, "ucl": constants["D4"][2] * mr_mean}

    return limits


def detect_out_of_control_points(
    data: pd.DataFrame, limits: Dict[str, Dict[str, float]], chart_type: str = "xbar"
) -> List[int]:
    """
...
def detect_runs_and_trends(
    data: pd.DataFrame, column: str = "Mean", center_line: Optional[float] = None, run_length: int = 8
) -> Dict[str, List[int]]:
    """
...
def calculate_process_capability(
    data: DataSeries, usl: float, lsl: float, target: Optional[float] = None
) -> Dict[str, float]:
    """
...
def calc_oc_curve_points(n: int, c: int, p_values: Optional[np.ndarray] = None) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def calculate_arl(p: float, n: int, c: int) -> float:
    """
...
def is_normal(data: DataSeries, alpha: float = 0.05) -> Tuple[bool, float]:
    """
