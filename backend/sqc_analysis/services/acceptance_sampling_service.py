...
from .utils_service import calc_oc_curve_points
...
class AcceptanceSamplingService:
    ...
    def calculate_single_sampling_plan(
        ...
        result = {
            ...
            "aql": float(p1),
            "ltpd": float(p2),
            "oc_curve": {"p_values": p_values.tolist(), "pa_values": pa_values.tolist()},
            "aoq_curve": {
            ...
            "ati_curve": {"p_values": p_values.tolist(), "ati_values": ati_values.tolist()},
            "asn_curve": {"p_values": p_values.tolist(), "asn_values": asn_values.tolist()},
        }
    ...
    def calculate_double_sampling_plan(
        ...
        single_plan = self.calculate_single_sampling_plan(
            lot_size, acceptable_quality_level, rejectable_quality_level, producer_risk, consumer_risk
        )
                ...
                for c2_try in range(max(r1_try - 1, c1_try), min(n1 + n2, r1_try + 10)):
                    ...
                    for i, p in enumerate(p_values):
                        ...
                        for x1 in range(c1_try + 1, r1_try):
                            ...
                            pa_given_x1 = sum(stats.binom.pmf(x2, n2, p) for x2 in range(c2_try - x1 + 1))
        ...
        for i, p in enumerate(p_values):
            ...
            for x1 in range(best_c1 + 1, best_r1):
                ...
                # Probability of acceptance on second sample given x1 in first
                pa_given_x1 = sum(stats.binom.pmf(x2, n2, p) for x2 in range(best_c2 - x1 + 1))
    ...
    def lookup_ansi_z14_plan(self, lot_size: int, inspection_level: str = "II", aql: float = 1.0) -> Dict[str, Any]:
        """
    ...
    def calculate_skip_lot_plan(self, base_plan: Dict[str, Any], frequency_index: int = 2) -> Dict[str, Any]:
        """
    ...
    def calculate_continuous_sampling_plan(
        self, clearing_interval: int, sampling_fraction: float, acceptance_number: int = 0
    ) -> Dict[str, Any]:
        """
        ...
        result = {
            ...
            "acceptance_number": acceptance_number,
            "afi_curve": {"p_values": p_values.tolist(), "afi_values": afi_values.tolist()},
            "aoq_curve": {
                "p_values": p_values.tolist(),
    ...
    def compare_sampling_plans(self, plan_list: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        for i, plan in enumerate(plan_list):
            ...
            if plan_type == "single":
                plan_details.append(
                    {"type": "Single", "n": plan.get("sample_size"), "c": plan.get("acceptance_number")}
                )
                ...
                plan_details.append(
                    {"type": "CSP-1", "i": plan.get("clearing_interval"), "f": plan.get("sampling_fraction")}
                )
