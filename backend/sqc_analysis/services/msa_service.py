...
class MSAService:
    ...
    def calculate_gauge_rr_anova(
        ...
        if not all(measurements_per_combination["count"] == n_trials):
            raise ValueError(
                "Unbalanced design detected. All part-operator combinations must have the same number of measurements."
            )
        ...
        df.groupby("part_operator")[measurements_col].mean()
        ...
        tolerance = (
            df[measurements_col].max() - df[measurements_col].min()
        )  # Use data range as tolerance if not provided
    ...
    def calculate_gauge_rr_range(
        ...
        ranges = (
            df.groupby([parts_col, operators_col])[measurements_col].max()
            - df.groupby([parts_col, operators_col])[measurements_col].min()
        )
        ...
        variance_table = {
            ...
            "StdDev": [float(ev), float(av), float(grr), float(pv), float(tv)],
            "StudyVar": [float(sv_ev), float(sv_av), float(sv_grr), float(sv_pv), float(sv_tv)],
            "%StudyVar": [float(psv_ev), float(psv_av), float(psv_grr), float(psv_pv), 100.0],
            "%Tolerance": [float(ptol_ev), float(ptol_av), float(ptol_grr), None, None],
        }
    ...
    def attribute_agreement_analysis(
        ...
        if has_repeats:
            summary["Overall Repeatability"] = float(
                np.mean(
                    [
                        r["agreement_percent"]
                        for r in repeatability_results.values()
                        if not np.isnan(r["agreement_percent"])
                    ]
                )
            )
