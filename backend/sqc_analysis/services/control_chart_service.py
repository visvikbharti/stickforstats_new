...
class ControlChartService:
    ...
    def generate_chart_data(
        self, chart_type: str, process_parameters: Dict[str, Any], special_cause: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        ...
        if special_cause is None:
            raw_data = generate_process_data(mean=mean, std_dev=std_dev, n_samples=n_samples)
        ...
        if chart_type in ["xbar_r", "xbar_s"]:
            ...
            if chart_type == "xbar_r":
                ...
                out_of_control = {"xbar": out_of_control_xbar, "r": out_of_control_r}
            else:  # xbar_s
                out_of_control_s = detect_out_of_control_points(df, limits, chart_type="s")
                out_of_control = {"xbar": out_of_control_xbar, "s": out_of_control_s}

            # Detect runs and trends
            patterns = detect_runs_and_trends(df, column="Mean", center_line=limits["xbar"]["center"])

        elif chart_type == "imr":
            # For IMR chart, we use individual data points
            df = pd.DataFrame({"Measurement": raw_data})
            ...
            limits["mr"] = {"center": mr_mean, "lcl": 0, "ucl": 3.267 * mr_mean}  # D4 constant for n=2

            # Detect out-of-control points
            out_of_control_i = []
            for i, value in enumerate(df["Measurement"]):
                if value > limits["individuals"]["ucl"] or value < limits["individuals"]["lcl"]:
                    out_of_control_i.append(i)

            out_of_control_mr = []
            for i, value in enumerate(df["MovingRange"].dropna()):
                if value > limits["mr"]["ucl"] or value < limits["mr"]["lcl"]:
                    out_of_control_mr.append(i + 1)  # +1 because first MR is at index 1

            out_of_control = {"individuals": out_of_control_i, "mr": out_of_control_mr}

            # Detect runs and trends
            patterns = detect_runs_and_trends(df, column="Measurement", center_line=limits["individuals"]["center"])
    ...
    def analyze_control_chart(
        self, data: Union[pd.DataFrame, List[Dict[str, Any]]], chart_type: str, subgroup_size: int = 5
    ) -> Dict[str, Any]:
        """
    ...
    def generate_attributes_chart(
        ...
        if chart_type == "p":
            ...
            if special_cause and special_cause.get("type") == "shift":
                ...
                baseline_data = np.random.binomial(n=sample_size, p=proportion, size=n_subgroups)
                ...
                if 0 < shift_point < n_subgroups:
                    baseline_data[shift_point:] = np.random.binomial(
                        n=sample_size, p=shift_proportion, size=n_subgroups - shift_point
                    )

                # Sample sizes can be variable for p charts
                if special_cause.get("variable_sample_size", False):
                    sample_sizes = np.random.randint(int(sample_size * 0.7), int(sample_size * 1.3), size=n_subgroups)
                else:
                ...
                defective_counts = np.random.binomial(n=sample_sizes, p=proportion, size=n_subgroups)
                proportions = defective_counts / sample_sizes
            ...
            # Detect out-of-control points
            out_of_control = []
            for i, (p, lcl, ucl) in enumerate(zip(df["Proportion"], limits["p"]["lcl"], limits["p"]["ucl"])):
                if p < lcl or p > ucl:
                    out_of_control.append(i)

            # Detect runs and trends
            patterns = detect_runs_and_trends(df, column="Proportion", center_line=p_bar)
            ...
            # Create DataFrame
            df = pd.DataFrame(
                {"Subgroup": range(1, n_subgroups + 1), "SampleSize": sample_size, "Defective": baseline_data}
            )
            ...
            # Detect runs and trends
            patterns = detect_runs_and_trends(df, column="Defective", center_line=np_bar)
            ...
            if special_cause and special_cause.get("type") == "shift":
                # Create baseline data
                baseline_data = np.random.poisson(lam=mean_defects, size=n_subgroups)
                ...
                # Replace data after shift point
                if 0 < shift_point < n_subgroups:
                    baseline_data[shift_point:] = np.random.poisson(lam=shift_mean, size=n_subgroups - shift_point)
            ...
            # Create DataFrame
            df = pd.DataFrame({"Subgroup": range(1, n_subgroups + 1), "Defects": baseline_data})

            # Calculate control limits
            c_bar = df["Defects"].mean()

            limits = {
                "c": {"center": c_bar, "lcl": max(0, c_bar - 3 * np.sqrt(c_bar)), "ucl": c_bar + 3 * np.sqrt(c_bar)}
            }
            ...
            # Detect runs and trends
            patterns = detect_runs_and_trends(df, column="Defects", center_line=c_bar)
            ...
            if special_cause and special_cause.get("type") == "shift":
                ...
                if process_parameters.get("variable_units", False):
                    units = np.random.randint(int(base_units * 0.7), int(base_units * 1.3), size=n_subgroups)
                else:
                ...
                defects = np.random.poisson(lam=expected_defects, size=n_subgroups)
                ...
                if 0 < shift_point < n_subgroups:
                    ...
                    defects[shift_point:] = np.random.poisson(
                        lam=expected_defects_shifted, size=n_subgroups - shift_point
                    )
            ...
            # Detect out-of-control points
            out_of_control = []
            for i, (u, lcl, ucl) in enumerate(zip(df["DefectsPerUnit"], limits["u"]["lcl"], limits["u"]["ucl"])):
                if u < lcl or u > ucl:
                    out_of_control.append(i)

            # Detect runs and trends
            patterns = detect_runs_and_trends(df, column="DefectsPerUnit", center_line=u_bar)
    ...
    def detect_control_chart_patterns(
        ...
        if chart_type in ["xbar", "xbar_r", "xbar_s"]:
            ...
            if control_limits is None:
                ...
                control_limits = calculate_control_limits(
                    df, control_type="xbar_r" if "Range" in df.columns else "xbar_s", subgroup_size=subgroup_size
                )
            ...
            if control_limits is None:
                subgroup_size = 5  # Default if not specified
                control_limits = calculate_control_limits(df, control_type="xbar_r", subgroup_size=subgroup_size)
            ...
            if control_limits is None:
                subgroup_size = 5  # Default if not specified
                control_limits = calculate_control_limits(df, control_type="xbar_s", subgroup_size=subgroup_size)
            ...
            if control_limits is None:
                ...
                control_limits = {"mr": {"center": mr_mean, "lcl": 0, "ucl": 3.267 * mr_mean}}  # D4 constant for n=2
        ...
        if len(data_values) > 10:
            ...
            if cycles:
                ...
                patterns["cyclical"] = {"detected": False}
        ...
        if chart_type in ["xbar", "xbar_r", "xbar_s", "imr", "individuals"]:
            sigma = (
                control_limits[chart_type.replace("xbar", "xbar") if "xbar" in chart_type else "individuals"]["ucl"]
                - control_limits[chart_type.replace("xbar", "xbar") if "xbar" in chart_type else "individuals"][
                    "center"
                ]
            ) / 3
