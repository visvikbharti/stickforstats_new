...
class EconomicDesignService:
    ...
    def calculate_optimal_design_parameters(
        ...
        _ = process_parameters.get("hourly_production", 100)  # Units produced per hour
        ...
        def expected_hourly_cost(params):
            ...
            if chart_type == "xbar":
                detection_prob = (
                    1 - stats.norm.cdf(k - shift_size * math.sqrt(n)) + stats.norm.cdf(-k - shift_size * math.sqrt(n))
                )
            else:
            ...
            _ = 1 / false_alarm_prob  # In-control ARL
        ...
        result = optimize.minimize(
            ...
            best_params,
            method="Nelder-Mead",
            bounds=[(min_sample_size, max_sample_size), (min_sampling_interval, max_sampling_interval), (2.0, 4.0)],
        )
        ...
        if chart_type == "xbar":
            detection_prob = (
                1
                - stats.norm.cdf(k_opt - shift_size * math.sqrt(n_opt))
                + stats.norm.cdf(-k_opt - shift_size * math.sqrt(n_opt))
            )
        else:
        ...
        if chart_type == "xbar":
            ...
            center_line = (
                process_parameters.get("p", 0.05) if chart_type in ["p", "np"] else process_parameters.get("c", 5)
            )
    ...
    def compare_design_alternatives(
        self, process_parameters: Dict[str, Any], cost_parameters: Dict[str, Any], alternatives: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        mean_time_to_failure = process_parameters.get("mean_time_to_failure", 100)  # Hours until process goes OOC
        shift_size = process_parameters.get("shift_size", 2.0)  # Size of shift in std_dev units
        _ = process_parameters.get("std_dev", 1.0)  # Process standard deviation
        _ = process_parameters.get("hourly_production", 100)  # Units produced per hour
        ...
        for i, alternative in enumerate(alternatives):
            ...
            if chart_type == "xbar":
                detection_prob = (
                    1
                    - stats.norm.cdf(k_factor - shift_size * math.sqrt(sample_size))
                    + stats.norm.cdf(-k_factor - shift_size * math.sqrt(sample_size))
                )
            else:
        ...
        if comparison_results:
            ...
            for result in comparison_results:
                ...
                result["costs"]["cost_difference_percent"] = (
                    (result["costs"]["cost_difference"] / best_cost * 100) if best_cost > 0 else 0
                )
    ...
    def calculate_cost_of_quality(self, quality_parameters: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        appraisal_total = inspection_cost + testing_cost + audit_cost + calibration_cost + other_appraisal

        internal_failure_total = scrap_cost + rework_cost + downtime_cost + yield_loss_cost + other_internal

        external_failure_total = (
            warranty_cost + returns_cost + complaint_cost + liability_cost + lost_sales_cost + other_external
        )
