...
const BootstrapCalculator = ({ project, projectData, onSaveResult }) => {
  ...
  const handleDataSelection2 = (event) => {
    const dataId = event.target.value;
    setSelectedDataId2(dataId);
  };

  // Handle file upload
  const handleFileUpload = (event, isSecondSample = false) => {
  ...
  const handleSimParamChange = (param) => (event) => {
  ...
  const handleSimParamSliderChange = (param) => (event, newValue) => {
    setSimulationParams({
      ...simulationParams,
      [param]: newValue
    });
  ...
  const renderDataSummary = (data, label = 'Data Summary') => {
    ...
    return (
      <Box sx={{ mt: 2, p: 2, bgcolor: 'background.paper', borderRadius: 1 }}>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={4}>
            <Typography variant="body2">Min: {min.toFixed(4)}</Typography>
          </Grid>
          <Grid item xs={4}>
            <Typography variant="body2">Max: {max.toFixed(4)}</Typography>
          </Grid>
        </Grid>
      </Box>
    );
  ...
  // Render component for selecting saved data
  const renderSavedDataSelection = () => (
  ...
  // Render component for entering new data
  const renderNewDataInput = () => (
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={intervalType === 'BOOTSTRAP_DIFFERENCE' ? 6 : 12}>
        ...
        {renderDataSummary(parsedData, "First Sample Summary")}
      </Grid>
      ...
      {intervalType === 'BOOTSTRAP_DIFFERENCE' && (
        <Grid item xs={12} md={6}>
          ...
          {renderDataSummary(parsedData2, "Second Sample Summary")}
        </Grid>
      )}
