...
export function applyWesternElectricRules(values, cl, ucl, lcl) {
  ...
  // Rule 5: 2 out of 3 points beyond 2� (same side)
  for (let i = 2; i < n; i++) {
    ...
    if (aboveCount >= 2 || belowCount >= 2) {
      violations.push({
        rule: 5,
        description: '2 of 3 beyond 2�',
        points: [i - 2, i - 1, i]
      });
    }
  }

  // Rule 6: 4 out of 5 points beyond 1� (same side)
  for (let i = 4; i < n; i++) {
  ...
  for (let i = 14; i < n; i++) {
    ...
    if (subset.every(v => v <= ucl1 && v >= lcl1)) {
      violations.push({
        rule: 7,
        description: '15 points within 1�',
        points: Array.from({ length: 15 }, (_, j) => i - 14 + j)
      });
    }
  }

  // Rule 8: 8 points beyond 1� (either side)
  for (let i = 7; i < n; i++) {
    const subset = values.slice(i - 7, i + 1);
    if (subset.every(v => v > ucl1 || v < lcl1)) {
      violations.push({
        rule: 8,
        description: '8 points beyond 1�',
 ...
 * @param {number} h - Decision interval (typically 4� or 5�)
