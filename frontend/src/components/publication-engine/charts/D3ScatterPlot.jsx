...
const D3ScatterPlot = ({ svgRef, onScalesReady }) => {
  ...
  useEffect(() => {
    ...
    g.append('text').attr('x', innerW / 2).attr('y', innerH + 45).attr('text-anchor', 'middle')
      ...
      .attr('font-weight', state.xAxis.fontWeight).attr('fill', state.xAxis.color).text(state.xAxis.label);
    g.append('text').attr('transform', 'rotate(-90)').attr('x', -innerH / 2).attr('y', -45).attr('text-anchor', 'middle')
      ...
      .attr('font-weight', state.yAxis.fontWeight).attr('fill', state.yAxis.color).text(state.yAxis.label);
