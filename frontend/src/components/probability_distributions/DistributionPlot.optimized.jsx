...
// Import specific d3 functions instead of the entire library
import { 
  select, scaleLinear, scaleBand, axisBottom, axisLeft, 
  line, area, curveCatmullRom, sum 
    ...
    switch (type) {
      ...
      case 'poisson': {
        ...
        return xValues.map(k => {
          if (k < 0) return 0;
          const logPmf = k * Math.log(lambda) - lambda - sum(Array.from({ length: k }, (_, i) => Math.log(i + 1)));
      ...
      case 'binomial': {
        ...
        return xValues.map(k => {
          if (k < 0 || k > n) return 0;
          const logComb = sum(Array.from({ length: k }, (_, i) => Math.log(n - i))) - 
                        sum(Array.from({ length: k }, (_, i) => Math.log(i + 1)));
    ...
    if (showPdf) {
      if (type === 'normal') {
        ...
        const lineGenerator = line()
          ...
          .y(d => yScale(d))
          .curve(curveCatmullRom.alpha(0.5)); // Smoother curve
        ...
        if (fillArea && fillRange[0] !== null && fillRange[1] !== null) {
          ...
          const areaGenerator = area()
            ...
            .y0(yScale(0))
            .y1(d => yScale(d))
            .curve(curveCatmullRom.alpha(0.5)); // Match the line curve
