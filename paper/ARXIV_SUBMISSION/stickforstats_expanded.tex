...
\author{
  Vishal Bharti$^{1,*}$~\orcidicon{0009-0003-1431-4457}
  \And
  Debojyoti Chakraborty$^{1,2,*}$~\orcidicon{0000-0003-1460-7594}
}
...
\Address{
  ...
  ORCID: \href{https://orcid.org/0009-0003-1431-4457}{0009-0003-1431-4457}\\
  ...
  ORCID: \href{https://orcid.org/0000-0003-1460-7594}{0000-0003-1460-7594}\\
  CSIR-Institute of Genomics and Integrative Biology\\
  Mathura Road, New Delhi 110025, India\\
...
\textbf{Paper Parser (SQS).} This module analyzes uploaded PDF manuscripts to detect common statistical reporting errors, computing a Statistical Quality Score (SQS) on a 0--100 scale. Drawing on JARS-Quant guidelines and documented statistical pitfalls \citep{appelbaum2018jars, keselman1998statistical}, the parser identifies issues such as missing effect sizes, unreported assumption checks, incomplete test statistics, and p-value reporting problems. The tool generates structured reports with specific recommendations for improving statistical reporting quality.
...
\item We present a comprehensive validation study comparing \pkg{StickForStats} results against established reference implementations (\pkg{SciPy}, \proglang{R}), demonstrating agreement to 14+ decimal places for standard statistical tests.
\item We document a backend test suite of 38 automated tests ensuring Design Contract compliance: that no statistical result can exist without Guardian context.
...
Design Contract test suite$^a$ & \checkmark & -- & -- & -- & -- \\
Web-based interface & \checkmark & -- & -- & -- & -- \\
Open source & \checkmark & -- & \checkmark & \checkmark & \checkmark \\
High-precision option & \checkmark & -- & Partial$^b$ & -- & -- \\
Educational content & \checkmark & -- & -- & Partial & \checkmark \\
Code export (R/Python) & \checkmark & -- & Native & -- & -- \\
\bottomrule
\multicolumn{6}{l}{\small $^a$38 automated backend tests enforcing assumption validation compliance} \\
\multicolumn{6}{l}{\small $^b$Via Rmpfr package, not integrated} \\
\end{tabular}
\end{table}
...
\pkg{StickForStats} follows a three-tier architecture comprising a user interface layer (\proglang{React}), an application layer (\proglang{Django} REST Framework with Guardian integration), and a data layer (database with optional caching).
...
\begin{itemize}
\item \textbf{Frontend:} React 18 with Material-UI for consistent design, Recharts for interactive visualizations (with Plotly.js for specialized 3D plots), and MathJax for mathematical notation rendering.
\item \textbf{Backend:} Django 4.2 with Django REST Framework for API design, optional Celery integration for asynchronous task processing in production deployments, and comprehensive logging.
\item \textbf{Statistical Engine:} NumPy $\geq$1.24 for array operations, SciPy $\geq$1.10 for statistical functions, statsmodels $\geq$0.14 for regression diagnostics, and mpmath for arbitrary precision.
\item \textbf{Data Layer:} PostgreSQL for production deployments (SQLite for development and single-user installations), optional Redis caching with automatic fallback to in-memory cache, and file storage for uploaded datasets.
\end{itemize}
...
\caption{StickForStats system architecture showing the three-tier design: user interface (React), application layer (Django REST with Guardian integration), and data layer (database, optional cache, file storage). The Guardian layer intercepts all statistical test requests.}
...
\subsection{Frontend Guardian components}

The frontend provides four specialized React components for displaying Guardian reports:

\begin{itemize}
\item \textbf{GuardianReportDisplay}: A comprehensive expandable panel showing all assumption checks, violations with severity coloring, confidence score gauge, and alternative test recommendations. This component appears above statistical results, ensuring users see assumption status before interpreting results.

\item \textbf{GuardianBadge}: A compact chip indicator showing confidence score percentage and violation counts. Placed next to result headers for at-a-glance status. Color-coded: green (success), orange (warning), red (critical).

\item \textbf{ConfidenceGauge}: A visual meter displaying the 0-100 confidence score with gradient coloring and threshold markers at 60\% and 80\%.

\item \textbf{ViolationCard}: Individual violation display showing assumption name, severity level, test statistics with p-values, and actionable recommendations.
\end{itemize}

A custom React hook (\code{useGuardianReport}) extracts Guardian context from API responses, providing ready-to-use props for display components. The hook handles null/undefined responses gracefully and includes helper functions for compliance checking, violation summarization, and status message generation.

\textbf{Design Contract enforcement}: If an API response lacks Guardian context (missing \code{guardian\_report} or \code{assumptions\_checked} fields), the frontend displays a ``Missing Guardian Context'' warning alert, making it visually apparent when results lack proper assumption validation.
...
POST /api/v1/stats/ttest/            # Independent/paired t-test
POST /api/v1/stats/anova/            # One-way ANOVA
POST /api/v1/stats/correlation/      # Pearson/Spearman correlation
...
\section{Paper Parser and Statistical Quality Score} \label{sec:parser}

The Paper Parser (internally implemented as the Statistical Quality Score, or SQS, system) analyzes uploaded PDF manuscripts to identify statistical reporting problems before submission. This addresses the finding that many published papers contain statistical errors that could have been detected through systematic checking \citep{bakker2011misreporting}.
...
The parser implements 45 detection rules organized by six categories:

\begin{itemize}
\item \textbf{Effect size rules} (11 rules): Detect absence of standardized and unstandardized effect sizes, confidence intervals for effect sizes, and interpretation guidelines
\item \textbf{Assumption rules} (8 rules): Verify reporting of normality, homogeneity, independence, and other assumption tests
\item \textbf{Precision rules} (8 rules): Check for confidence intervals, exact $p$-values, and appropriate decimal reporting
\item \textbf{Reproducibility rules} (7 rules): Check for data availability statements, software versions, and seed reporting
\item \textbf{Sample rules} (6 rules): Check for N reporting, power analysis, attrition documentation, and inclusion criteria
\item \textbf{Guideline compliance rules} (5 rules): Verify JARS-Quant, CONSORT, and other reporting standard elements
\end{itemize}
...
# API endpoint
url = "http://localhost:8000/api/v1/stats/ttest/"
...
def analyze_with_guardian(data, test_type="ttest"):
    ...
    url = f"http://localhost:8000/api/v1/stats/{test_type}/"
    response = requests.post(url, json=data)
    result = response.json()
...
url = "http://localhost:8000/api/v1/stats/anova/"
...
url = "http://localhost:8000/api/v1/stats/correlation/"
...
High-precision arithmetic was validated by comparing results at different precision levels (standard 64-bit, 50-digit, 100-digit) and verifying convergence. The implementation uses Python's \pkg{mpmath} library for arbitrary-precision arithmetic, which has been extensively validated in the numerical computing community.

\begin{itemize}
\item Standard normal CDF and quantile functions: Agreement across precision levels
\item Student's $t$ distribution functions: Agreement across precision levels
\item $F$ distribution functions: Agreement across precision levels
\item Chi-square distribution functions: Agreement across precision levels
\end{itemize}
...
\begin{itemize}
\item \pkg{SciPy} 1.11.0 \citep{virtanen2020scipy}: Primary Python reference for statistical functions
\item \proglang{R} 4.4.1 \citep{rcore2023}: Cross-validation of case study results
\end{itemize}

All statistical tests, distributions, and numerical algorithms in \pkg{StickForStats} were validated against \pkg{SciPy}'s implementations. Additionally, case study results were cross-validated against \proglang{R} using the provided \texttt{validate\_against\_R.R} script in the replication package.
...
\pkg{StickForStats} includes an automated test suite with 38 backend tests covering Guardian logic and middleware compliance. This ensures Design Contract compliance: ``No statistical result may exist without an explicit, traceable assumption context.''

\subsubsection{Backend tests (38 tests)}

The backend test suite validates Guardian integration layer and middleware compliance:

\begin{table}[htbp]
\centering
\caption{Backend test coverage summary}
\label{tab:backend-tests}
\begin{tabular}{llr}
\toprule
Test Module & Coverage Area & Tests \\
\midrule
\code{test\_guardian\_integration} & GuardianEnrichedResult dataclass & 4 \\
 & GuardianServiceWrapper & 3 \\
 & Test type resolution & 4 \\
 & @guardian\_protected decorator & 2 \\
 & API compliance structure & 1 \\
 & Violation severity handling & 3 \\
 & Confidence score calculation & 2 \\
 & Design Contract compliance & 3 \\
\code{test\_guardian\_middleware} & Middleware initialization & 4 \\
...
\begin{itemize}
\item \code{GuardianEnrichedResult.to\_dict()} includes all required fields: \code{guardian\_report}, \code{assumptions\_checked}, \code{violations}, \code{confidence\_score}, \code{can\_proceed}, \code{alternative\_tests}, \code{\_guardian\_context}, and \code{\_contract\_compliant}
\item Critical violations block analysis by default unless Expert Mode is enabled
\item Warning violations allow proceeding with annotations
\item Statistical endpoints are correctly detected for middleware interception
\item Missing Guardian context is logged with Design Contract violation warnings
\end{itemize}
...
\begin{lstlisting}[language=bash,caption={Running the Guardian test suite}]
# Backend tests (Django)
cd backend
python manage.py test core.guardian.tests
# Output: Ran 38 tests in 1.273s - OK
\end{lstlisting}

All 38 tests pass, providing confidence that the Guardian system correctly enforces the Design Contract on the backend. Frontend Guardian components (\code{GuardianReportDisplay}, \code{GuardianBadge}, \code{ConfidenceGauge}, \code{ViolationCard}) and the \code{useGuardianReport} hook enforce display-level compliance by showing a ``Missing Guardian Context'' warning when API responses lack Guardian data.
...
The UCI Wine Quality dataset \citep{cortez2009wine} contains physicochemical properties and quality ratings for Portuguese wines. We analyze the red wine subset ($n = 1{,}599$), examining the relationship between alcohol content and quality.

\textbf{Analysis:} Correlation between alcohol percentage and quality rating (ordinal scale 3--9).

\textbf{Traditional approach:}
\begin{lstlisting}[language=Python]
r, p = stats.pearsonr(wine['alcohol'], wine['quality'])
print(f"r = {r:.3f}, p = {p:.2e}")
# Output: r = 0.476, p = 2.83e-91
\end{lstlisting}
...
#   Shapiro-Wilk: W=0.858, p<0.001
#   Quality is ordinal, not continuous normal
#
# - Linearity: PASS
#   Quadratic R^2 improvement: 1.0%
...
# Pooled effect (random): 0.263, 95% CI [0.213, 0.314]
# Heterogeneity: I^2 = 14.4%, Q = 12.85
#
# Guardian Report:
...
#   Egger's test: intercept=1.84, p=0.010
#   Funnel plot shows asymmetry
#   Recommendation: Consider publication bias in interpretation
\end{lstlisting}

\textbf{Guardian finding:} Egger's test ($p = 0.010$) indicates significant funnel plot asymmetry, suggesting publication bias. Researchers should interpret the pooled effect with caution and consider sensitivity analyses.
...
\textbf{Statistical Quality Score (SQS) for journals:} Beyond individual researcher use, the Paper Parser's SQS system has potential applications in peer review. The current implementation provides a scoring system (0--100) evaluating manuscripts across six categories: effect size reporting, assumption transparency, sample and power documentation, statistical precision, reproducibility indicators, and guideline compliance, using 45 detection rules with field-specific weights. Journals could configure field-specific thresholds, similar to plagiarism detection sensitivity settings. This would not replace human judgment but could standardize the statistical review component that currently varies across reviewers.
...
Guardian implements eight validators covering the most common assumptions for parametric tests: normality, variance homogeneity, independence, outliers, sample size adequacy, modality, linearity, and homoscedasticity. When violations are detected, Guardian provides severity classifications, specific recommendations, and alternative test suggestions. Validation against SciPy demonstrates computational correctness to 14+ decimal places. An automated test suite of 38 backend tests ensures that the Design Contract---``no statistical result may exist without an explicit, traceable assumption context''---is enforced at the API layer, while frontend Guardian components enforce display-level compliance.
