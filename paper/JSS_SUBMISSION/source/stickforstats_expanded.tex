...
\item We document a comprehensive test suite of 93 automated tests (38 backend, 55 frontend) ensuring Design Contract compliance: that no statistical result can exist without Guardian context.
...
\bottomrule
\multicolumn{6}{l}{\small $^a$93 automated tests enforcing assumption validation compliance} \\
...
\pkg{StickForStats} includes a comprehensive automated test suite with 93 tests covering both backend Guardian logic and frontend display components. This ensures Design Contract compliance: ``No statistical result may exist without an explicit, traceable assumption context.''
...
\subsubsection{Frontend tests (55 tests)}

The frontend test suite validates React components and hooks for Guardian display:

\begin{table}[htbp]
\centering
\caption{Frontend test coverage summary}
\label{tab:frontend-tests}
\begin{tabular}{llr}
\toprule
Test Module & Coverage Area & Tests \\
\midrule
\code{useGuardianReport.test.js} & Hook basic functionality & 4 \\
 & Guardian props extraction & 2 \\
 & Blocked analysis detection & 2 \\
 & Memoization behavior & 1 \\
 & \code{isGuardianCompliant} helper & 4 \\
 & \code{getViolationSummary} helper & 4 \\
 & \code{shouldSuggestExpertMode} helper & 4 \\
 & \code{getGuardianStatusMessage} helper & 6 \\
 & Design Contract compliance & 1 \\
\code{GuardianComponents.test.jsx} & GuardianReportDisplay rendering & 4 \\
...
\begin{itemize}
\item \code{useGuardianReport} hook correctly extracts Guardian context from API responses
\item ``Missing Guardian Context'' warning displays when Guardian data is absent
\item Confidence score gauge renders with appropriate coloring (green for high, red for low)
\item Violation cards display severity, p-values, and recommendations
\item Alternative test chips are clickable and trigger navigation
\item Expert Mode override warnings are prominently displayed
\end{itemize}
...
\begin{lstlisting}[language=bash,caption={Running the complete test suite}]
...
# Output: Test Suites: 2 passed, Tests: 55 passed
\end{lstlisting}
