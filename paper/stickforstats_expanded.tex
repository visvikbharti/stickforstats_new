...
\author{
  Vishal Bharti$^{1,2}$\\CSIR-Institute of Genomics and Integrative Biology
  \And
  Debojyoti Chakraborty$^{1,2,*}$\\CSIR-Institute of Genomics and Integrative Biology
}
...
\Address{
  Vishal Bharti\\
  $^1$Division of Chemical and Systems Biology\\
  ...
  URL: \url{https://github.com/visvikbharti/stickforstats_new}\\[1em]
...
\textbf{Code export.} For analyses, users can export equivalent \proglang{Python} code, facilitating verification and integration with existing workflows.
...
\item \textbf{Test execution:} The statistical test is executed (Guardian does not block, only reports).
...
This architecture ensures that users cannot receive statistical results without also receiving assumption validation information.
...
\begin{enumerate}
\item \textbf{Comprehensiveness:} Check all relevant assumptions for each test type.
...
The fourth principle deserves elaboration. Guardian does not prevent users from conducting analyses with violated assumptions. This design choice reflects the reality that assumption violations exist on a spectrum, that researchers may have domain knowledge justifying violations, and that blocking would create usability barriers. Instead, Guardian ensures that users are \emph{informed} about assumption status.
...
#   Levene's test: F=6.35, p=0.002
#   Variance ratio (max/min): 1.89
#   Recommendation: Consider Welch's ANOVA
#
# - Normality (versicolor): WARNING
#   Shapiro-Wilk: W=0.978, p=0.465 (PASS)
#   Shapiro-Wilk (virginica): W=0.971, p=0.258 (PASS)
#   Shapiro-Wilk (setosa): W=0.978, p=0.460 (PASS)
\end{lstlisting}
...
\textbf{Recommendation followed:} Welch's ANOVA, which does not assume equal variances, yields $F = 138.91$, $p < 0.001$. The conclusion (significant species differences) holds, but with appropriate method.
...
# Pooled effect (random): 0.332, 95% CI [0.251, 0.413]
# Heterogeneity: I^2 = 42.3%, Q = 19.07, p = 0.06
#
# Guardian Report:
...
#   Egger's test: intercept=1.89, p=0.032
#   Funnel plot shows asymmetry
#   Recommendation: Apply trim-and-fill correction
#   Estimated missing studies: 3 (left side)
#
# Trim-and-fill adjusted effect: 0.278
\end{lstlisting}

\textbf{Guardian finding:} Egger's test ($p = 0.032$) indicates significant funnel plot asymmetry, suggesting publication bias. The trim-and-fill method estimates 3 missing studies and adjusts the pooled effect downward from 0.332 to 0.278.

\textbf{Key insight:} Without Guardian's automatic publication bias check, researchers might report the inflated estimate of 0.332 without considering bias.
...
Meta-analysis & Publication bias & Inflated effect & Trim-and-fill \\
\bottomrule
\end{tabular}
\end{table}
...
\pkg{StickForStats}' primary contribution is the Guardian system---automatic assumption validation integrated into the analysis pipeline. This represents a shift from optional validation (requiring user initiative) to default validation (requiring user opt-out).

We do not claim this approach solves the reproducibility crisis. Assumption validation is one component of sound statistical practice. Study design, sampling, measurement validity, and appropriate inference all remain researcher responsibilities. However, by automating assumption checking, we remove one common failure point from the chain of statistical decision-making.
...
\subsubsection{User override}

Guardian does not prevent analysis with violated assumptions. Users can ignore warnings. This design choice prioritizes usability but means determined users can still produce invalid analyses.

\textbf{Rationale:} Blocking would create false negatives (preventing valid analyses where domain knowledge justifies violations) and would likely lead users to abandon the platform. Informed choice is preferable to forced compliance.
...
\textbf{Natural language explanations:} Plain-language descriptions of violations and recommendations for non-statistician users.
