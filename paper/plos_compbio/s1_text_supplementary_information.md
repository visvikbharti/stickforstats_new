...
|---|---|---|---|
| 1 | `normality` (NormalityValidator) | Normality of the distribution / residuals | Shapiro–Wilk, Anderson–Darling |
| 2 | `variance_homogeneity` (VarianceHomogeneityValidator) | Equal variances across groups | Levene, Brown–Forsythe median correction |
| 3 | `independence` (IndependenceValidator) | Independence of observations | Lag-1 Pearson autocorrelation on observation order |
| 4 | `outliers` (OutlierDetector) | Absence of influential outliers | IQR fencing and z-score screen |
| 5 | `sample_size` (SampleSizeValidator) | Adequate n for the test | Power-based minimum-n heuristics |
| 6 | `modality` (ModalityDetector) | Unimodality | Kernel-density (Silverman bandwidth) screen |
| 7 | `linearity` (LinearityValidator) | Linear relationship (correlation/regression) | Quadratic-term check with Wald–Wolfowitz runs test |
| 8 | `homoscedasticity` (HomoscedasticityValidator) | Constant residual variance (regression) | Breusch–Pagan on fitted-model residuals |
...
## S1.2 Programmatic access — code examples
...
To complement the four main case studies, Guardian was run on three classic R datasets
...
## S1.4 Test-suite coverage (Guardian)
