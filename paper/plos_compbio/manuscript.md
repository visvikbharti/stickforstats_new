...
To evaluate the retrospective verification engine---the statcheck-style consistency checker that recomputes reported p-values from the reported test statistic and degrees of freedom (cf. [14])---we assembled a corpus of 20 open-access articles from PubMed Central that report inline APA-style statistics, retrieved by a fixed query (Methods; the fetch script, corpus manifest, and results are provided under `paper/replication/manuscript_validation/`). The engine extracted 980 statistical claims, of which 468 carried a test statistic and 295 were fully specified (statistic, degrees of freedom, and p-value) and therefore recomputable. Recomputing each p-value with SciPy and comparing in a rounding- and inequality-aware manner (as statcheck does [14]), 275 of 295 recomputable claims (93.2%) were consistent and 20 were flagged for review.

Manual review of the 20 flagged claims (Table 7) delineates the engine's behaviour honestly. Nine were repeated-measures ANOVAs carrying a Greenhouse-Geisser or Huynh-Feldt sphericity correction: these articles report the *uncorrected* degrees of freedom alongside the *corrected* p-value, so any tool that recomputes from the reported df necessarily disagrees---a limitation shared with statcheck, which likewise does not recover the sphericity estimate. One was a sample-size-determination formula (Z = 1.96 with an assumed proportion), not a hypothesis test. One was an extraction artifact---a thousands-separated statistic ("3,950.2") truncated at the comma (a known parser fix). The remaining nine were genuine recompute-versus-reported discrepancies; for example, a reported F(6, 128) = 6.8, p = 0.03 recomputes to p approximately 3 x 10^-6. The exercise confirms that the engine recovers the large majority of correctly reported statistics while making its residual false-positive classes explicit rather than hidden.

**Table 7. Classification of the 20 claims flagged in the 20-article verification corpus.**

| Class | Count | Interpretation |
|---|---|---|
| Sphericity-corrected RM-ANOVA (Greenhouse-Geisser / Huynh-Feldt) | 9 | Uncorrected df reported with corrected p; a limitation shared with statcheck |
| Sample-size / power formula (e.g. Z = 1.96) | 1 | A critical value, not a test result |
| Extraction artifact (thousands separator) | 1 | Parser limitation (documented fix) |
| Genuine recompute-vs-reported discrepancy | 9 | Reporting inconsistencies surfaced for human review |
