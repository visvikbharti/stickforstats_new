...
Guardian computes a confidence score using severity-based penalty weights:

\begin{equation}
\text{Confidence} = \max\left(0, 1 - \frac{\sum_i \text{penalty}(v_i)}{\max\_penalty \times 1.2}\right)
\end{equation}

where penalty weights are $3.0$ for critical violations, $2.0$ for warnings, and $1.0$ for minor issues. These weights reflect the relative impact of each violation type on analysis validity: critical violations that invalidate results receive the highest penalty, warnings requiring attention receive moderate penalty, and minor acceptable concerns receive the lowest penalty.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{figures/figure2.pdf}
\caption{Guardian assumption validation workflow: (1) identify test requirements, (2) run validators in parallel, (3) calculate confidence score, (4) proceed or recommend alternatives based on threshold.}
\label{fig:workflow}
\end{figure}
